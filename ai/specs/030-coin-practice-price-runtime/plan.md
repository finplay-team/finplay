...
각 production 이슈는 자신이 구현한 controller의 요청·응답·오류를 이 문서와 `docs/api-contracts.md`의 030 계획 표에 대조한다. 구현과 계약이 다르면 코드를 임의로 맞추지 않고 먼저 spec 변경을 확정한다. 일치하면 같은 커밋에서 `ai/api-routes.md`의 해당 행을 계획 표에서 실제 라우트 표로 옮기고 `docs/api-contracts.md`의 해당 계약에서 “계획” 상태를 제거한다. 계획 4개가 한 번에 모두 구현된 것처럼 일괄 이동하지 않는다.
