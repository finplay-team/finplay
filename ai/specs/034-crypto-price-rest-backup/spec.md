...
> **(2026-08-23 정정 — 이슈 #536)** 아래 두 번째 체크박스("stale 판정의 기준은 관측 시각으로 바뀐다")는 **실제로는 성립하지 않는다.** `isStale`의 호출자인 `PriceStore.isPriceAvailable()`·`getLatestPrices()`가 둘 다 `receivedAt`(체결 시각)을 넘기고, `receivedAt`을 갱신하는 것은 웹소켓 `saveTick`뿐이다 — 즉 REST 폴링은 stale 판정에 영향을 주지 않는다. 같은 이유로 PRICE-REST-002의 마지막 항목("폴링이 연속 실패해 관측 시각이 10초를 넘기면 STALE로 떨어진다")도 성립하지 않는다.
>
> 036 이후 stale 개념의 유일한 소비자는 `CryptoPriceSnapshotService`(변동 카드 재료 신뢰도 게이트)이고, "마지막 체결이 방금 있었는가"로 재료를 거르는 것이 그 게이트의 목적에 어긋나지 않아 **현재 동작을 정본으로 인정하고 문서를 현실에 맞췄다.** 관측 시각 기준으로 되돌리는 것은 필요해지면 그때 별도로 판단한다. 체크박스 원문은 이력으로 유지한다(036 대체 표기와 같은 방식).
