...
| 03:46 | implementer | `.\gradlew.bat spotlessApply compileJava compileTestJava` 후 5개 클래스 `.\gradlew.bat test --tests ...`(전부 failures=0, XML로 확인) | 5번(진입 대본이 041 고정→2단계)이 노출한 기존 5개 통합 테스트 픽스처 파손 수정, 049 ORDERBASICS-015 게이트(4번) |
...
- 03:46 — 5번이 진입 대본을 041 고정에서 2단계로 되돌리며 드러난 기존 테스트 파손 5건을 수정(테스트 전용, 프로덕션 코드 무변경): `PracticeAttemptServiceTest` 기대값을 `CRYPTO_ORDER_BASICS_V1`로 정정, `PracticeExitPresetOcoIntegrationTest`·`PracticeScenarioFullJourneyIntegrationTest`는 픽스처가 041(`CRYPTO_STORY_V1`)로 전환하도록 고쳤는데 그 과정에서 049 ORDERBASICS-015 게이트(프리셋 선택·지정가는 시장가·지정가 왕복 필요)도 함께 걸려, 왕복이 시험 대상이 아닌 테스트는 워밍업 왕복 또는 엔티티 직접 조작으로 게이트를 우회하고 왕복이 시험 대상인 테스트는 실제로 왕복을 마치도록 재구성, `PracticeOrderBasicsNoAutoExitIntegrationTest`·`PracticeScenarioTickIntegrationTest`는 지정가 이전에 시장가 왕복을 추가. 5개 클래스 40개 테스트 전부 초록.
