...
- [x] ~~**화면 게이팅(2단계는 시장가·지정가만, 3단계는 손절·익절만 활성) — 프론트 합의 필요.**~~ —
      **결정됨.** `finplay-frontend/docs/frontend-reply-505.md`: 프론트가 비활성+이유로 처리한다.
      서버는 판정(#503)과 거부(4번 항목)까지만 제공한다.
- [x] ~~**`advance-script`를 누가 언제 호출하는가**(사용자 버튼 vs 조건 충족 시 자동) — 프론트 합의 필요.~~ —
      **결정됨.** 사용자 버튼이다. 서버는 엔드포인트와 조건 검사만 제공한다(5번 그대로 구현).
- [x] ~~**`scenarioStage`에 `"ORDER_BASICS"`라는 새 값이 생긴다** — 프론트가 알아야 한다.~~ —
      **전달됨.** `finplay-frontend/docs/frontend-reply-505.md`에서 `null`과 동일 취급하기로 합의,
      `docs/api/education.md`에도 6번에서 반영했다.
