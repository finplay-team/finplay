...
| 분산 락 | ~~하지 않는다(재구성은 멱등, 현재 단일 인스턴스 전제). 다중 인스턴스 전환 시 재검토~~ → **재검토 완료(이슈 #539).** `RankingRebuildLock`(시장 단위 Redis 락) 추가 — `plan.md` |
...
- ~~**분산 락을 두지 않는다.** 재구성은 멱등하다(같은 원장에서 같은 결과가 나오고, 임시 키 교체라 중간 상태도 남기지 않는다). 현재 단일 인스턴스 전제이므로 중복 실행 방지 장치를 지금 만들지 않는다. **다중 인스턴스로 전환하면 재검토 대상이다** — `plan.md`에 재검토 조건을 명시했다.~~ → **재검토 완료(이슈 #539).** ADR-0021이 확정한 블루-그린 배포에서는 이전 색 인스턴스가 다음 배포까지 떠 있어 위 "현재 단일 인스턴스 전제"가 이미 깨져 있었다. `RankingRebuildLock`(시장 단위 Redis 락)을 추가해 `rebuild(Market)`을 감쌌다 — 상세는 `plan.md` "다중 인스턴스 전환 시 재검토" 절.
...
- ~~**다중 인스턴스 중복 실행 방지(분산 락)** — 위 비즈니스 규칙 참고. 다중 인스턴스 전환 시 재검토.~~ → **해소됨(이슈 #539).** 위 비즈니스 규칙 참고.
...
- **재구성 도중 커밋된 매도는 교체에 덮인다(PR 리뷰 지적, 이슈 #279).** 재구성은 T0에 DB를 읽고 T2에 `RENAME`한다. 그 사이에 매도가 커밋되면 after-commit `addScoreWithRetry`가 **본 키**에 쓴 값이 T0 스냅샷으로 통째로 교체된다. 그 계좌의 첫 매도였다면 member 자체가 사라져, 다음 매도나 다음 재구성(최대 약 24시간)까지 `rank: null` + `REBUILDING`으로 보인다. 기동 훅도 예외가 아니다 — `ApplicationReadyEvent`는 웹 서버가 이미 요청을 받는 시점에 발화한다.
  - **고치지 않는 이유**: ZSET은 파생 데이터라 원장이 깨지지 않고, 다음 매도 또는 다음 재구성이 자가 치유한다. 창을 없애려면 재구성 중 쓰기를 막는 락이나 임시 키 동시 기록이 필요한데, 과설계 금지라는 확정과 어긋난다. 새벽 04:20이라 이 창에 매도가 걸릴 확률 자체도 낮다. **이슈 #539가 추가한 `RankingRebuildLock`은 이 한계를 고치지 않는다** — 그 락은 인스턴스 간 재구성 중복 실행만 막을 뿐, 재구성이 도는 동안 들어오는 정상 매도 커밋까지 막는 락이 아니다. 이 항목의 근거였던 "분산 락 없음" 전제만 없어졌을 뿐 한계 자체는 그대로 남는다.
- **기동 시 재구성이 걸리는 만큼 readiness도 늦어진다(PR #284 재리뷰).** 기동 훅은 동기라 원장이 커질수록 스프링 부트의 readiness 상태(`ReadinessState.ACCEPTING_TRAFFIC`) 발행이 그만큼 늦어지고, 지연이 재구성 때문이라는 단서가 로그에 남지 않는다. **`/actuator/health/readiness`를 호출해 확인하려 들지 말 것** — 이 프로젝트는 `management.endpoint.health.probes.enabled`를 켜지 않아(어느 프로필에도 `management` 설정이 없다) 그 경로가 열려 있지 않고, 배포 스모크도 `/actuator/health`만 부른다(`deploy/README.md`). 지연은 기동 로그의 재구성 시작·완료 시각으로 본다. **리스너 순서로는 해결할 수 없다** — `EventPublishingRunListener.ready()`가 `publishEvent(ApplicationReadyEvent)`로 리스너를 전부 동기 디스패치한 **다음** 줄에서 `AvailabilityChangeEvent.publish(ACCEPTING_TRAFFIC)`를 부르기 때문이다(바이트코드 확인). 한때 `@Order(Ordered.LOWEST_PRECEDENCE)`를 붙였다가 걷어낸 이유가 이것이고, 게다가 `ApplicationListenerMethodAdapter.resolveOrder`가 `@Order` 없을 때 이미 `LOWEST_PRECEDENCE`를 돌려줘 값 자체도 기본값과 같았다.
  - **고치지 않는 이유**: 지연을 실제로 없애려면 리스너 안에서 별도 스레드로 넘겨야 하는데, RANK-001 `plan.md`가 `@EnableAsync`·전용 스레드풀을 추가하지 않기로 못박아 뒀다. 그 결정을 새로 뒤집을 만큼 현재 원장 규모에서 지연이 크지 않다. 비동기로 옮기면 기동 직후 짧은 `REBUILDING` 구간이 생기는데 `status`가 이미 그 상태를 표현하므로, 원장이 커져 지연이 실제로 문제가 되면 그때 `plan.md` 결정과 함께 재검토한다.
