...
| 분산 락 | ~~없음(멱등·단일 인스턴스 전제). **다중 인스턴스 전환 시 재검토** — 아래 별도 절~~ → **재검토 완료(이슈 #539).** `RankingRebuildLock`(시장 단위 Redis 락) 추가 — 아래 별도 절 |
...
> **재검토 완료 (이슈 #539).** 아래 조건 1(임시 키 충돌)이 실제로 발생했다 — ADR-0021이 확정한 블루-그린 배포에서는 이전 색 인스턴스가 다음 배포까지 계속 떠 있어, 이 절이 "지금은 아니다"라고 전제했던 단일 인스턴스 조건이 이미 깨져 있었다. `RankingRebuildLock`(시장 단위 Redis 락, `CryptoWatchLock`·`StockCollectionLock`과 같은 패턴)을 추가해 `rebuild(Market)`을 감쌌다. 아래는 이 결정 이전의 원래 기록이며, 조건 1의 피해 분석은 실제보다 낙관적이었다 — 정정은 그 항목 바로 아래에 남긴다.
...
1. **임시 키 충돌** — 두 인스턴스가 같은 `ranking:{market}:rebuild` 키에 동시에 `ZADD`하면 서로의 중간 결과가 섞인다. 결과 자체는 같은 원장에서 나오므로 값이 어긋나지는 않지만, 한쪽이 `RENAME`한 뒤 다른 쪽이 남은 `ZADD`를 계속하면 새로 만들어진 임시 키가 다음 재구성까지 남는다. 인스턴스별 임시 키 suffix나 분산 락(`SET NX`)이 필요해진다.
   ...
   > **정정 (이슈 #539).** 위 분석은 실제보다 낙관적이었다 — `RankingStore.replaceAll`은 `DEL` → 청크 `ZADD` → `RENAME` 순서로 실행되므로, 두 인스턴스가 겹치면 한쪽의 `DEL`이 다른 쪽이 `ZADD`로 채우던 내용을 실제로 지운다. 남는 것은 찌꺼기 임시 키가 아니라 매도 이력이 있는 계좌가 랭킹에서 조용히 빠지는 **데이터 유실**이고, 다음 재구성(최대 24시간 뒤)까지 알아챌 방법이 없다.
