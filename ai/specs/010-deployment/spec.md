...
> `dev`에 머지되면 GitHub Actions가 이미지 빌드부터 ALB 전환까지 수행한다. 아래 "자동 배포(CD)" 절이
...
MVP 기능을 실제 환경에 올려 팀과 시연자가 확인할 수 있게 한다. PR 단계에서 최소 CI로 전체 build·테스트를 통과시키고, `dev`에 머지되면 파이프라인이 블루-그린으로 배포하며, 배포 후 담당자가 스모크 스크립트로 기본 동작을 확인한다.
...
- **리뷰어가 PR을 `dev`에 머지하면 그것으로 배포가 끝난다** — 배포 담당자가 따로 할 일이 없다 (ADR-0021).
...
- [ ] **CI는 CD의 전제다** (2026-08-12, ADR-0021). `dev` 머지가 곧 배포이므로, 머지 전에 build·테스트를 거르는 이 워크플로우가 공개 URL 앞의 유일한 자동 게이트가 된다. **`dev` 브랜치 보호 규칙의 required status check로 등록돼 있어야 한다** — 등록돼 있지 않으면 빌드가 깨진 코드도 머지되고 그대로 배포된다.
...
**`dev`에 머지되면 GitHub Actions가 이미지 빌드부터 ALB 전환까지 수행한다. 사람의 개입 지점은 PR 머지 버튼 하나다.** 결정 근거·대안은 **ADR-0021**이 정본이고, 이 절은 그것을 요구사항으로 옮긴 것이다. 구축 순서·AWS 콘솔 설정·실패 대응은 **`deploy/cd-runbook.md`** 가 정본이다.

- [ ] `dev` push를 트리거로 배포 워크플로우가 실행된다. `main`은 배포 소스가 아니다 — 시연·심사 시점의 스냅샷이다.
...
- [ ] IAM 역할의 신뢰 정책은 **브랜치까지 한정한다** (`repo:finplay-team/finplay-backend:ref:refs/heads/dev`). 배포 워크플로우는 `pull_request`를 트리거로 쓰지 않는다 (공개 레포).
