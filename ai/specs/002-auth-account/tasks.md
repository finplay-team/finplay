...
- [ ] 통합 테스트(인증→가입→계좌 2개, 가입 토큰 재사용 거부, 가입 실패 후 같은 토큰 재시도 성공, Fake OAuth 신규 가입·기존 이메일 409 거부, 토큰 회전) + ai/api-routes.md 갱신 — **Issue #6에서 토큰 회전 MySQL 통합·롤백 테스트와 라우트 동기화 완료, 나머지 시나리오 잔여**
...
- [x] 전체 회귀(Issue #9/#10 포함)·`./gradlew build`·`ai/api-routes.md`·`ai/specs/002-auth-account/tasks.md` 동기화
...
- [x] 전체 회귀·`./gradlew build`·`ai/api-routes.md`·`ai/specs/002-auth-account/tasks.md` 동기화
...
- [x] Fake `EmailSender` + Testcontainers MySQL 통합 테스트(EMAIL/OAuth 각 성공 흐름, 실패 시 기존 `users`·`accounts`·`orders`·`executions` 불변)·전체 회귀·`./gradlew build`·`ai/api-routes.md`·`ai/specs/002-auth-account/tasks.md` 동기화
...
- [x] Testcontainers MySQL 통합 테스트(발송→확인 성공, 재사용·5회초과·만료·재발송무효화·타인요청 격리, 확인 성공 후 기존 Refresh Token 401, 계좌·주문·체결·OAuth 연결 불변, 동시 이메일 경합 409+원자적 롤백)·전체 회귀·`./gradlew build`·`ai/api-routes.md`·`ai/specs/002-auth-account/tasks.md` 동기화
...
- [x] `ai/prd.md` AUTH-005에 비밀번호 변경 항목 추가·`ai/api-routes.md`·`docs/api-contracts.md`·`ai/specs/002-auth-account/tasks.md` 동기화·전체 회귀·`./gradlew build`
...
- [x] `ai/prd.md`에 `AUTH-006` 신설·§5 공통 오류표 신규 409 코드·엔드포인트 목록 추가와 `ai/api-routes.md`·`docs/api-contracts.md`·`ai/specs/002-auth-account/tasks.md` 동기화
...
- [x] `PasswordResetVerification.incrementAttemptCount()`·`consume(now)`(#115가 `attempt_count`·`consumed_at`을 미뤄둔 부분)와 `PasswordResetVerificationRepository.findFirstByEmailAndCodeHashIsNotNullOrderByCreatedAtDesc(email)` 추가 (+ `@DataJpaTest`로 최신 발송 행 반환·거부 행(`code_hash` NULL) 건너뜀·다른 이메일 미반환 검증). 신규 Flyway 마이그레이션 없음
- [x] `PasswordResetService.validateAndConsumeCode(email, code)` — 요청 이력 없음/소비됨/만료 → 5회 초과(증가+즉시 만료 후 429) → 코드 불일치(증가 후 400) → 미가입 400 → 소셜 전용 409 → 소비 순서 구현, 재설정 대상 `User` 반환. 계정 상태 판정을 인증번호 검증 **뒤에** 두어 확인 경로가 무제한 계정 열거 오라클이 되지 않게 한다(#115 D4의 발송 제한이 이 경로에는 없기 때문). `@Transactional` 미선언 — 호출자 트랜잭션에 편입 (+ 단위 테스트, 실패 경로에서 `consume` 미호출 확인)
- [x] `AuthService.confirmPasswordReset(email, code, newPassword)`(`@Transactional(noRollbackFor = BusinessException.class)`) — 검증·소비 → `User.changePassword(encode(newPassword), now)` → `saveAndFlush` → `RefreshTokenRepository.revokeAllActiveByUserId` 순서로 구현하고 **새 토큰 쌍은 발급하지 않는다**(#114와 의도적으로 다른 전 기기 로그아웃). `PasswordResetConfirmRequest`(이메일·6자리 코드·새 비밀번호 8~100자)·`PasswordResetController` `POST /api/auth/password-resets/confirm`(204 본문 없음)·`SecurityConfig.PUBLIC_POST_PATHS` 공개 경로 추가 (+ `InOrder` 단위 테스트, `@WebMvcTest`로 204·400·429·409와 비인증 접근 허용 검증)
- [x] Testcontainers MySQL 통합 테스트(발송→확인 성공, 새 비밀번호 로그인 성공·기존 비밀번호 401, 재사용·5회초과 후 정답 거부·만료·재발송 무효화·타인 이메일 격리, 확인 성공 후 기존 Refresh Token으로 `/api/auth/refresh` 401, 검증 실패 시 `attempt_count` 증가 커밋 + `password_hash` 불변, 이메일·닉네임·`social_accounts`·계좌·잔액·주문·체결 불변)·전체 회귀·`./gradlew build`
- [x] `ai/prd.md` `AUTH-006` 구현 단계 문구·확인 단계 계약·엔드포인트 목록 갱신과 `ai/api-routes.md`·`docs/api-contracts.md`(다른 기기 Access Token 잔존 한계 명시 포함)·`ai/specs/002-auth-account/tasks.md` 동기화
...
## Issue #121 인증번호 시도 횟수 갱신 유실 — 남은 두 서비스 원자성 확보와 공통화 작업 항목 (5개)

상세 설계는 `issue-121-plan.md` 참고.

- [x] `EmailVerificationRepository.findFirstByEmailOrderByCreatedAtDesc`에 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 적용 — 동시 오답 5건/6건/8건 버스트 회귀 테스트를 **먼저 작성해 `@Lock` 없는 코드에서 실패하는 것을 실측**한 뒤 잠금을 넣고 통과를 확인한다(두 실행의 실측값을 `run-log.md`에 기록, 생성 SQL에 `... for update` 확인). `EmailVerificationService` 본문은 한 줄도 바꾸지 않는다
- [x] `EmailChangeVerificationRepository.findFirstByUserIdAndNewEmailOrderByCreatedAtDesc`에 같은 잠금 적용 — 같은 "수정 전 실패 실측 → 수정 → 통과" 절차를 밟고, 같은 커밋에서 `EmailChangeConfirmIntegrationTest`가 잠금 쿼리를 트랜잭션 밖에서 직접 호출하는 6곳(152·164·185·201·247·287행)을 jdbcTemplate 헬퍼로 교정한다(read-only 트랜잭션에서 `SELECT ... FOR UPDATE`가 거부되기 때문). 생성 SQL에 `users` 조인이 생기지 않는지 함께 확인한다. `validateAndConsumeCode`에 `@Transactional`을 새로 붙이지 않는다
- [x] `VerificationCodeHasher`(생성자에서 시크릿을 받는 final 클래스)·`VerificationCodePolicy`(`@Component`, 5분 만료·60초 재발송·1시간 5회·하루 10회·최대 5회 시도 상수 + `generateCode`·`expiresAt`·`isAttemptLimitReached`) 도입과 세 서비스 치환 — 각 서비스의 `@Value` 생성자 파라미터를 그대로 두어 `EMAIL_VERIFICATION_SECRET`(가입 인증·이메일 변경) / `PASSWORD_RESET_SECRET`(재설정) 소유 관계를 바꾸지 않고, 한쪽 시크릿으로 만든 코드가 다른 쪽 경로에서 통하지 않는 가드 테스트를 추가한다
- [x] `checkSendRateLimit`(집계 함수만 인자로 받아 60초 `> 0` → 1시간 `>= 5` → 하루 `>= 10` 순서와 비교 연산자를 그대로 이관)과 `expirePreviousCodes`(`ExpirableVerification` 인터페이스 + `expireAll`, 조회 쿼리는 세 서비스에 그대로 남김) 공통화 — 발송 제한 429 경계 3종과 재발송 시 이전 코드 무효화가 세 도메인에서 불변임을 확인한다 — **`expirePreviousCodes` 공통화는 제외했다**(U3·D6이 예고한 경우). 세 서비스가 공유하는 코드가 `for (x : rows) x.expire(now)` 뿐이고 조회 조건 3개는 의도적으로 다르며, `expireAll`에는 정책 판정이 없어 `VerificationCodePolicy`에 넣으면 책임이 흐려진다(`conventions.md` 리뷰 질문). 각 서비스의 재발송 무효화는 기존 단위 테스트로 계속 보호된다
- [x] 전체 회귀·`./gradlew build`·문서 동기화 — 공통화 이후 동시성 테스트 재실행, `run-log.md`에 두 경로의 수정 전 실패 실측값·수정 후 통과·SQL 확인 결과 기록, PRD `AUTH-004`(249·251·255행)·`AUTH-005`(278행)·`AUTH-006`(297-298행) 정책 서술과 상수값 대조(어긋나면 고치지 말고 보고), `ai/specs/002-auth-account/tasks.md` 체크. **컨트롤러 변경이 없으므로 `ai/api-routes.md`·`docs/api-contracts.md`는 건드리지 않는다**

Issue #121은 **동작을 바꾸지 않는 수정**이다 — 응답 코드·판정 순서·오류 메시지가 하나라도 달라지면 실패로 본다. 범위는 PR #120 코멘트로 축소 확정됐다. ① `PasswordResetService`는 PR #120 `385144c`에서 이미 `@Lock`으로 해결됐고 동시성 테스트 4건이 함께 머지돼 이번 범위가 아니다(공통화 대상으로만 참여). ② 방식은 `@Lock(LockModeType.PESSIMISTIC_WRITE)`이며 원자적 `@Modifying UPDATE`는 기각됐다 — 한도 판정이 증가 전 값 기준이고 성공 경로만 증가하지 않는 비대칭 때문에 UPDATE 한 방으로 옮기면 분기별 증가 여부와 판정 순서를 다시 설계해야 한다. ③ **순서는 원자성 먼저, 공통화 나중이다** — 공통화를 먼저 하면 "수정 전 코드에서 실패"라는 수용 기준이 성립하지 않고(증명 대상이 리팩터링된 코드로 바뀐다), `@Lock` 방식에서는 원자성이 repository 2개·공통화가 service 3개로 파일이 겹치지 않아 "같은 파일을 두 번 헤집는" 비용도 실재하지 않는다. 미확정 7건(이슈 본문의 조건부 UPDATE 지시와 코멘트의 불일치, 시크릿이 실제로는 2분할인 점, `expirePreviousCodes`·`checkSendRateLimit` 공통화의 실익, 잠금 대기 타임아웃, 쿼리 필터 차이, HMAC 예외 메시지 통일)은 `issue-121-plan.md` U1~U7 참고.
