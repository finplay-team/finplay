...
- [x] `docs/prd.md` `AUTH-006` 구현 단계 문구·확인 단계 계약·엔드포인트 목록 갱신과 `docs/api-routes.md`·`docs/api-contracts.md`(다른 기기 Access Token 잔존 한계 명시 포함)·`docs/specs/002-auth-account/tasks.md` 동기화
...
- [x] 전체 회귀·`./gradlew build`·문서 동기화 — 공통화 이후 동시성 테스트 재실행, `run-log.md`에 두 경로의 수정 전 실패 실측값·수정 후 통과·SQL 확인 결과 기록, PRD `AUTH-004`(249·251·255행)·`AUTH-005`(278행)·`AUTH-006`(297-298행) 정책 서술과 상수값 대조(어긋나면 고치지 말고 보고), `docs/specs/002-auth-account/tasks.md` 체크. **컨트롤러 변경이 없으므로 `docs/api-routes.md`·`docs/api-contracts.md`는 건드리지 않는다**
