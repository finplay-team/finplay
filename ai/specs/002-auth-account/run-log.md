...
- Issue #121 Task 3: 정책 상수 5개·`generateCode`·`hmac`의 3중 복제를 `VerificationCodePolicy`(@Component)·`VerificationCodeHasher`(각 서비스가 자기 `@Value` 시크릿으로 직접 생성)로 걷어냈고, 세 서비스의 `@Value` 파라미터를 그대로 둬 `EMAIL_VERIFICATION_SECRET`(가입·이메일 변경)/`PASSWORD_RESET_SECRET`(재설정) 분배를 보존했다 — 교차 시크릿 거부를 고정하는 가드 테스트 5건 추가, 기존 기대값 수정 없이 229건 통과.
- Issue #121 Task 2: `EmailChangeVerificationRepository.findFirstByUserIdAndNewEmailOrderByCreatedAtDesc`에 `@Lock(PESSIMISTIC_WRITE)`을 넣어 동시 오답 5·6·8건 시나리오 3건이 red → green이 됐고, 같은 커밋에서 이 잠금 쿼리를 트랜잭션 밖에서 부르던 기존 테스트 6곳을 jdbcTemplate + `findById` 헬퍼로 돌려 기존 11건도 그대로 통과한다.
- Issue #121 Task 1: `EmailVerificationRepository.findFirstByEmailOrderByCreatedAtDesc`에 `@Lock(PESSIMISTIC_WRITE)` 한 줄만 추가해 동시 오답 5·6·8건 시나리오 3건이 red → green으로 뒤집혔고, 서비스 본문 무수정이라 응답 계약은 그대로다.
- Issue #115 재리뷰 판정: 차단 0건, 권장 2건, 참고 5건. 직전 차단(도달 불가 409)이 `User.hasPassword()`로 실제 해소됐고 상수 이동은 `AuthService` 동작을 바꾸지 않음(값 동일·참조 위치만 이동, 판별 회귀는 단위 3건+실제 `oauthLogin` 통합 테스트로 고정)을 확인, 머지 가능.
