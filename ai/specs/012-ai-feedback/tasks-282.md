...
- [x] **4. 문서 정리 — `PostSellFeedbackReader` Javadoc·spec·prd 정합 확인**
  ...
  - `ai/prd.md` §3은 **대상이 아니다.** FEED-007·010·011은 이미 완료로 기록돼 있고 코인 지원도 #275가 이미 반영했다. 이 이슈는 응답 계약·제공 기능을 늘리거나 줄이지 않고 내부 트랜잭션 경계만 재배치하므로 CLAUDE.md 규칙 10의 "제공하는 기능이 그대로인 변경"에 해당한다(`tasks-244.md` 5번 항목과 같은 판단).
  - `ai/api-routes.md`·`docs/api-contracts.md`도 **대상이 아니다** — 새 엔드포인트나 응답 계약 변경이 없다.
