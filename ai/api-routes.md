...
| PUT | /api/education/practice/attempts/{market}/exit-preset | education | 현재 실행 세대의 손절·익절 프리셋(`CAUTIOUS`\|`BALANCED`\|`RELAXED`) 선택. **보유 중이 아닐 때만** 허용하며 응답은 `PracticeAttemptResponse`. **(이슈 #502) 응답의 튜토리얼 잔액 3필드가 실값이다** | 042 EXITPRESET-003, Issue #477; Issue #502 |
