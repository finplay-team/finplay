...
| POST | /api/dev/stock-daily-imports | market | **(local 프로필 전용)** 평일 08:25 일봉 아카이브 배치를 즉시 한 번 실행해 종목별 "있어야 할 구간(3년 전~직전 영업일) − 이미 저장된 구간"만 KIS에서 채운다(`stock_daily_candles`, `stock_candles`와는 별도 테이블). 그 시각을 기다리지 않거나 앱이 꺼져 있어 건너뛴 날의 보정용. 요청 본문 없음, 최초 호출은 종목별 최대 3년치 순차 조회라 수 분 이상 걸릴 수 있음. `local` 프로필이 아니면 컨트롤러 빈 자체가 없어 404 | 050 MKT-011 (개발 도구, 이슈 없음) |
...
| POST | /api/exit-plans | order | 코인 holding에 대한 **일반 경로**(`intentionId` 생략) 손절·익절 OCO 예약 생성 (201). `holdingId`·`quantity`·`exitPriceType` 필수, `Idempotency-Key` 헤더 필수. `intentionId`를 지정하는 교육 경로는 아직 지원하지 않음(400). **(052 EXITFREE-020) 튜토리얼 예약은 이 경로를 쓰지 않는다** — 039 이후 튜토리얼에는 intention이 없어 attempt 전용 경로(`POST /api/education/practice/attempts/{market}/exit-plan`)로 갈랐다 | 021 일반 리스크관리 OCO, Issue #348; 052 EXITFREE-020 |
| DELETE | /api/exit-plans/{exitPlanId} | order | 인증 사용자 본인 소유의 `PENDING` OCO 예약을 취소하고 예약 수량을 반환 (204, 본문 없음). 경로(일반/교육) 공통. **(052 EXITFREE-020) 튜토리얼 예약 중 사용자가 직접 건 것(`POST .../attempts/{market}/exit-plan`)은 여기서 취소된다** — 042 자동 예약(대본을 쓰지 않는 실행)만 409 `EXIT_PLAN_TUTORIAL_INSTRUMENT_NOT_ALLOWED`로 남는다 | 021 일반 리스크관리 OCO, Issue #348; 042 EXITPRESET-016, Issue #477; 052 EXITFREE-020 |
...
| PUT | /api/education/practice/attempts/{market} | education | attempt 잠금(없으면 생성)→튜토리얼 계좌→progress 잠금으로 legacy 완료와 직렬화. **(이슈 #491) 같은 사용자·시장 동시 진입 2건이 모두 2xx로 끝난다** — 예전에는 무조건 `INSERT IGNORE`를 먼저 쏘고 같은 행을 `FOR UPDATE`로 잠가 S→X 승격 교착으로 한쪽이 500이었다. `READ COMMITTED`로 좁혀 선언(첫 진입 갭 락 교착 회피)하고, 잔여 교착은 진입 경로에서 1회 재시도. 새 빈 행만 원장 쓰기 없이 결정적 완료 `REPLAY`로 lazy 전환하고 기존 완료는 무변경, 기존 미완료+completion 충돌은 정합성 오류 | 039 TUTORIAL-FLOW-001·002·005, Issue #378 |
| POST | /api/education/practice/attempts/{market}/restart | education | 미완료 현재 실행의 pending 예약과 순보유를 원자 정리한 뒤 run을 증가시키고 종목 선택 상태로 초기화. 완료 attempt는 무변경 `REPLAY`. **(이슈 #491) 교착 시 1회 재시도** | 039 TUTORIAL-FLOW-003·004·005, Issue #378 |
| POST | /api/education/practice/attempts/{market}/advance-script | education | 2단계(주문 방법) 대본을 마친 실행을 **같은 run 안에서** 3단계(041 이야기) 대본으로 전환. 본문 없음. attempt 잠금 → COMPLETED·대본 미사용·이미 3단계·시장가·지정가 왕복 미완료·보유 중이면 각각 409, 통과하면 현재 run의 PENDING 지정가·예약을 정리하고 대본 식별자만 교체(커서 다섯 컬럼 초기화). run·튜토리얼 계좌·`exitPreset`은 유지 | 049 ORDERBASICS-018~021, Issue #507 |
| GET | /api/education/practice/attempts/{market}/chart | education | attempt seed·version·anchor로 재현한 29+1 일봉을 side effect 없이 순수 조회. ACTIVE는 3초마다 가상 1분 진행하고 완료 `REPLAY`는 `completedAt`에 고정되어 이후 변하지 않음. **대본 실행은 막(`scenarioStage`)·진행 여부·원인 상태·공개된 사건·안내 가격 범위(`priceGuideRange`)를 함께 반환** | 039 TUTORIAL-FLOW-005·009·010·011·012, Issue #378; 041 SCENARIO-015·016, Issue #488; 049 ORDERBASICS-009~011, Issue #507 |
| POST | /api/education/practice/attempts/{market}/tick | education | 현재 attempt를 먼저 잠그고 현재 canonical 가격으로 같은 run의 지정가 주문 조건을 판정·체결한 뒤 같은 관측 시각의 29+1 차트 반환. **생성기 버전 2(대본)는 대본 커서를 전진시키며 건너뛴 가상 분마다 순차 판정한다.** 응답의 사건 노출 4필드와 `priceGuideRange`는 GET chart와 같다. **(이슈 #491) 교착 시 1회 재시도** | 039 TUTORIAL-FLOW-010·011·012, Issue #378; 041 SCENARIO-013·015·016, Issue #472·#488; 049 ORDERBASICS-009~011, Issue #507 |
| PUT | /api/education/practice/attempts/{market}/exit-preset | education | **(052) 자유 입력 `exit-rates`로 대체됐다 — 프론트 전환이 끝나면 지운다. 지금은 그대로 동작한다**(내부적으로 `exit-rates`와 같은 서비스 메서드에 위임). 현재 실행 세대의 손절·익절 프리셋(`CAUTIOUS`\|`BALANCED`\|`RELAXED`) 선택. **보유 중이 아닐 때만** 허용하며 응답은 `PracticeAttemptResponse`. **(이슈 #502) 응답의 튜토리얼 잔액 3필드가 실값이다** | 042 EXITPRESET-003, Issue #477; Issue #502 |
| PUT | /api/education/practice/attempts/{market}/exit-rates | education | **(052) 손절 비율·익절 비율을 서로 독립적으로 자유 입력.** body `{"stopLossRate":3.0,"takeProfitRate":5.0}` — 퍼센트 수(3%는 `3`), 손절도 양수, 손절 `[2,5]`·익절 `[3,8]` 양 끝 포함, 소수 첫째 자리까지. 범위·자릿수 위반은 400 `VALIDATION_ERROR`. **프리셋 3개 조합만 허용하는 검증은 없다** — 손절 5 + 익절 3도 유효하다. 거부 판정(보유 중 409 `PRACTICE_STEP_LOCKED`, 2단계 미완료 409 `PRACTICE_STAGE_LOCKED`, 완료 409 `PRACTICE_ALREADY_COMPLETED`)과 응답 `PracticeAttemptResponse`는 `exit-preset`과 **완전히 같다**(같은 서비스 메서드에 위임). **(052 EXITFREE-020) 대본을 쓰는 실행에서는 매수 체결 시 자동 예약이 더 이상 생기지 않는다** — 여기서 정한 비율은 기준선(참조선) 계산에만 쓰이고, 실제 예약은 사용자가 아래 `POST .../exit-plan`으로 직접 건다. 대본을 쓰지 않는 실행(생성기 버전 1·legacy)만 042 그대로 자동 예약이 생긴다 | 052 EXITRATES-001~003, EXITFREE-020 |
| POST | /api/education/practice/attempts/{market}/exit-plan | education | **(052 EXITFREE-020) 사용자가 직접 거는 손절·익절 OCO 예약.** body는 `exit-rates`와 **같다** (`{"stopLossRate":3.0,"takeProfitRate":5.0}`, 같은 구간·자릿수 검증). 현재 실행 세대의 **보유 전량**을 그 진입의 체결가(`practice_risk_snapshots.entry_price`) 기준으로 예약하고 201 `ExitPlanResponse`를 반환. **진입당 정확히 1회(write-once)** — 취소한 뒤 같은 진입에서 다시 만드는 것도 409 `EXIT_PLAN_ALREADY_EXISTS`. 보유 없음·attempt 없음·진행 중 아님은 409 `PRACTICE_STEP_LOCKED`, 대본 없는 실행(자동 예약 경로)도 같은 코드, 2단계 대본 실행은 409 `PRACTICE_STAGE_LOCKED`, 완료된 attempt는 409 `PRACTICE_ALREADY_COMPLETED`. **취소는 새 경로를 만들지 않고 `DELETE /api/exit-plans/{id}`가 그대로 받는다** | 052 EXITFREE-020 |
| PUT | /api/education/practice/attempts/{market}/instrument | education | 현재 attempt에 같은 시장의 거래 가능한 튜토리얼 샘플 종목을 선택하고 실행 anchor·date·seed·generator version을 영속. **(이슈 #502) 응답의 튜토리얼 잔액 3필드가 실값이다** — 예전에는 항상 `0`이라 화면이 "보유 현금 0원"을 그렸다 | 039 TUTORIAL-FLOW-006, Issue #378; Issue #502 |
...
| GET | /api/education/practice?market= | education | 영속 `PracticeAttempt`가 있으면 현재 run의 risk snapshot·귀속 BUY/SELL·관찰·reflection으로 4단계 진행 또는 불변 `REPLAY`를 순수 조회하며 stale run과 favorite·intention을 배제. attempt가 없는 기존 샘플·실제 종목은 026 chain과 OCO 분리를 호환 유지. **attempt 경로는 진입별 대조 배열(`entries`)·공개된 사건(`revealedEvents`)·"안 팔았다면" 기준가(`priceAfterSell`)를 함께 반환.** **(049 ORDERBASICS-023, Issue #512 "5-A") `entries[]`의 각 항목은 그 진입이 열릴 때 attempt가 쓰던 대본 식별자(`scenarioScriptId`)를 진입별로 고정해 담는다** — 2→3단계 전환이 같은 run 안에서 대본만 갈아끼우므로, 전환 전후로 연 진입이 한 배열에 섞여도 화면이 대본별로 끊어 그릴 수 있다. **(이슈 #503) 튜토리얼 5단계 진행 판정(`tutorialStageProgress`)도 함께 반환** — 시장가·지정가 왕복은 현재 run의 체결 원장으로, 프리셋 단계는 그 실행의 프리셋 선택 여부(`exitPresetSelected`)로 판정한다. 판정만 하고 순서를 강제하지는 않는다. **(052) `exitPresetSelected`의 판정이 두 번 넓어졌다** — 프리셋이든 자유 비율이든 그 실행에서 한 번이라도 정했으면 `true`이고, **(EXITFREE-020) `POST .../exit-plan`으로 예약을 만든 것도 근거로 센다**(취소·체결된 예약도 남는다). 대본 없는 실행의 자동 예약은 세지 않는다. 필드 이름은 그대로다. 응답 루트에 `exitRateBounds`(자유 입력 허용 구간)가 더해졌고, `attempt` 객체에는 `exitStopLossRate`·`exitTakeProfitRate`(항상 non-null, 미선택이면 기본값 3·5)와 같은 `exitRateBounds`가 더해졌다. **(052 EXITFREE-020·021·022) 예약 세 값이 함께 온다** — `exitPlanCreatable`(boolean), `pendingExitPlan`(없으면 `null`, 있으면 `exitPlanId`·비율 2·가격선 2·`entryPrice`·`quantity`·`reservedAt`), `exitExperience`(`stopLossExperienced`·`takeProfitExperienced`·`bothExperienced`·`recommendedNext`). 셋 다 현재 실행 세대 기준이라 재시작하면 초기화된다 | 026 MKT-PRACTICE-008, Issue #305; 031 SANDBOX-005·007, Issue #339; 039 TUTORIAL-FLOW-001·005·007·008·012, Issue #378; 041 SCENARIO-019b·020·021, Issue #488; Issue #503 |
