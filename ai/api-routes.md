...
| PUT | /api/education/practice/attempts/{market} | education | insert(`ON DUPLICATE KEY UPDATE`)→attempt→progress 잠금으로 legacy 완료와 직렬화. **(이슈 #491) 같은 사용자·시장 동시 진입 2건이 모두 2xx로 끝난다** — 예전 `INSERT IGNORE`는 유니크 인덱스 레코드에 S를 남긴 뒤 같은 레코드에 X를 요구해 잠금 승격 교착으로 한쪽이 500이었다. 잔여 교착은 진입 경로에서 1회 재시도. 새 빈 행만 원장 쓰기 없이 결정적 완료 `REPLAY`로 lazy 전환하고 기존 완료는 무변경, 기존 미완료+completion 충돌은 정합성 오류 | 039 TUTORIAL-FLOW-001·002·005, Issue #378 |
