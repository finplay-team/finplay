...
| GET | /api/education/practice?market= | education | 영속 `PracticeAttempt`가 있으면 현재 run의 risk snapshot·귀속 BUY/SELL·관찰·reflection으로 4단계 진행 또는 불변 `REPLAY`를 순수 조회하며 stale run과 favorite·intention을 배제. attempt가 없는 기존 샘플·실제 종목은 026 chain과 OCO 분리를 호환 유지. **attempt 경로는 진입별 대조 배열(`entries`)·공개된 사건(`revealedEvents`)·"안 팔았다면" 기준가(`priceAfterSell`)를 함께 반환.** **(이슈 #503) 튜토리얼 5단계 진행 판정(`tutorialStageProgress`)도 함께 반환** — 시장가 왕복·지정가 왕복·프리셋 적용을 현재 run의 체결 원장으로 서버가 판정한다. 판정만 하고 순서를 강제하지는 않는다 | 026 MKT-PRACTICE-008, Issue #305; 031 SANDBOX-005·007, Issue #339; 039 TUTORIAL-FLOW-001·005·007·008·012, Issue #378; 041 SCENARIO-019b·020·021, Issue #488; Issue #503 |
