...
| 주식 캔들 데이터 (수집·보관·조회) | **먼저 어느 데이터인지 판별한다 — 주식 캔들은 성격이 다른 둘이다.** ① **1분봉 `stock_candles`(재생용)** — 과거 거래일을 오늘 09:00~15:30에 재생하는 원본. 수집·보관은 PRD MKT-005 + `ai/specs/003-market-data`, 재생·공개 상한은 MKT-002 + `StockReplayService`. **보관 기간 20영업일은 이 데이터에만 적용된다.** ② **일봉 `stock_daily_candles`(장기 차트용)** — 실제 달력 날짜의 3년치 아카이브. PRD MKT-011 + `ai/specs/050-stock-daily-archive`가 정본이며 **아직 미구현**이다. 그 위의 조회 계약은 `013-candle-interval`(일·주·월봉 집계 규칙)과 `048-candle-history-pagination`(커서 페이지네이션)이 소유한다. **일봉 조회는 현재 ①을 집계해 응답하며 ②를 아직 읽지 않는다** — 그 전환은 재생 모델의 공개 상한과 충돌해 이슈 #506·#495의 결정이 선행된다 |
