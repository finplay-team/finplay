...
- `ai/adr/0002-architecture.md` — 패키지 구조, 도메인 간 참조 규칙
- `ai/adr/0003-testing-strategy.md` — 요구 테스트 레벨
- `ai/adr/0004-flyway-migrations.md` — 스키마 변경 규칙
...
2. 다음을 점검한다.
   ...
   - 문서 동기화: controller 추가/변경에 `ai/api-routes.md`(라우트 목록) 또는 `docs/api/`의 해당 도메인 파일(계약 상세) 갱신 누락
   - 문서 동기화: 요구사항 ID를 완료로 만드는 변경(새 엔드포인트 등)에 `ai/prd.md` §3 "구현 현황" 행 갱신 누락 (CLAUDE.md 규칙 10). 기능 제공 범위가 그대로인 리팩터링·테스트·문서 변경은 대상이 아니다 — 갱신하지 않았다고 지적하지 않는다
...
6. **직전 리뷰 지적을 고친 PR이면 그 수정 자체를 검증 대상으로 본다.** 지적을 반영하는 과정에서 새 결함이 생기는 일이 이 저장소에서 반복됐다(`ai/agent-mistakes.md` 2026-08-03). 고친 자리뿐 아니라 그 옆·같은 응답의 다른 필드·같은 규칙을 공유하는 다른 시장(주식/코인)까지 본다.
7. 대상 spec 폴더가 지시됐으면 `run-log.md`에 기록한다 (없으면 생략 — 여러 spec에 걸친 PR 리뷰처럼 폴더가 특정되지 않을 때). 형식은 `ai/specs/README.md` 참조. AI 로그 표에 한 행(점검 범위 명령 + 점검한 문서), 모니터링 섹션에 판정 요약 한 줄.
