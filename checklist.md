...
- [x] `docs/specs/007-journal/spec.md` 확장 — 목표가·손절가 필수 필드 승격 (계획 대조의 입력)
...
### 착수 전 남은 일
- [ ] **팀 합의** — `docs/notion-sync-012.md`의 변경안을 Notion에 반영. 뉴스 미사용 원칙이 **세 곳**(운영 정책, api 명세서 §12, 레포 prd.md)에 있었고 레포만 개정된 상태다
- [ ] **팀 논의** — 투자일기 경로(`buy-journal` vs `journal`). 매수는 남동엽, 매도는 정욱재, 상세는 정욱재, 목록은 정예진이라 네 사람 범위에 걸쳐 있다
- [ ] 엔드포인트 DB에 `instruments/{id}/price-moves` 행 추가 + `ai/post-sell/{id}`·`trades/{id}/buy-journal` 담당자 지정
...
- [ ] GitHub 이슈 생성 — 아직 만들지 않았다. 번호를 미리 지어내지 않는다 (2026-07-31 교훈)
