...
// 다섯 키가 그 경로로 실제 존재하는지까지 단정해 "튜닝은 application.yml 수정"(§튜닝)이 성립함을 보장한다.
...
class FeedbackLlmPropertiesIntegrationTest {
	...
	@Test
	@DisplayName("application.yml에 feedback.llm 다섯 키가 §C-7 값으로 실제 존재한다")
	void applicationYmlDeclaresEveryFeedbackLlmKey() {
		...
		assertThat(environment.getProperty("feedback.llm.max-tokens")).isEqualTo("512");
