 ...
 *
 * <p><b>필요 시 온디맨드 수집을 트리거한다</b>(ADR-0016 §결정 1). 근거 매칭이 비어 있으면
 ...
 * {@code market_news_items}에 쓴다(ADR-0016).
 */
@Slf4j
@Service
...
public class CryptoPriceMoveWatcher {
	...
	// 온디맨드 수집(ADR-0016) 후 1회 재매칭, 그래도 0건이면 종료 → 서술 → 저장. 락은 z-score 게이트 통과
	...
	private boolean watchOne(Instrument instrument, LocalDateTime now) {
		...
		try {
			...
			// 첫 매칭이 비면 온디맨드 수집(ADR-0016) 후 1회만 재매칭한다 — 수집 직후에도 근거가 하나도 없으면
