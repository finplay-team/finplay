// feedback.detection.* 프로퍼티(FeedbackDetectionProperties)를 빈으로 등록하는 feedback 도메인 설정 클래스.
package com.finplay.api.feedback.config;

import org.springframework.boot.context.properties.EnableConfigurationProperties;
import org.springframework.context.annotation.Configuration;

// record 기반 @ConfigurationProperties는 명시적으로 등록해야 빈이 된다 (FeedbackLlmConfig와 같은 방식).
// @Bean 메서드가 없어 self-invocation이 없으므로 CGLIB 프록시(proxyBeanMethods)가 필요 없다.
//
// NewsCollectionPropertiesConfig에 얹지 않고 프리픽스마다 설정 클래스를 나눈 것은 FeedbackLlmConfig 주석과
