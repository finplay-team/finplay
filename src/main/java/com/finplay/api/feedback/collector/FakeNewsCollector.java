...
/**
 * {@code @Profile("!prod")}로 로컬·테스트에서만 등록된다 ({@code FakeEmailSender} 선례). 이 빈이 있어서
 * <b>네이버 검색 키가 없어도 기동과 {@code ./gradlew build}가 통과한다</b> (ADR-0011, spec §실패 처리의
 * "네이버·DART 키 없음 → {@code Fake*Collector}가 빈 목록 반환. 기동·테스트 정상").
 *
