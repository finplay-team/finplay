// 시장가 매수·매도 주문의 검증·체결·계좌/보유 갱신을 하나의 트랜잭션으로 처리하는 서비스
...
public class OrderService {
	...
	public OrderResponse createOrder(Long userId, String idempotencyKey, OrderCreateRequest request) {
		...
		return request.side() == OrderSide.SELL
			? createSellOrder(userId, idempotencyKey, request, instrument, account)
			: createBuyOrder(userId, idempotencyKey, request, instrument, account);
	}

	private OrderResponse createBuyOrder(
		Long userId, String idempotencyKey, OrderCreateRequest request, Instrument instrument, Account account) {
