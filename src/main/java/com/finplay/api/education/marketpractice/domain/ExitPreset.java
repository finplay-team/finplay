 ...
 * {@code ×0.97}·{@code ×1.05}와 정확히 같아야 하고(EXITPRESET-002), 나머지 둘은 041 대본이 허용하는 구간의
 * 양 끝이다 — 특히 손절률 1%p 간격은 2막 루머 저점 {@code 0.975}가 {@code CAUTIOUS}와 {@code BALANCED}의
 * 손절선 구간 사이에 들어가게 하는 값이라 임의 조정 대상이 아니다(041 plan §프리셋 도달 조건 검증).
