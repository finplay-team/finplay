...
public class CandleQueryService {
	...
	public List<CandleResponse> getCandles(Long instrumentId, String interval, LocalDateTime from, LocalDateTime to) {
		...
		// 코인(MKT-008, 이슈 #20): sourceTradingDate 개념이 없는 실제 달력 시각의 실시간 분봉이므로 from>to 판정도
		// 날짜를 포함한 전체 시각으로 비교한다 — 아래 주식과 달리 날짜 성분을 버리지 않는다.
		if (instrument.getMarket() == Market.CRYPTO) {
			if (from != null && to != null && from.isAfter(to)) {
				throw new BusinessException(ErrorCode.VALIDATION_ERROR, "from은 to보다 늦을 수 없습니다.");
			}
			return cryptoCandleProvider.getCandles(instrument.getSymbol(), from, to).stream()
				.map(CandleResponse::from)
				.toList();
		}

		// 리뷰 확정(PR #87 QA FAIL, 옵션 c): 주식 캔들은 항상 재생 중인 단일 거래일(source_trading_date) 안에서만 조회되므로
