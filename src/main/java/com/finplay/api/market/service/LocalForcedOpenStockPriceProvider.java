...
//
// 시장상태를 읽는 모든 소비자(PriceQueryService.assertOrderable, SSE의 snapshot·price·status 이벤트)가 같은
// getMarketStatus()를 쓰므로, 화면에 OPEN으로 보이는 동안 주문도 실제로 통과한다 — 프론트에 거짓 상태를 보여주지 않는다.
@Service
@Primary
@Profile("local")
