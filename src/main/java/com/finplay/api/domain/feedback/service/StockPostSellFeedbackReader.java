 ...
 * {@link PostSellFeedbackContextReader}가 마쳤고, 시장 분기는 {@link PostSellFeedbackReader}가 한다. 이 클래스는
 * {@link CryptoPostSellFeedbackReader#read}와 <b>대칭 시그니처</b>로 주식 조립만 맡는다.
 *
 * <p><b>동작은 이슈 #282에서 바뀌지 않았다</b> — 이 클래스는 {@code PostSellFeedbackReader}의 주식 분기 코드를
 * 값·순서 하나 바꾸지 않고 옮긴 것이고, 여전히 <b>단일 {@code @Transactional(readOnly = true)}</b>다(spec §FEED-012
 * 결정 5). 그 이슈가 트랜잭션 밖으로 끌어낸 것은 코인 경로의 REST 호출뿐이다.
 *
...
class StockPostSellFeedbackReader {
	...
	/**
	 * 주식 매도 체결 1건의 회고에서 <b>서술을 뺀 전부</b>를 조립한다.
	 *
	 * @param trade 주식 매도 체결. 검증은 호출부가 이미 마쳤다
