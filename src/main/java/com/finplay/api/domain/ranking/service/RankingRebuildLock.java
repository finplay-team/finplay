 ...
 * 동안 같은 시장을 잠근다. ADR-0021 §5(전환은 파이프라인이 상태를 읽어서 정한다)·§6(헬스체크 실패 시 이전
 * 색으로 되돌린다)이 함께 확정한 블루-그린 배포는 전환 이후에도 이전 색 인스턴스를 다음 배포까지 내리지
 * 않으므로, 두 인스턴스의 스케줄러가 같은 시각에 같은 시장을 재구성하면 한쪽의 DEL이 다른
