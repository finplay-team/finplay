...
| AI 피드백 — 코인 변동 감시 | FEED-005·006 코인 분기 | **완료** | `012` plan의 이슈 8 (Issue #225). `CryptoPriceSnapshotService` 스냅샷 기록·`CryptoPriceMoveWatcher` 탐지·카드 확정, `GET /api/instruments/{id}/price-moves` 코인 분기(게이트 없음, 최근 24시간, `originTradeDate=null`) (#225) |
