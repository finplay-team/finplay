...
|---|---|---|
| 투자일기 | 주문 전에 목표가·손절가 필수 | 매수 체결별 작성만 1차 MVP, 수정·목록·상세·매도 회고·AI 복기는 후속 Spec |
...
- 주식·코인 계좌를 합산한 전체 포트폴리오와 매수 체결별 투자일기를 확인할 수 있다.
...
- When 닉네임을 변경하거나 새 이메일 인증을 완료
- Then 본인 정보만 변경되고 기존 계좌·시드머니·잔액·주문·체결·투자일기는 그대로 유지된다.
...
- 인증 사용자는 본인 소유의 매수 체결 건에 투자일기를 한 번 작성할 수 있다.
- `buyTradeId`가 실제 매수 체결인지, 요청자 소유인지 검증한다.
- 매수가 아닌 체결, 없는 체결, 타인 체결, 이미 투자일기가 작성된 체결은 데이터 변경 없이 거부한다.
- 투자일기 본문과 작성시각을 저장하며 주문·체결 원장은 변경하지 않는다.
- 투자일기 수정·목록·상세, 매도 회고 구 API, AI 복기는 1차 범위에서 제외한다.
...
- `trade_journals`: 회원, 매수 체결, 본문, 생성·수정시각
...
- `UNIQUE(trade_journals.buy_trade_id)`
...
- `market`: 종목·주식 시세 공급자(`StockPriceProvider` — KRX 재생·KIS 실시간)·빗썸·Redis 시세
- `order`: 시장가 검증·체결·멱등성·수수료
- `portfolio`: 보유·FIFO lot·거래내역·평가손익·합산 요약·투자일기
...
- 투자일기 작성: 매수 체결·소유권·중복 검증+투자일기 저장 원자 처리
...
6. 조회
   ...
   - QueryDSL 목록 조회
7. 매수 투자일기
   - 본인 매수 체결별 투자일기 1회 작성
   - 수정·목록·상세·매도 회고·AI 복기는 후속 Spec으로 분리
8. 커뮤니티
   - 게시물 CRUD·페이지네이션
   - 평면 댓글 작성·조회·본인 삭제
9. 통합 검증
...
- 매수 투자일기의 체결 유형·소유권·중복 작성·주문과 체결 원장 불변 테스트
- 게시물·댓글 타인 변경 거부 테스트
...
5. 주식 2회 매수 후 일부 매도하고 매수 체결에 투자일기를 작성한다.
...
11. 다른 계정으로 내 정보·주문·체결·투자일기·콘텐츠를 조회하거나 변경할 수 없는지 확인한다.
