...
| 주문 유형 | 시장가·지정가 | 시장가만 |
| 지정가 체결 | 이벤트 드리븐 상시 처리 | 2차에서 지정가+상시 처리(이벤트 드리븐, 코인 내부 가격 모델 갱신(빗썸 웹소켓 수신) 시 인메모리 `ApplicationEvent` + `@TransactionalEventListener(AFTER_COMMIT)`으로 적재 — 랭킹과 동일 패턴) — 배치 체결에서 재변경 (2026-08-03, LMT-002). **2차 지정가는 코인을 우선으로 시작하고 주식은 추후 처리한다**(2026-08-05, LMT-001~004) |
...
| 지정가 주문·상시 체결 | LMT-001~004 | **미착수** | `OrderType` enum은 `MARKET`만. spec 폴더 미생성. 2차 범위는 코인 우선, 주식은 추후 처리(2026-08-05 확정) |
...
- 지정가 주문과 상시 체결 (LMT-001~004, 2026-08-03 이벤트 드리븐 상시 처리로 재변경 — 배치 아님) — **미착수**. **2차 범위는 코인을 우선으로 시작하고 주식은 추후 처리한다**(2026-08-05 확정 — 주식은 재생 데이터 기반이라 "이 가격 도달 시" 조건 자체가 성립하기 어렵고, 분봉 판정 해상도·재생세션 자동취소·OCO와의 잠금 순서 공유 같은 부가 복잡도가 있다). 동시 체결 경합과 취소(LMT-003)·체결 트리거 동시 도착 경합은 비관적 락(SELECT FOR UPDATE, OCO와 동일한 lock 순서 원칙을 holding → order로 확장)으로 제어하기로 확정했다(2026-08-05)
...
> 지정가(LIMIT) 주문은 2026-07-23 결정으로 1차 MVP 범위에서 제외됐고(시장가만), 2차(1차 고도화)에서 다룬다. 체결 방식은 2026-08-03 재확인 결과 배치가 아니라 상시 처리(이벤트 드리븐)로 확정했다. **2차 지정가는 코인을 우선으로 시작하고 주식은 추후 처리한다(2026-08-05 확정)** — 주식은 재생 데이터 기반이라 "이 가격에 도달하면"이라는 지정가 조건 자체가 잘 맞지 않고, 분봉 판정 해상도(미마감 봉의 고가·저가 미노출)·재생세션 종료 시 자동 취소·OCO와의 잠금 순서 공유 같은 주식 전용 부가 복잡도가 있다. 아래 LMT-001~004는 코인 전용으로 서술하며, 주식 지정가를 다루게 되면 그 시점에 이 부가 복잡도를 포함해 별도로 정의한다.
...
- **동시성 제어(2026-08-05 확정)**: 동시 체결 경합(같은 주문에 대해 가격 갱신 이벤트가 겹쳐 도착하는 경우)과 취소(LMT-003) 요청·체결 트리거가 동시에 도착하는 경합은 **비관적 락(`SELECT ... FOR UPDATE`)으로 주문 행을 잠그는 방식으로 제어한다** — 주식 전용 OCO(§3단계 투자 실습)와 동일한 잠금 순서 원칙을 확장해 `holding → order` 순으로 잠근다. 먼저 락을 획득한 트랜잭션의 커밋이 확정된 뒤, 나중 트랜잭션은 갱신된 `status`를 보고 스스로 거부(이미 `FILLED`면 취소 거부)·no-op(이미 취소됐으면 체결 skip) 처리하므로 예약 이중 반환이나 체결 후 취소 같은 경합은 발생하지 않는다.
- **큐 적재·소비 방식(2026-08-05 확정)**: 별도 메시지 큐 없이 **인메모리 Spring `ApplicationEvent` + `@TransactionalEventListener(AFTER_COMMIT)`**으로 처리한다 — 랭킹(`RankingEventListener`)과 동일한 패턴이다. 가격 갱신으로 지정가 체결 조건이 충족되면 이벤트를 발행하고, 커밋 이후 리스너가 체결을 트리거한다. 후속 고도화에서 Redis/Kafka 등 외부 큐로 교체할 수 있도록 리스너 경계는 열어둔다.
