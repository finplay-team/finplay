...
| 랭킹 | 실시간 실현손익 | 1차 제외, 2차 — 시장별(STOCK/CRYPTO) 분리, Redis ZSET으로 순위 관리, REST 조회 (SSE push는 검토 후 제외 — 랭킹은 체결마다 push할 만큼 긴급하지 않고 Notion API 표에도 REST만 등재됨, 2026-08-03, RANK-001·RANK-002) |
...
- 실시간 실현손익 랭킹 (RANK-001~002, 시장별(STOCK/CRYPTO) 분리 집계, Redis ZSET으로 순위 관리, REST 조회 — SSE push는 검토 후 제외, 2026-08-03) (`docs/specs/014-ranking`)
- 알림
...
- `limit`은 생략 시 기본 10건을 반환한다. 요청값이 50을 초과해도 서버는 최대 50건까지만 반환한다 — 남용 방지를 위한 상한이다.
...
- 순위는 Redis ZSET(시장별 키)으로 관리하며, score는 `accounts` 테이블의 시장별 실현손익 컬럼(ACCT-002) 값을 그대로 쓴다 — 매도 체결로 `accounts.실현손익`이 갱신될 때(ORD-005 FIFO 배분 확정 시점) 같은 트랜잭션 흐름에서 해당 회원의 ZSET score도 즉시 갱신한다(이벤트 드리븐) — 지정가 체결 트리거(LMT-002)와 동일하게 배치가 아닌 상시 처리다. 매 조회마다 `trades`를 재집계하지 않는다.
- 실현손익의 정본은 MySQL `accounts.실현손익`이며 Redis ZSET은 조회 성능을 위한 파생 데이터다. Redis 유실 시 MySQL 원장으로 재구성한다 — 재구성 트리거·절차의 세부 구현은 착수 시 확정한다.

#### RANK-002 내 랭킹 조회 (2차 MVP)

- `GET /api/rankings/me?market=`로 인증 사용자 본인의 실현손익 순위를 조회한다.
- `market`은 RANK-001과 동일하게 필수다.
- 상위 `limit`건 안에 들지 않아도 정확한 순위를 반환한다 — Redis ZSET의 순위 조회는 순위표 내 노출 여부와 무관하게 계산되므로 "순위 없음"으로 대체하지 않는다.
- RANK-001과 동일하게 매도 체결 이력이 없는 사용자는 랭킹 대상이 아니다 — 이 경우의 응답 형태(순위 필드 생략·전용 상태값 등)는 착수 시 확정한다.
- 다른 사용자의 순위는 이 엔드포인트로 조회할 수 없다.

**Decision Gate**: 응답 필드(닉네임 노출 범위 등 개인정보 최소화), 전체 오류 코드, Redis 키 이름·자료구조 세부, 다중 인스턴스 환경에서의 score 갱신 경합(2차 범위인 동시성 모델과 연결), 공동 순위 계산의 구체적 구현(ZSET 기본 순위 결과를 애플리케이션에서 어떻게 보정할지)은 착수 시 `docs/specs/014-ranking`에서 확정한다.
