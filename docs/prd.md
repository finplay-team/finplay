...
- 투자 지식 튜토리얼 미션과 확정 교육 자료 기반 코치 설명
...
- 튜토리얼 과정·보상·교육 코치 정책은 `docs/specs/011-investment-education-policy`에서 확정한다. production 구현은 2차 MVP 완료와 3차 착수 승인 뒤에만 시작한다. AI 리포트 주기는 3차 시작 전 별도 Spec으로 확정한다. 종목 뉴스 요약은 2차로 이동했으므로 3차에서는 다루지 않는다. ~~뉴스 갱신주기~~는 2026-08-03에 확정했다(위 항목).
