...
| 투자 실습 — 코인 가상 가격 실행 환경 | COIN-PRICE-RUNTIME-001~012 | **일부 완료** | `030-coin-practice-price-runtime`, 이슈 #314·#318(PR). 세션 생성·조회 API와 결정적 version 1 생성기 구현 완료(재기동 재현·ACTIVE unique 검증 포함). next tick, 교육 지정가·holding 관찰 가격원 격리는 후속 이슈. 기존 합성 시세는 표시 전용 호환 유지하고 주식은 3차 MVP |
