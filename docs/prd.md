...
| 지정가 주문·상시 체결 | LMT-001~004 | **일부 완료(LMT-001~003)** | LMT-001(생성)·LMT-002(체결 트리거) 완료 — PR #215(`docs/specs/015-limit-order`, 코인 전용). LMT-003(취소) 완료 — 이슈 #218(PR 생성 후 번호 갱신 필요). LMT-004(미체결 목록조회)는 범위 밖, 후속 이슈. 주식 지정가는 추후 처리(2026-08-05 확정) |
