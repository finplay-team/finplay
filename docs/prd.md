...
### C-004 AI 정책 (2026-08-02 뉴스 사용 조항 개정)
...
- 뉴스 기사 본문은 저장하지 않는다. 제목·언론사·원문 URL·발행시각만 저장하고 본문은 AI 입력으로만 사용한 뒤 폐기한다 (C-006과 같은 취지).
...
- **뉴스 출처·저작권 결정을 3차에서 2차로 앞당겼다 (2026-08-02)**: 2차 "AI 피드백"이 뉴스를 근거로 쓰게 되면서 3차를 기다릴 수 없게 됐다. 출처는 네이버 뉴스 검색 API(분 단위 발행시각)와 OpenDART 공시검색 API(일 단위 접수일자)로 확정했고, 저작권 대응은 "본문 미저장, 제목·언론사·원문 URL·발행시각만 저장"이다 (C-004). **갱신주기는 미확정** — 3차 "종목 뉴스 요약"에서 정한다.
