...
- 입력: 이메일, 중복되지 않는 닉네임, 8자 이상 비밀번호, 약관 동의
- 비밀번호는 단방향 해시로 저장한다.
- 이메일 또는 닉네임 중복은 409로 거부한다.
- 성공 시 주식·코인 계좌를 같은 트랜잭션에서 생성한다.

수용 기준:

- Given 중복되지 않는 입력값
- When 회원가입 요청
- Then 회원·주식계좌·코인계좌가 한 번만 생성되고 JWT 토큰이 반환된다.
...
- 최초 로그인은 회원과 두 계좌를 생성한다.
- 제공자 이메일이 기존 이메일 회원과 동일하면 해당 회원에 소셜 계정을 연결한다.
...
Base URL: `/api` (버전 프리픽스 없음 — 2026-07-23 확정, `docs/conventions.md`)
...
- `GET /accounts`
- `GET /accounts/{market}`
- `GET /accounts/{market}/holdings`
- `GET /trades?market=&cursor=&limit=`
...
- `GET /instruments/{instrumentId}/candles?interval=1m&from=&to=`
- `GET /stocks/stream` (SSE, 주식 시세 — 시장별 스트림 분리)
- `GET /cryptos/stream` (SSE, 코인 시세)
...
- `POST /trades/{buyTradeId}/journal`
- `PATCH /trades/{buyTradeId}/journal`
- `GET /journal?market=&cursor=&limit=`
...
| 404 | NOT_FOUND | 대상 없음 |
| 409 | DUPLICATE_RESOURCE | 이메일·닉네임·소셜계정 중복 |
...
- `auth`: 회원·JWT·OAuth·Refresh Token
- `account`: 두 계좌 생성·잔고·요약
- `market`: 종목·주식 KRX 데이터 수집(파일 임포트)과 재생·업비트·Redis 시세
...
- **Decision Gate**: 다음 항목은 KRX 상품의 데이터 형식·timestamp 의미·제공시각이 확인되기 전까지 확정하지 않는다 — 정상 분봉 개수 판단 기준(임시 숫자 포함), 거래 없는 분을 상품이 어떻게 표현하는지, 분봉 timestamp가 구간 시작·종료 중 무엇을 의미하는지, `KrxFileImporter`의 production 파일 포맷 파서, 정확한 데이터 제공시각(수집 배치 실행시각 포함), API 상품 수집 여부. 재생기·주문 로직·수집 골격(전체 파일 오류 검증)은 샘플 데이터로 먼저 설계·테스트할 수 있다.
