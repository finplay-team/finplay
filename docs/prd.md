...
- `GET /api/orders`
  - 인증 사용자의 전체 주문을 최신 요청순으로 반환한다. MVP에서는 별도 페이지네이션을 두지 않는다.
...
- 3차 시작 전 튜토리얼 보상, AI 리포트 주기를 별도 Spec으로 확정한다. 종목 뉴스 요약은 2차로 이동했으므로 3차에서는 다루지 않는다. ~~뉴스 갱신주기~~는 2026-08-03에 확정했다(위 항목).
