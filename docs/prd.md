...
- Fixture는 자동 테스트와 시연에 필요한 16종 데이터를 포함한다.
...
- 최신 틱이 기준시간을 초과하거나 연결이 끊기면 해당 코인 주문을 거부한다.
...
- 유효한 최신 가격 한 개로 즉시 전량 체결한다.
- 부분 체결과 슬리피지는 없다.
- 지정가 요청은 `UNSUPPORTED_ORDER_TYPE`으로 거부한다.

#### ORD-002 주문 가능 시간

- 주식은 Fixture 재생 장중에만 주문 가능하다.
...
- `market`: 종목·주식 Fixture·업비트·Redis 시세
...
- 주식 Fixture는 외부 데이터 승인과 무관하게 저장소에 포함된 시연·테스트 데이터만 사용한다.
