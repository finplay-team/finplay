...
- 과거 1분봉은 한국투자증권(KIS) Open API 호출로 수집한다. 데이터 재생·주문 로직은 수집 방식이 바뀌어도 수정하지 않는다.
...
- 주식 시세 공급자를 공통 계약 `StockPriceProvider`로 분리하고 구현 2개를 둔다.
  ...
  - `KisHistoricalReplayPriceProvider` — KIS Open API로 조회한 과거 실제 1분봉 재생. **공개 배포의 기본 Provider**다.
...
- KIS WebSocket은 체결 틱 단위로 들어오므로, 차트(1분봉)에 쓰려면 **서버가 틱을 1분 OHLCV로 집계하는 책임**을 갖는다. 집계 결과는 `KisHistoricalReplayPriceProvider`가 제공하는 분봉과 같은 모델이어야 한다.
- 실행 환경은 다음 설정으로 결정한다.
...
- KIS 실시간 키(`KIS_APP_KEY`·`KIS_APP_SECRET`·`KIS_ACCOUNT_NO`·`KIS_ACCOUNT_PRODUCT_CODE`)가 없어도 `KIS_HISTORICAL` 환경에서는 애플리케이션과 자동 테스트가 정상 실행되어야 한다.
...
- KIS 실시간 키(`KIS_APP_KEY`·`KIS_APP_SECRET`·`KIS_ACCOUNT_NO`·`KIS_ACCOUNT_PRODUCT_CODE`)가 없어도 `STOCK_FEED_PROVIDER=KIS_HISTORICAL` 환경에서는 애플리케이션과 자동 테스트가 정상 기동해야 한다 (MKT-007).
...
- 주식 KIS Open API 데이터 수집·재생 개장·장중·마감 테스트
- 시세 공급자 선택 테스트 — PRIVATE+KIS_REALTIME, PUBLIC+KIS_HISTORICAL 각각의 Provider 선택, 승인 없는 PUBLIC+KIS_REALTIME 기동 실패(fail-fast), KIS 실시간 키 없이 KIS_HISTORICAL 빌드·기동 성공, 두 Provider가 `PriceQueryService` 동일 계약 만족, 화면 가격과 체결가격이 같은 Provider 사용
...
- **Decision Gate (한국투자 서면 답변, 실시간 한정)**: 공개 환경(`PUBLIC`)의 시세 공급자를 `KIS_REALTIME`으로 전환하는 것은 한국투자증권의 서면 허용 또는 계약 완료가 확인된 뒤에만 판단한다. 답변 전까지 공개 배포는 `KIS_HISTORICAL`로 진행하며, 아직 받지 않은 답변 내용을 문서·코드·설정 기본값에 반영하지 않는다 (C-007). 과거 데이터는 이미 공공데이터로 확인되어 이 게이트 대상이 아니다. 개인 개발 환경의 KIS 실시간 구현은 이 게이트와 무관하게 진행한다.
- **Decision Gate (KIS 과거 데이터 상품 세부사항)**: 다음 항목은 KIS Open API 과거 데이터 상품의 데이터 형식·timestamp 의미·제공시각이 확인되기 전까지 확정하지 않는다 — 정상 분봉 개수 판단 기준(임시 숫자 포함), 거래 없는 분을 상품이 어떻게 표현하는지, 분봉 timestamp가 구간 시작·종료 중 무엇을 의미하는지, `KisHistoricalCandleCollector`의 production 응답 파서, 정확한 데이터 제공시각(수집 배치 실행시각 포함). 재생기·주문 로직·수집 골격(전체 응답 오류 검증)은 샘플 데이터로 먼저 설계·테스트할 수 있다.
