...
> 상태: **1차 MVP 구현 완료. 2차 MVP(1차 고도화) 진행 중** (최종 갱신 2026-08-04)
>
> 목적: 팀이 Notion에서 확정한 제품 범위를 사람과 AI 구현자가 같은 요구사항으로 읽고, **현재 차수의 범위**를 넘지 않으며, 테스트 근거를 남기면서 구현하도록 만든 저장소 구현 기준 문서다.
>
> **이 문서는 1차 MVP 전용이 아니다.** 최초 반입 시점에는 1차 MVP만 담았으나 이후 2차 MVP(1차 고도화) 요구사항이 차수 표시와 함께 계속 추가됐다 — 지정가(LMT-001~005), 알림(NOTI-001~005), 투자일기(JOUR-001~006), 랭킹(RANK-001~002), 캔들 기간 확장(MKT-009), 3단계 투자 실습(`docs/specs/016-investment-education-policy`), AI 피드백(`docs/specs/012-ai-feedback`). **§4·§5·§6의 제목에 남은 "1차"는 그 절이 1차 내용만 담는다는 뜻이 아니다** — 각 요구사항 ID와 절마다 붙은 차수 표시를 기준으로 읽는다. 지금 무엇이 실제로 구현됐는지는 **§3의 "구현 현황"** 표를 정본으로 본다.
...
| AI 피드백 — 반사실 시뮬레이션·집단 비교 | FEED-010·011 | **완료** | `012` plan의 이슈 7 (PR #216, Closes #212). 반사실 3종 `returnRate` 수수료 재계산, `peerComparison`의 `NO_EVENT`·`INSUFFICIENT_SAMPLE`·`READY` 판정, 장 마감 배치(`PeerStatsBatchService`)의 `price_move_peer_stats` 확정 집계 |
| AI 피드백 — 코인 변동 감시 | FEED-005·006 코인 분기 | **완료** | `012` plan의 이슈 8 (PR #236, Closes #225). `CryptoPriceSnapshotService` 스냅샷 기록·`CryptoPriceMoveWatcher` 탐지·카드 확정, `GET /api/instruments/{id}/price-moves` 코인 분기(게이트 없음, 최근 24시간, `originTradeDate=null`) |
...
- 지정가 체결 알림 — 지정가 매수·매도 체결 시에만 발생(시장가는 즉시 응답으로 확인되므로 제외), SSE 실시간 push 포함 (NOTI-001~005, 2026-08-04, 이슈 #140) — **미착수** (지정가 체결 트리거 LMT-002가 선행)
- 지정가 주문과 상시 체결 (LMT-001~005, 2026-08-03 이벤트 드리븐 상시 처리로 재변경 — 배치 아님) — **일부 완료**: LMT-001~004(생성·체결 트리거·취소·미체결 목록) 완료, LMT-005(주문 수정) 정책만 확정하고 미착수(2026-08-06 반입). **2차 범위는 코인을 우선으로 시작하고 주식은 추후 처리한다**(2026-08-05 확정 — 주식은 재생 데이터 기반이라 "이 가격 도달 시" 조건 자체가 성립하기 어렵고, 분봉 판정 해상도·재생세션 자동취소·OCO와의 잠금 순서 공유 같은 부가 복잡도가 있다). 동시 체결 경합과 취소(LMT-003)·체결 트리거 동시 도착 경합은 비관적 락(SELECT FOR UPDATE, 잠금 순서 `order → account → holding` — 상세는 §LMT-002)으로 제어하기로 확정했다(2026-08-05, 잠금 순서는 2026-08-05 `docs/specs/015-limit-order` 구현·검증 중 정정)
...
- 코인 틱 집계와 캐싱 (MKT-010, 2026-08-06 튜터 피드백 계기로 착수 결정, MKT-008의 "틱 미집계·Redis 미저장" 결정을 뒤집음) — **미착수**. 코인 캔들(MKT-008)의 진행 중 1분봉을 빗썸 REST 재조회 대신 서버가 WebSocket 유입으로 직접 만들어 Redis에 캐싱한다. 새 MySQL 테이블은 만들지 않는다. **거래량 미확보·WebSocket 메시지 포맷 미검증 두 Decision Gate를 먼저 해소한 뒤 spec을 쓴다**(§MKT-010)
...
- 지정가·예약·부분 체결·슬리피지 (2차 LMT-001~005로 정의됨 — LMT-001~004 구현 완료, LMT-005 주문 수정은 미착수. 부분 체결·슬리피지는 여전히 범위 밖)
- 실시간 랭킹 (→ 2차 RANK-001 구현 완료)
- AI 피드백·AI 챗봇·그래프 DB (→ 2차 AI 피드백 일부 구현 완료. AI 챗봇·그래프 DB는 여전히 범위 밖)
- 알림 (2차 NOTI-001~005로 정의됨, 미착수)
...
> 이 절은 1차 MVP 요구사항으로 시작해 2차 MVP 요구사항이 차수 표시와 함께 추가됐다. **각 요구사항 ID의 차수는 그 절의 머리말과 제목의 "(2차 MVP)" 표기로 판정한다** — 표기가 없는 것이 1차다. 2차 요구사항 중 실제 구현된 것과 미착수인 것은 §3 구현 현황 표를 본다.
>
> - 1차: AUTH-001~006, ACCT-001~003, MKT-001~008, ORD-001~006, PORT-001~002, COM-001~003
> - 2차: MKT-009, **MKT-010**, PORT-003, LMT-001~005, NOTI-001~005, JOUR-001~006, RANK-001~002 (3단계 투자 실습 EDU-PRACTICE-*와 AI 피드백 FEED-*는 이 문서에 요구사항 절을 두지 않고 각각 `docs/specs/016-investment-education-policy`·`docs/specs/012-ai-feedback`이 정본이다)
...
- **(2026-08-06 MKT-010으로 대체 — 결정만 뒤집혔고 아직 미구현이다)** 코인 1분봉을 Redis에 저장하지 않는다는 기존 결정은 MKT-010에서 뒤집기로 했다. 다만 **새 MySQL 테이블을 만들지 않는다는 원문 의도는 그대로 유지**하며, 코인 시세의 정본도 여전히 외부(빗썸)다 — Redis 캐시는 정본을 대체하는 게 아니라 최근 짧은 구간만 보완한다. **현재 동작은 아직 이 문장의 원래 내용(저장 없이 중계)이다** — 구현 상태는 §3 구현 현황 표를 본다.
...
- 분 이하 해상도의 실시간 갱신은 프론트가 이 캔들 API를 짧은 주기로 재조회해 얻는다 — 진행 중 분봉이 매 호출마다 최신 값으로 갱신되므로 그것으로 충분하다(코인 전용 SSE 스트림은 두지 않는다. **이 SSE 제외 결정은 MKT-010에서도 유지된다** — 집계 결과는 여전히 이 캔들 API의 재조회로 전달한다). **(2026-08-06 MKT-010으로 대체 — 결정만 뒤집혔고 아직 미구현이다)** 서버가 틱을 분봉으로 집계하지 않는다는 기존 결정은 MKT-010에서 뒤집기로 했다 — 코인에도 주식의 틱 집계기(MKT-007)에 대응하는 구성요소를 만든다. 구현되면 이 캔들 API가 먼저 그 결과(Redis)를 보고 없는 구간만 빗썸 REST로 보충한다. 세부와 선결 과제는 MKT-010을 따른다.
...
**착수 전에 반드시 풀어야 하는 선결 과제 (Decision Gate)**

> 아래 세 가지가 해소되기 전에는 구현에 들어가지 않는다. 임시 숫자나 추정 필드명으로 확정하지 않는다 — MKT-005의 임계치 Decision Gate와 같은 성격이다.

- **거래량을 채울 수 없다.** 캔들 응답 계약(`docs/api-contracts.md`)의 `volume`은 필수 필드이고 코인은 소수 수량(`BigDecimal`)이다. 그런데 지금 우리가 WebSocket에서 실제로 뽑아내는 값은 **심볼·가격·수신시각 셋뿐**이다(`BithumbTickerMessageParser.BithumbTick`, `ticker` 채널 `content.closePrice` 파싱). 거래 수량이 없으므로 **현재 유입만으로는 OHLC는 만들어도 V를 만들 수 없다.** 이걸 해결하지 않으면 우리 봉이 빗썸 봉을 대체할 수 없다. 구독 채널이나 파싱 필드를 바꿔 수량을 받을 수 있는지 **실제 빗썸 연결로 확인**한 뒤, 가능하면 그 방식을, 불가능하면 "우리 봉으로 대체하지 않는 다른 설계"를 택한다.
- **WebSocket 메시지 포맷 자체가 미검증이다.** `BithumbWebSocketFeedClient`·`BithumbTickerMessageParser`의 구독 요청 JSON과 응답 필드 구성은 **실제 빗썸 연결로 확인된 적이 없다**고 두 클래스 주석에 명시돼 있다(이슈 #104 Decision Gate). 검증되지 않은 파서 위에 집계를 얹으면 집계 로직의 버그와 파싱의 버그를 구분할 수 없다. 외부 스모크로 포맷을 먼저 확정한다(C-005 — 자동 테스트로 실제 빗썸을 호출하지 않는다).
- **현재 빗썸 호출량을 먼저 실측한다.** §10에 이미 **레이트리밋 Decision Gate**가 있다 — "관측 전에 임의의 캐시 계층이나 TTL 숫자를 넣지 않는다". MKT-010은 바로 그 캐시 계층이므로 이 게이트의 대상이며, 폐기하지 않고 충족시킨다. 착수 전에 현재 호출량을 기록하고 구현 후 같은 기준으로 다시 측정해 대조한다. 실측 없이 개선을 주장하지 않는다(C-005). TTL 숫자도 이 실측 뒤에 정한다.

**spec에서 확정할 세부**

진행 중 분봉을 어디에 두는지(서버 메모리 vs Redis), 분 경계에서 누가 봉을 확정하는지, 늦게 오거나 순서가 뒤바뀐 유입을 어떻게 다루는지, 우리 데이터와 빗썸 데이터가 다를 때 무엇을 정본으로 볼지, 서버 재시작으로 생긴 빈 구간 처리, Redis TTL — 착수 시 `docs/specs/NNN-crypto-tick-aggregation`에서 확정한다. 기존 `price:crypto:<symbol>:snapshots` ZSET(아래 §6 Redis 키 책임)이 "Redis에 코인 시계열을 담는" 선례이므로 키 설계·retention 방식을 참고한다 — 다만 그쪽은 매 분 1회 샘플링이고 이번 건은 유입 누적이라 성격이 다르다.

- 범위: 1차 고도화. **2026-08-06 착수 결정, 아직 코드 없음** — 위 Decision Gate 해소 → spec 작성 → 구현 순서로 진행한다.
...
LMT-001~004의 상세 계약(요청·응답 필드, 전체 오류 코드)은 `docs/specs/015-limit-order/spec.md`에서 확정했다. LMT-001~004는 전부 완료돼 `OrderType.LIMIT`·`OrderStatus.PENDING`을 포함한 지정가 경로가 코드에 존재한다. **LMT-005는 이 문서에서 정책만 확정한 상태이며 미착수다** — 상세 계약(`PATCH` 요청 필드, 부분 갱신 허용 형태, 전체 오류 코드)은 구현 착수 시점에 spec에서 확정하고, `docs/api-routes.md`·`docs/api-contracts.md`도 그때 함께 갱신한다.

### 알림

> 알림은 2026-07-23 결정으로 1차 MVP 범위에서 제외됐고, 2차(1차 고도화)에서 다룬다. 범위를 지정가 체결 알림으로 한정하고 SSE 실시간 push를 포함하는 것으로 2026-08-04 확정했다(이슈 #140). 알림 목록·읽음 처리·삭제(NOTI-003~005)는 이슈 #140에는 없었으나 Notion API 명세서(정본) "알림" 그룹에 이미 정의돼 있어 2026-08-04 함께 반입했다. **2차 지정가(LMT-001~004)가 코인 전용으로 시작하므로(2026-08-05 확정) 2차 알림도 사실상 코인 지정가 체결에서만 발생한다** — 주식 지정가를 다루게 되는 시점에 알림 대상도 함께 넓어진다.
...
상세 계약은 2차 착수 시 알림 spec에서 확정한다. **알림 spec 폴더는 아직 만들지 않았고 번호도 배정되지 않았다** — 이전 서술이 가리킨 `docs/specs/013-notification`은 존재하지 않으며 `013`은 캔들 기간 확장(`013-candle-interval`)이 이미 점유하고 있다(2026-08-04 정정). 착수 시점에 비어 있는 번호(현재 `015`는 지정가용으로 참조 중이므로 `017` 이상)를 실제 폴더 목록으로 재확인해 배정한다 — 번호를 미리 지어내지 않는다.
...
- **향후 재검토 조건**: 투자일기 본문을 실제로 읽어 분석하는 기능(AI 피드백·주간/월간 리포트, spec 012)이 붙는 시점에 **그 기능의 스펙에서** "피드백·리포트 생성 시점 기준으로 그 직전/직후 수정을 제한한다" 같은 규칙을 정의한다. 잠금 트리거는 매도 이벤트가 아니라 **피드백 생성 이벤트**에 묶는 것이 설계 방향이다. `JOURNAL_LOCKED` 오류 코드는 그 결정 전까지 만들지 않는다.
...
- 실현손익의 정본은 MySQL `accounts.realized_pnl`이며 Redis ZSET은 조회 성능을 위한 파생 데이터다. Redis 유실 시 MySQL 원장으로 재구성한다 — 재구성 트리거·절차의 세부 구현은 착수 시 확정한다. 커밋 이후 갱신 자체가 실패하는 경우(예: 그 순간 Redis 장애)는 재시도(backoff) 후에도 실패하면 로그만 남기고 매도 체결 자체에는 영향을 주지 않는다 — 이 경우의 보상·정합성 재확인(재구성 배치 등)은 여전히 Decision Gate다.
...
- RANK-001과 동일하게 매도 체결 이력이 없는 사용자는 랭킹 대상이 아니다. **(2026-08-05 확정) RANK-001이 이런 회원을 ZSET에 member 자체가 없는 상태로 처리하는 것과 같은 원칙으로, 전용 상태값(enum)이나 오류 코드를 새로 만들지 않고 순위 필드를 부재(null)로 표현한다.**
...
**Decision Gate**: 전체 오류 코드, Redis 키 이름·자료구조 세부, 커밋 이후 Redis 갱신 실패 시 재구성 배치 등 보상·정합성 확인 경로는 착수 시 `docs/specs/014-ranking`에서 확정한다. 닉네임 노출 범위·매도 이력 없는 사용자 응답 형태·다중 인스턴스 score 갱신 경합·공동 순위 계산 방식은 위와 같이 확정했으므로 이 Decision Gate에서 제외한다(2026-08-05).
...
- **(2026-08-06 MKT-010 추가 예정, 미구현)** 코인 진행 중 1분봉 캐시: WebSocket 유입으로 만든 분봉을 담는 키가 추가된다. 정확한 키 형태·TTL은 착수 시 spec에서 확정한다. MySQL에는 저장하지 않는다(새 테이블 없음). 위 `:snapshots`와는 **별개 키**다 — 그쪽은 분당 샘플, 이쪽은 OHLCV 누적으로 목적이 다르다.
...
- **(MKT-010 선결)** 실제 빗썸 WebSocket 메시지 포맷 확정 — 구독 요청 JSON과 응답 필드 구성이 `BithumbTickerMessageParser` 추정과 맞는지(이슈 #104 Decision Gate), 그리고 **분봉 거래량을 채울 체결 수량을 받을 수 있는지**. 이 둘이 확인되기 전에는 MKT-010 구현에 들어가지 않는다
- 실제 카카오 OAuth
- 실제 네이버 OAuth
...
- **Decision Gate (빗썸 공개 API 레이트리밋)**: 코인 차트는 요청마다 빗썸을 조회하고 응답을 캐시하지 않는다. 동시 사용자 증가로 실제 레이트리밋 차단이 관측되면 그때 짧은 TTL 캐시 도입 여부를 판단한다 — 관측 전에 임의의 캐시 계층이나 TTL 숫자를 넣지 않는다.
  - **(2026-08-06) MKT-010이 이 게이트에 걸린다 — 폐기하지 않고 충족시킨다.** MKT-010은 캐시 계층을 새로 넣는 작업이므로 이 게이트의 대상이다. 다만 MKT-010의 근거는 레이트리밋 차단 관측만이 아니라 "외부 호출량이 사용자 수에 비례한다"는 구조적 문제와 "빗썸 장애가 곧 차트 장애"라는 가용성 문제까지 포함한다. 그래도 **이 게이트가 요구하는 실측을 건너뛰지 않는다** — MKT-010 착수 전에 현재 빗썸 REST 호출량(동시 사용자 수·차트 재조회 주기 기준 실측)을 먼저 기록하고, 구현 후 같은 기준으로 다시 측정해 대조한다. 실측 없이 "호출이 줄었다"고 주장하지 않는다(C-005). TTL 숫자도 이 실측 뒤에 정한다.
...
- 알림은 지정가 매수·매도 체결 알림으로 범위를 한정하고 SSE 실시간 push를 포함하는 것으로 확정했으며(2026-08-04, NOTI-001~005, 이슈 #140), SSE 연결·인증 스코프·알림 페이로드 필드 등 세부 계약은 착수 시 알림 spec에서 확정한다 — **spec 폴더는 미생성이고 번호 미배정이다**(`013`은 캔들 기간 확장이 점유). 알림은 지정가 체결 트리거(LMT-002)가 선행되어야 하므로 LMT보다 먼저 착수하지 않는다.
