...
| 투자 실습 — 시장가/지정가 매매 기반 완료 경로(OCO 없이) | MKT-PRACTICE-001~003 완료, 004·005·007(관찰만) 일부 완료 | **일부 완료** | `026-market-order-practice-tutorial`. 2단계 chain 해석(PR #295, 이슈 #294), 참조 가격선·evidence A/B 판정(PR #298, 이슈 #297), `POST /api/education/practice/holding-observations`(PR #302, 이슈 #300)까지 완료. `holding-reflections`·`GET /api/education/practice`(tasks.md 4·5번)가 남아 있어 사용자가 3단계를 끝까지 완료할 수는 아직 없다 |
