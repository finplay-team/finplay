...
> **정본 안내 (2026-07-24 개정)**
> - 팀이 확정하는 **제품 범위·API 단계·담당자는 [Notion 10 X TEN](https://app.notion.com/p/10-X-TEN-ae2b1fddfba9830abe9c813974422885)이 정본**이다.
...
|---|---|---|
| 투자일기 | 주문 전에 목표가·손절가 필수 | 1차 MVP 제외, 1차 고도화에서 별도 Spec으로 구현 |
| 주문 유형 | 시장가·지정가 | 시장가만 |
...
6. 조회
   ...
   - QueryDSL 목록 조회
7. 투자일기 — **1차 고도화로 이관, 현재 MVP에서 착수 금지**
   - `docs/specs/007-journal/`은 후속 구현을 위한 사전 문서이며 현재 MVP 완료조건에 포함하지 않는다.
8. 커뮤니티
   - 게시물 CRUD·페이지네이션
   - 평면 댓글 작성·조회·본인 삭제
9. 통합 검증
...
5. 주문 목록에서 주문요청 정보가 보이고, 체결내역에서 체결가격·수수료·실현손익이 보이는지 확인한다.
6. 코인 최신 시세에서 매수·매도한다.
7. 코인 시세 연결을 끊고 주문 차단을 확인한다.
8. 게시물을 작성·수정하고 댓글을 작성·삭제한다.
9. 다른 계정으로 주문·체결·콘텐츠를 조회하거나 변경할 수 없는지 확인한다.
