...
새로 작성하는 주석은 다음 기준을 따른다. 주석 관련 규칙의 정본은 이 절이며, `AGENTS.md`·`CLAUDE.md`는 이 절을 참조한다.

**유지한다**
- 코드만으로 드러나지 않는 비즈니스 제약, 기술적 함정, 트랜잭션·동시성·보안상 중요한 이유.
- 공개 API 계약과 프레임워크 사용에 필요한 Javadoc (예: 외부에 노출되는 인터페이스, 프레임워크 동작과 얽힌 설정값의 근거).

**축약한다**
- 클래스 Javadoc·메서드 Javadoc·인라인 주석이 같은 설명을 반복하는 경우 — 가장 적절한 한 곳에만 남기고 나머지는 지운다.

**삭제 대상으로 본다 (새로 작성하지 않는다)**
- 코드가 무엇을 하는지 그대로 옮겨 적는 주석.
- spec·plan·tasks·ADR·PR·이슈 본문의 내용을 그대로 반복하는 주석.
- PR 번호, 변경 이력, 작성 날짜처럼 Git과 관련 문서가 이미 관리하는 정보.

**적용 방법**
- 중요한 제약은 가능하면 테스트로 검증하고, 주석에는 "왜"만 간결히 남긴다.
- 과거 spec·plan·tasks에 남아 있는 개별 "주석을 추가한다" 지시보다 이 절의 기준이 우선한다. 해당 문서를 소급 수정하지는 않되, 새로 작성하는 주석은 이 절을 따른다.

이 절은 앞으로 작성하는 주석의 기준이며, 기존 코드에 이미 있는 주석을 이 기준에 맞춰 일괄 정리하라는 뜻은 아니다.

## 기타

- 새 Java 소스 파일은 클래스명만으로 책임이 충분히 드러나면 첫 줄 역할 주석을 강제하지 않는다. 이름만으로 책임을 파악하기 어려운 경우에만 한 줄 한국어 주석으로 보충한다. 기존 파일의 첫 줄 주석을 이 규칙을 이유로 지울 필요는 없다.
