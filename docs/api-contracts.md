...
| `holdHighPrice`·`holdLowPrice`와 두 비율 | 보유 구간이 **199분 이하면 1분봉**(`holdHighBasis="MINUTE"`), **초과면 일봉 표본**(`"DAILY"`)이다. 경계가 200이 아니라 **199**인 것은 양 끝을 포함해 세면 `199분 = 200봉`이라 그 위는 공급자 상한을 넘기 때문이다. 일봉 표본은 **매수 시각 이후·매도 시각 이전에 확정된 일봉 close**(= 매수일 ~ 매도 전날)만 쓰므로 **실제 최고가 이하일 수 있다.** 해당 일봉이 하나도 없으면(같은 날 안에서 200분 초과 보유) 극값·`atHoldHigh`가 전부 `null` |
