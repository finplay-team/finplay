...
| POST | /api/auth/oauth/login-exchange | body `{"code":"<위 리다이렉트의 code>"}` | 200 `{"accessToken":"<JWT>","refreshToken":"<JWT>","accessTokenExpiresInSeconds":3600,"refreshTokenExpiresInSeconds":1209600}` | 400 `VALIDATION_ERROR`(code 누락·형식 오류·만료·이미 소비됨 — 사유 구분 없이 동일 응답) 공통 오류 형식 | 002 AUTH-003, Issue #10 |
