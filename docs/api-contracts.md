...
- **`to`는 그 정확한 경계 시각을 배제(exclusive)한다** — 빗썸이 그렇게 동작한다(2026-08-03 외부 스모크로 실측 확인, `docs/specs/013-candle-interval/run-log.md` "`to` 파라미터의 경계 포함 여부" 참조). `1d`·`1w`·`1M`에서 `to`로 오늘 날짜(자정 KST로 해석됨)를 넘기면 **그 날짜의 봉이 응답에서 빠진다.** 서버는 이 값을 보정 없이 그대로 전달하므로, 오늘 봉까지 포함하려면 클라이언트가 `to`를 그다음 경계(다음 날 자정)로 넘기거나 `to`를 생략해야 한다. 이 배제 동작을 서버가 흡수해 항상 포함(inclusive)으로 만드는 개선은 이번 범위에 포함되지 않았다.
