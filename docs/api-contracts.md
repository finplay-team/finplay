...
순서는 도메인 기준이다 — auth → market → community.
...
- 아직 마감하지 않은 분봉(예: 장중 09:05 요청 시 09:05 분봉 자체)은 응답에서 제외된다 — `StockReplayService`의 가격 API 공개 컷오프 로직을 재사용한다. 09:00~09:00:59(첫 분봉 구간)에는 첫 분봉만, 09:01 이후에는 마감이 완료된 마지막 분봉까지, 15:30 마감 후에는 그날 공개된 분봉 전체가 후보가 된다.
