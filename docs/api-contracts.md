...
| Method | URL | 인증 | 경로 변수 | 성공 응답 | 오류 응답 | Spec |
|---|---|---|---|---|---|---|
| GET | /api/instruments/{instrumentId}/price | Access Bearer 필수 | `instrumentId` | 200 `{"price":71200,"sourceTime":"2026-07-22T09:00:00","status":"AVAILABLE","sourceTradingDate":"2026-07-22"}` (`PriceResponse`) — 코인은 `sourceTradingDate`가 `null`. **코인은 `status`가 `"STALE"`일 수도 있다**(032, 연결은 유지되지만 최신 틱이 10초를 넘긴 경우) — 예: `{"price":95000000,"sourceTime":"2026-07-28T10:30:00","status":"STALE","sourceTradingDate":null}`. `STALE`도 `price`·`sourceTime`은 마지막으로 실제 수신된 값이라 `AVAILABLE`과 동일하게 non-null이다 | 존재하지 않는 `instrumentId`는 404 `NOT_FOUND`. 유효한 최신 가격이 없으면(주식: 재생세션 미준비·미공개 분봉, 코인: 연결 끊김이거나 그 심볼의 시세를 한 번도 받은 적이 없음) 409 `PRICE_UNAVAILABLE` — **코인이 stale(연결 유지 + 10초 초과)인 것만으로는 더 이상 409가 아니다**(032 PRICE-STALE-001, 이슈 #355). 숫자가 아닌 `instrumentId`는 400 `VALIDATION_ERROR`. Access 인증 실패는 401 `UNAUTHORIZED` 공통 오류 형식 | 003 MKT-002/MKT-003/MKT-004, 032, Issue #16, Issue #355 |

이 API는 회원 소유 리소스가 아니라 공개 종목 정보 조회이므로 별도 소유권 검사는 없다(로그인만 하면 누구나 조회 가능). `PriceQueryService`가 `instrument.getMarket()`에 따라 주식은 주입된 `StockPriceProvider`(현재 구현체가 무엇인지 이 API·서비스는 알지 못한다), 코인은 `PriceStore`(Redis)에 위임하며, 어느 경로든 유효성 판정 실패는 컨트롤러가 아니라 서비스가 `BusinessException`으로 던지고 `GlobalExceptionHandler`가 응답 코드로 변환한다(컨트롤러에 try-catch 없음). **이 `STALE` 완화는 이 화면 조회 엔드포인트(및 이를 그대로 호출하는 소비자)에만 적용된다** — 주문 체결 가격 판정(`PriceQueryService.getOrderExecutionPrice`)은 이번 완화 대상이 아니며 stale 상태에서 여전히 `PRICE_UNAVAILABLE`로 주문을 거부한다(032 PRICE-STALE-003, MKT-004 fail-closed 유지). 주식 가격 조회는 이번 완화 대상이 아니다(032 범위 제외).
