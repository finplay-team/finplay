...
| Method | URL | 인증 | 경로 변수 | 성공 응답 | 오류 응답 | Spec |
|---|---|---|---|---|---|---|
| GET | /api/instruments/{instrumentId}/price | Access Bearer 필수 | `instrumentId` | 200 `{"price":71200,"sourceTime":"2026-07-22T09:00:00","status":"AVAILABLE","sourceTradingDate":"2026-07-22"}` (`PriceResponse`) — 코인은 `sourceTradingDate`가 `null`. **코인은 `status`가 `"STALE"`일 수도 있다**(032, 연결은 유지되지만 마지막 관측 시각이 10초를 넘긴 경우) — 예: `{"price":95000000,"sourceTime":"2026-07-28T10:30:00","status":"STALE","sourceTradingDate":null}`. `STALE`도 `price`·`sourceTime`은 마지막으로 실제 수신된 값이라 `AVAILABLE`과 동일하게 non-null이다. 034부터는 3초 주기 빗썸 REST 폴링이 체결 없이도 관측 시각을 계속 갱신해 정상 운영 중에는 `STALE`이 거의 뜨지 않는다(PRICE-REST-001·002) | 존재하지 않는 `instrumentId`는 404 `NOT_FOUND`. 유효한 최신 가격이 없으면(주식: 재생세션 미준비·미공개 분봉, 코인: 연결 끊김이거나 그 심볼의 시세를 한 번도 받은 적이 없음) 409 `PRICE_UNAVAILABLE` — **코인이 stale(연결 유지 + 10초 초과)인 것만으로는 더 이상 409가 아니다**(032 PRICE-STALE-001, 이슈 #355). 숫자가 아닌 `instrumentId`는 400 `VALIDATION_ERROR`. Access 인증 실패는 401 `UNAUTHORIZED` 공통 오류 형식 | 003 MKT-002/MKT-003/MKT-004, 032, 034, Issue #16, Issue #355, Issue #369 |

이 API는 회원 소유 리소스가 아니라 공개 종목 정보 조회이므로 별도 소유권 검사는 없다(로그인만 하면 누구나 조회 가능). `PriceQueryService`가 `instrument.getMarket()`에 따라 주식은 주입된 `StockPriceProvider`(현재 구현체가 무엇인지 이 API·서비스는 알지 못한다), 코인은 `PriceStore`(Redis)에 위임하며, 어느 경로든 유효성 판정 실패는 컨트롤러가 아니라 서비스가 `BusinessException`으로 던지고 `GlobalExceptionHandler`가 응답 코드로 변환한다(컨트롤러에 try-catch 없음). **이 `STALE` 완화는 034부터 주문 체결 가격 판정(`PriceQueryService.getOrderExecutionPrice`)에도 그대로 적용된다** — 코인은 연결 유지 + 수신 이력이 있으면 stale이어도 마지막 가격으로 시장가·지정가 주문이 체결된다(PRICE-REST-004, 032 PRICE-STALE-003의 fail-closed 체결 차단을 대체). 034가 함께 도입한 REST 폴링 백업(3초 주기 빗썸 ticker 전체 심볼 조회) 덕분에 정상 운영 중에는 stale 판정 자체가 거의 발생하지 않는다(PRICE-REST-001·002). **여전히 409 `PRICE_UNAVAILABLE`로 거부되는 코인 경우는 연결이 끊겼거나 그 심볼의 시세를 한 번도 받은 적이 없는 두 경우뿐이다**(PRICE-REST-005, fail-closed 잔여선 — "가격이 오래된 것"과 "가격 자체가 없는 것"은 여전히 다른 상태다). 주식 가격 조회·체결은 이번 완화 대상이 아니다(032·034 범위 제외).
...
- **snapshot**: `{"market":"CRYPTO","emittedAt":"2026-08-10T09:05:12","prices":[{"symbol":"BTC","price":71200000,"sourceTime":"2026-08-10T09:05:10","status":"AVAILABLE"}, ...]}` — 코인 12종 전체를 배열 1건에 담는다. 가격이 없는 종목도 배열에서 빠지지 않고 `price`·`sourceTime`은 `null`, `status`는 `UNAVAILABLE`이다. **연결은 유지되지만 최신 틱이 10초를 넘긴(stale) 종목은 빈 값으로 떨어지지 않는다** — `price`·`sourceTime`은 마지막으로 실제 수신된 값 그대로 채워지고 `status`는 `"STALE"`이다(032 PRICE-STALE-004, 이 서비스가 내부적으로 재사용하는 `PriceQueryService.getPriceQuote`가 완화된 값을 그대로 실어 나른다). 예: `{"symbol":"XRP","price":870,"sourceTime":"2026-08-10T09:04:55","status":"STALE"}`. 코인은 `sourceTradingDate`·`marketStatus` 개념이 없어 항상 `null`이며 `MarketSnapshotEvent`의 `@JsonInclude(NON_NULL)`로 필드 자체가 응답에서 생략된다(주식 snapshot과 같은 DTO를 재사용). id 없음.
...
| DTO | 필드 순서와 타입 | nullable 규칙 |
|---|---|---|
| `InvestmentPracticeResponse` | `String tutorialKey`, `String status`, `Integer currentStep`, `List<PracticeStepResponse> steps`, `LocalDateTime completedAt`, `Long rewardAmount`(신규, 이슈 #343) | `tutorialKey`·`status`·`steps` non-null; `currentStep`은 완료 시 null, `completedAt`은 완료 전 null. `currentStep`은 샘플 종목 chain이 미완료 상태로 진행 중이면 `4`. `rewardAmount`는 `status=COMPLETED`일 때만 `5000000`, 그 외(`NOT_STARTED`·`IN_PROGRESS`)에는 항상 `null` |
