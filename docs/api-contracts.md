...
> 아래 규칙은 원래 `1m` 계약이며 `1d`·`1w`·`1M`(013)에도 그대로 적용됐다 — 코인은 4개 `interval` 모두 저장·집계 없이 빗썸에 위임하는 같은 구조였다. **(2026-08-06 MKT-010, `021-crypto-tick-candle-cache`) `1m`만 이 구조에서 갈라졌다** — 서버가 빗썸 체결 스트림으로 직접 만든 최근 진행 중 분봉을 Redis에 캐싱하고, 캐시에 없는 구간만 아래 REST 위임으로 보충한다. `1d`·`1w`·`1M`은 이 절 그대로이며 위 "코인 위임(`1d`·`1w`·`1M`) 규칙 요약"을 참조한다.
