...
**`status`(`READY`\|`REBUILDING`) — 빈 `content`의 의미를 구별해주는 필드다 (Issue #279).** `READY`면 ZSET이 원장을 반영하고 있다는 뜻이라 `content: []`는 "실제로 매도 이력이 있는 회원이 없다"는 정상 상태다. `REBUILDING`이면 Redis ZSET이 유실돼 재구성(기동 훅 또는 매일 04:20 KST 배치) 전이라는 뜻이고, 지금 보이는 값은 신뢰할 수 없다. 판정은 **ZSET에 멤버가 하나도 없으면서 그 시장에 매도 체결 이력 계좌가 존재할 때만** `REBUILDING`이고 그 외에는 전부 `READY`다. 유실 상태여도 오류가 아니라 200이며, 기존 필드(`market`·`content`)는 그대로라 이 필드를 모르는 클라이언트는 영향을 받지 않는다(필드 추가만, 하위 호환).
...
| Method | URL | 인증 | 쿼리 파라미터 | 성공 응답 | 오류 응답 | Spec |
|---|---|---|---|---|---|---|
| GET | /api/rankings/me | Access Bearer 필수 | `market`(필수, `STOCK`\|`CRYPTO` 리터럴만 허용) | 200 `{"market":"STOCK","status":"READY","rank":3,"nickname":"존버맨","realizedPnl":120000}` (`MyRankingResponse`); 매도 체결 이력이 없으면 200 `{"market":"STOCK","status":"READY","rank":null,"nickname":"투자왕","realizedPnl":0}`(오류 아님); 매도 이력이 있는데 집계가 유실됐으면 200 `{"market":"STOCK","status":"REBUILDING","rank":null,"nickname":"존버맨","realizedPnl":0}` | `market` 누락 또는 `STOCK`\|`CRYPTO` 외 리터럴(예: `FOREX`)은 400 `VALIDATION_ERROR`. Access 인증 실패는 401 `UNAUTHORIZED` 공통 오류 형식 | 014 RANK-002, Issue #233·#279 |
...
**`status`는 Redis 장애 시의 폴백이 아니다.** "ZSET을 읽을 수 있는데 내용이 비어 있다"를 표현하는 필드이며, Redis 자체가 죽으면 두 엔드포인트 모두 여전히 500 `INTERNAL_ERROR`다.
