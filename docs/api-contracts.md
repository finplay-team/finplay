...
| Method | URL | 인증 | 쿼리 파라미터 | 성공 응답 | 오류 응답 | Spec |
|---|---|---|---|---|---|---|
| GET | /api/instruments/{instrumentId}/candles | Access Bearer 필수 | `interval`(필수, `1m`\|`1d`\|`1w`\|`1M` 중 하나, **대소문자 구분**), `from`·`to`(선택, ISO-8601 `LocalDateTime`, 예: `2026-07-27T09:00:00`), `cursor`(선택, ISO-8601 `LocalDateTime` **문자열**, 예: `2026-07-27T09:00:00` — 직전 페이지 `content[0].sourceTime`을 그대로 넣는다) | 200 `CandleListResponse` — `{"content":[{"sourceTime":"2026-07-22T09:00:00","open":71000,"high":71500,"low":70900,"close":71200,"volume":12345}, ...],"nextCursor":"2026-07-22T09:00:00","hasNext":true}`. `content`는 시각 오름차순, 모든 `interval`·모든 시장 공통 최대 200개(봉 개별 필드는 013 계약 그대로 불변). `nextCursor`는 `content`의 가장 오래된 봉(`content[0]`)의 `sourceTime`이고 `hasNext=false`면 항상 `null`. `hasNext`는 "이번 페이지 `content`가 200개로 꽉 찼는가"이며 유일한 예외는 주식 `1m`으로 200개여도 항상 `false`다. 주식은 어떤 `interval`이든 아직 공개된 분봉이 없거나 재생세션이 준비되지 않은 경우 예외 없이 200(빈 `content`) — **단 `marketStatus=CLOSED`이면 빈 `content` 대신 마지막 재생 거래일의 하루치 봉을 반환한다**(038, 아래 "주식 캔들 전용 규칙" 참조) | `interval`이 `1m`·`1d`·`1w`·`1M`이 아니면(대소문자 변형 `1D`·`1W`·`1MO`·`1min` 포함) 400 `VALIDATION_ERROR` — **커서가 유효해도 이 판정이 먼저다.** `cursor`가 ISO-8601 파싱에 실패하면 400 `VALIDATION_ERROR`(주식 `1m`에서도 형식 검증은 동일하게 적용된다 — 무시되는 것은 "형식이 맞는 커서의 효과"뿐이다). `cursor`가 **없을 때만** `from > to`면 400 `VALIDATION_ERROR` — **코인은 `interval`과 무관하게 항상 시각까지 포함해 비교**하지만, 주식 `1d`·`1w`·`1M`은 날짜 성분만 비교한다(아래 요약 참조). `cursor`가 있으면 `to`는 값·검증 모두에서 무시되고, `from`이 정규화된 상한보다 뒤여도 400이 아니라 빈 `content`·`hasNext=false`·`nextCursor=null`인 정상 200이다(아래 "커서 페이지네이션 규칙" 참조). 존재하지 않는 `instrumentId`는 404 `NOT_FOUND`(`interval` 검증 이후). 코인에서 빗썸 조회가 실패하면 502 `MARKET_DATA_PROVIDER_ERROR`. Access 인증 실패는 401 `UNAUTHORIZED`. **새 오류 코드는 만들지 않는다** — 4값 확장(013)·커서 도입(048) 모두 `interval` 허용값·성공 응답 형태만 바꿨다 | 003 MKT-002·MKT-008, 013 MKT-009, 027 MKT-010(이슈 #242, `1m` 코인만 캐싱 경로 추가), 038, 048 CANDLE-PAGE-001~012·025·026, Issue #17, Issue #20, Issue #143, Issue #384, Issue #473 |
...
#### 커서 페이지네이션 규칙 (048, CANDLE-PAGE-001~012·025·026, 이슈 #473)

> 200개 상한 자체는 013에서 정한 그대로 유지된다(이슈 #155 — 읽기 비용 방어). 이 절은 "그 앞 구간을 이어서 받는" 계약만 추가한다. 013·027에 남아 있던 "200개를 넘는 구간을 이어붙이는 페이징은 1차 범위가 아니다" 서술은 이 절로 대체됐다(013·027 문서 자체는 이력 표시만 남기고 원문을 지우지 않는다).

- **`to`와 `cursor`를 동시에 보내면 `to`는 무시된다** — 400이 아니다. `from`은 하한으로 계속 유효하다.
- **마지막 페이지가 정확히 200개면 빈 `content` 페이지가 한 번 더 나오고 거기서 `hasNext=false`가 된다** — 버그가 아니라 계약이다. 소비자는 빈 페이지 1회를 정상 종료로 받아들인다.
- **끝 판정은 오직 `hasNext`다.** 거래일이 없는 주·월, 체결이 없는 분 때문에 `content` 안의 봉 사이가 달력상 비는 것은 데이터 끝의 신호가 아니다.
- **코인은 `content`의 가장 오래된 봉이 요청한 `from`보다 과거일 수 있다** — 빗썸이 "구간"이 아니라 "`to` 기준 최신 N개"를 주기 때문이며 페이징 이전부터의 기존 동작이다. `from`은 코인에서 하한 보장이 아니라 조회 폭의 힌트다. `nextCursor`는 요청 창이 아니라 실제로 돌려준 가장 오래된 봉에서 나오므로 이어받기에는 영향이 없다.
- **커서는 분 단위로 내림 해석된다.** 서버는 `to = cursor - 1분`으로 정규화해 조회한다 — 응답 `sourceTime`은 항상 분 경계이므로 정상 페이징(직전 페이지의 `nextCursor`를 그대로 다음 `cursor`로 쓰는 흐름)에서는 절대 발생하지 않지만, 소비자가 `10:00:30`처럼 분 경계가 아닌 값을 직접 넣으면 `09:59:30`으로 내려가 `10:00` 봉이 응답에서 빠진다.
- **주식 `1m`은 커서를 조회에 적용하지 않는다** — 형식이 올바른 커서를 보내도 봉 목록은 커서가 없을 때와 동일하며, `hasNext`는 항상 `false`·`nextCursor`는 항상 `null`이다(CANDLE-PAGE-025·026). 형식이 잘못된 커서는 주식 `1m`에서도 400이다.
...
- **한 요청의 최대 개수는 200개**(빗썸 API 상한)다. `from`·`to` 범위가 200분을 넘으면 `to`(또는 커서가 있으면 그 정규화된 상한) 기준 최신 200개만 반환한다. **200개를 넘는 구간은 `cursor`로 이어받는다**(048, 위 "커서 페이지네이션 규칙" 참조).
...
- **데이터가 없는 과거 구간·체결이 뜸한 구간 관측(외부 스모크, 2026-08-20 KST, `docs/specs/048-candle-history-pagination/run-log.md` 참조)** — 빗썸 공개 캔들 REST(`GET /v1/candles/minutes/1`)를 직접 호출해 확인한 결과: ① `to=1990-01-01T00:00:01`처럼 그 심볼에 체결이 있을 수 없는 먼 과거를 요청하면 **오류가 아니라 빈 배열 `[]`(HTTP 200)**이 온다 — CANDLE-PAGE-016이 "데이터 끝"으로 취급해야 한다고 명시한 형태와 일치한다. ② 체결이 상대적으로 뜸한 `KRW-STEEM`을 `count=200`으로 요청하면 응답 개수는 **정확히 200개**였고, 가장 오래된 봉(`candle_date_time_kst`)은 요청 시각(`to`) 기준 약 **2일 2시간 전**으로 — 창 폭(200분)보다 훨씬 과거까지 뻗었다. `resolveCount`가 계산한 200분 창과 실제 응답이 커버하는 시간 폭이 다르다는 §9-1의 논리적 결론을 실측으로 확인했다.
...
**조회 시 생성하지 않는다.** GET이 외부 LLM을 호출하고 DB에 쓰면 `docs/conventions/code.md`의 "GET은 부수효과 없음"을 어기고, 배치 갱신 직후 동시 요청이 전부 LLM을 호출하며, 그 순간의 첫 사용자가 최대 40초(20초 × 재생성 1회)를 기다린다. 배치로 옮기면 이 경로는 순수 조회가 되고 호출량도 사용자 수와 무관하게 고정된다. 코인은 실시간이라 미래를 모르므로 미리 만들어 두어도 스포일러 위험이 없다 — 주식이 사전 배치인 이유가 비용이 아니라 스포일러 차단이었던 것과 대비된다.
