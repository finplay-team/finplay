...
**Part C와 상태값이 갈리는 구간은 `READY` + 09:00 이전 한 곳뿐이다.** 그때 카드 목록은 `EMPTY`(`originTradeDate` 채움)이고 `summaryStatus`는 `NOT_YET`이라 **한 화면의 두 영역이 서로 다른 상태값을 보인다** — 의도된 차이다. **개장 후에는 Part C가 `NOT_YET`이 될 수 없다** — `summaryStatus`의 `NOT_YET`은 재생세션 미준비와 09:00 이전 두 곳에서만 나오므로(아래 "종목 뉴스 요약 조회" 절), 개장 후 첫 `revealTime` 이전 시각에는 카드 목록만 `EMPTY`이고 Part C는 `READY`·`EMPTY`·`UNAVAILABLE` 중 하나다.
