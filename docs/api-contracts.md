...
- 만료 판정은 매 요청 시점에 재계산하며 샘플 attempt 응답의 4단계 `status=EXPIRED`로 명시한다. attempt 자체를 GET에서 변경하지 않는다. 이미 `practice_completions` 행과 `attempt.status=COMPLETED`가 있으면 만료 여부를 다시 검사하지 않는다 — 완료 replay는 원장 변화와 무관하게 회귀하지 않는다.
