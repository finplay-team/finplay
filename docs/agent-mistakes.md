...
| 2026-07-25 | 저장소 전용 `feature` 하네스 위에 범용 planning·SDD·task-review 하네스를 중첩 적용 | Issue 하나에 별도 ledger·brief·review package와 Task별 reviewer가 추가되고 구현 계획이 656줄로 팽창해 같은 검증을 반복 | `feature`를 단일 오케스트레이션 경로로 고정하고 중복 산출물과 범용 하네스 중첩을 중단 | `feature` 사용 중에는 해당 스킬의 planner → implementer → tester → build → reviewer → 문서 동기화 단계만 사용하고 계획은 기본 200줄 이내로 유지 |
