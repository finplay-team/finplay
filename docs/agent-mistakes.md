...
| 2026-07-25 | 실행 중인 Gradle 테스트를 짧은 하네스 타임아웃으로 끊은 직후 같은 작업 디렉터리에서 테스트를 다시 실행 | 두 테스트 작업이 `build/test-results/test/binary`를 경합해 `in-progress-results-generic.bin` `NoSuchFileException`으로 실패 | 첫 빌드 종료와 Gradle daemon `IDLE`을 확인한 뒤 충분한 타임아웃으로 단독 재실행해 통과 | Gradle 명령은 예상 실행시간보다 긴 타임아웃을 사용하고, 타임아웃 뒤에는 자식 Java/Gradle 프로세스 종료 상태를 확인한 후 재실행 |
| 2026-07-25 | 저장소 전용 `feature` 하네스 위에 범용 planning·SDD·task-review 하네스를 중첩 적용 | Issue 하나에 별도 ledger·brief·review package와 Task별 reviewer가 추가되고 구현 계획이 656줄로 팽창해 같은 검증을 반복 | 프로젝트 전용 스킬을 정본으로 사용하고 범용 스킬은 설계 선택이나 실패 원인이 불명확할 때 필요한 분석에만 제한 | 요구사항이 명확하면 planner, 원인이 명확한 실패면 implementer ↔ tester 루프를 사용하고 별도 plan·ledger·task brief·reviewer 체인을 만들지 않음 |
