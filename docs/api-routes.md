...
| GET | /api/instruments/{instrumentId}/price | market | 인증 사용자의 종목 현재가 조회. 주식은 StockPriceProvider(재생/실시간 공급자 불문), 코인은 PriceStore(Redis)에서 유효한 최신 가격을 반환 — 연결 유지 + stale(10초 초과)이면 `status: "STALE"`로 마지막 가격을 계속 반환(체결 판정은 별도) | 003 MKT-002/MKT-003/MKT-004, 032, Issue #16, Issue #355 |
