...
| POST | /api/community/posts | community | 인증 사용자의 텍스트 게시물 작성. `instrumentId`(선택)로 종목을 하나 태그할 수 있다 — 존재하지 않거나 비활성(`tradable=false`) 종목은 400 `VALIDATION_ERROR`. `imageId`(선택)로 `POST /api/community/posts/images`에서 미리 업로드한 이미지 하나를 선(先)업로드-후(後)참조로 연결한다 — 존재하지 않으면 404, 타인 소유면 403, 이미 다른 게시물에 연결됐으면 400. 응답에 `likeCount`·`likedByMe`(생성 직후는 항상 0·false) 포함 | 008 COM-001, 022 COM-004·COM-006, Issue #23, Issue #246, Issue #248; 045 LIKE-002 |
| GET | /api/community/posts?page=&size=&instrumentId=&sort= | community | 인증 사용자의 게시물 목록을 페이지네이션으로 조회. `sort`는 `latest`(기본값, 생략 시 기존과 동일한 최신순)\|`popular`(좋아요 내림차순, 동률은 최신순) — 그 외 값은 400 `VALIDATION_ERROR`. `instrumentId` 지정 시 그 종목이 태그된 게시물만 반환(미지정 시 전체, `sort`와 함께 사용 가능). 응답에 태그 종목·첨부 이미지 정보, 각 게시물의 `likeCount`·`likedByMe`(배치 조회로 N+1 방지) 포함(없으면 각각 `null`) | 008 COM-001, 022 COM-004·COM-006, Issue #24, Issue #246, Issue #248; 045 SORT-001, LIKE-002 |
| GET | /api/community/posts/{postId} | community | 인증 사용자의 커뮤니티 게시물 단건 조회. 응답에 태그 종목·첨부 이미지 정보, `likeCount`·`likedByMe` 포함(없으면 각각 `null`) | 008 COM-001, 022 COM-004·COM-006, Issue #25, Issue #246, Issue #248; 045 LIKE-002 |
