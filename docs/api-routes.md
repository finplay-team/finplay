...
| POST | /api/community/posts | community | 인증 사용자의 텍스트 게시물 작성. `instrumentId`(선택)로 종목을 하나 태그할 수 있다 — 존재하지 않거나 비활성(`tradable=false`) 종목은 400 `VALIDATION_ERROR` | 008 COM-001, 022 COM-004, Issue #23, Issue #246 |
...
| PATCH | /api/community/posts/{postId} | community | 본인 소유 커뮤니티 게시물의 제목·본문·종목 태그(`instrumentId`, 선택) 전체 교체 수정. 존재하지 않거나 비활성 종목 태그는 400 `VALIDATION_ERROR` | 008 COM-001, 022 COM-004, Issue #26, Issue #246 |
