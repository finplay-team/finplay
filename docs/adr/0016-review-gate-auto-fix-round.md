...
- 이력 코멘트·판정 병합·승인 스텝(자동 수정 라운드가 실패/취소/빌드실패/재리뷰실패로 끝나는 경우를 포괄하는 스텝들과 `judge`, 조건부 승인)은 `always()`가 아니라 `!cancelled()`로 돈다 — "앞선 스텝 실패로 success() 체인이 깨진 뒤에도 실행"이라는 목적은 유지하되, 취소된 런에서까지 이력 코멘트가 붙거나 승인으로 이어지지 않는다(PR #293 3차 리뷰 참고 1).
