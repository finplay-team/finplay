...
>
> **재도입 시점 확정**: 최소 CI(전체 build·테스트 + docs-only 스킵)는 **`docs/specs/010-deployment`에서 첫 배포 직전에** 재도입한다. 이 문서가 다루는 "이슈 트리거 CI 하네스"는 그보다 뒤의 별도 단계이며, 010과 혼동하지 않는다 — 010은 빌드 게이트, 이 문서는 에이전트 실행 자동화다.
