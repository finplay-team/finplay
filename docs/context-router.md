...
| 배포 스택 실행 (수동 배포 절차 — 파이프라인 폴백) | `deploy/README.md` (+ `compose.deploy.yaml`·`deploy/nginx.conf`·`Dockerfile`). **DB·캐시는 이 스택 안에 없다** — RDS·ElastiCache이며 근거는 ADR-0020. **정상 배포 경로는 더 이상 이 문서가 아니다** — ADR-0021 이후 이 절차는 파이프라인이 막혔을 때만 쓴다. **nginx는 폐기 예정이다** — ADR-0022가 프론트를 S3로 분리하기로 결정했으므로 `deploy/nginx.conf`와 compose의 nginx 서비스는 제거 대상이며, 아직 파일이 남아 있어도 정본은 0022다 |
