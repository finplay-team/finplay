...
>
> **작성 규칙(`docs/specs/README.md`)** — 항목 하나 = implementer 1회 투입 = 커밋 1개. `run-log.md`에 시각·에이전트·실행 명령·근거를 1줄씩 남긴다. 테스트 레벨은 ADR-0003을 따른다.
>
...
- **새 엔드포인트가 없다.** `docs/api-routes.md`는 대상이 아니다. `docs/api-contracts.md`는 `016fb543`에서 이미 갱신됐다 — **다시 쓰지 않는다.**
...
- [ ] **4. 코인 조회 경로 테스트 — 게이트·200봉 경계·일봉 부재**
  ...
  - **200봉 경계**(결정 4) — 보유 **199분 이하**면 `holdHighBasis="MINUTE"`이고 1분봉 close 최댓값인지, **200분 초과**면 `"DAILY"`이고 **매수일 ~ 매도 전날 일봉 close**만 표본에 들어오는지(**매도일 일봉이 섞이면 안 된다** — 보유하지 않은 구간의 가격이다). 경계값 두 개를 각각 케이스로 둔다.
  - **일봉 표본이 비는 경우** — 같은 날 안에서 200분 초과 보유. `holdHighPrice`·`holdHighAt`·`sellVsHighRate`·`counterfactuals.atHoldHigh`가 전부 `null`이고 **오류가 아닌지**(§실패 처리 표). `holdLowPrice`·`sellVsLowRate`도 같은 규칙인지.
...
- [ ] **5. E2E 고정 — 코인 체결이 200이고 주식 경로·원장이 그대로다**
...
- [ ] **6. `docs/prd.md` §3 "구현 현황" 갱신**
  ...
  **갱신 대상이다** (CLAUDE.md 규칙 10). 195·196행의 FEED-007·FEED-010·011 행은 **주식 전용으로 완료**였고 이 PR이 **코인 체결에 같은 기능을 연다** — 제공하는 기능의 범위가 실제로 늘어나므로 리팩터링·강화성 개선(#244가 비대상이었던 이유)과 다르다.
  ...
  - 근거 칸에는 **무엇이 열렸는지**를 적는다 — "코인 체결도 200(§FEED-012 결정 1~4: KST 자정 게이트·일봉 종가·200분 정밀도 경계·00:05 확정 배치)". 판정만 있고 근거가 없으면 다음 사람이 검증할 수 없다.
  - 검증 — 없음(문서 대조뿐).
