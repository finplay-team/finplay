...
- [x] **2. `V13__create_ai_feedback_tables.sql` — 일곱 테이블**
  ...
  - `price_move_events`의 유니크에 `event_type`이 들어간다(§데이터 모델). 그 유니크가 실제로 충돌을 막는지 확인하는 `@DataJpaTest`는 **#4 소유**다.
    - **결과적으로 이 이슈가 그 케이스를 먼저 만들었다.** 4번 항목의 유니크 테스트를 쓰면서 `window_start=09:00`이 같아도 `INTRADAY`/`OPENING_GAP`이면 2건이 공존하는 것을 단정하게 됐다(`PriceMoveEventRepositoryTest`). 완료 조건의 **소유는 `plan.md` 배정표대로 #4에 남는다** — #4는 탐지 로직이 그 두 카드를 실제로 만들어 내는 경로까지 확인해야 하므로, 제약만 보는 이 테스트로 대체되지 않는다.
