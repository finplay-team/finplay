...
- [ ] **5. Part C 조회 API — 종목 뉴스 목록·요약 (주식)**
  ...
  - **절단은 정렬과 별개다**(§뉴스 매칭 범위, 2026-08-04 확정). 상한을 넘으면 **공시를 먼저 채우고 남은 자리를 뉴스 최신순으로** 채운 뒤, 살아남은 항목을 위 정렬로 실어 보낸다. 공시가 목록 아래쪽에 오는 것은 계약대로다. **픽스처를 상한 위로 잡아야 이 규칙이 검증된다** — 상한 아래 픽스처는 절단 자체가 일어나지 않아 규칙의 유무를 구분하지 못하고, 규칙을 지운 회귀에도 초록이다.
