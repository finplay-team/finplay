...
그래서 **Part C와 상태값이 갈리는 구간은 `READY` + 09:00 이전 한 곳뿐이다** — 카드 목록 `EMPTY` vs `summaryStatus` `NOT_YET`. **개장 후에는 두 값이 갈리지 않는다**: Part C의 `NOT_YET`은 위 판정 순서 1·2번에서만 나오므로 개장 후 첫 `reveal_time` 이전 시각에 `summaryStatus`는 `NOT_YET`이 아니다.
