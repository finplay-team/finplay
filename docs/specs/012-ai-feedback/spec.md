...
가격 조회에만 "분봉" 표현을 쓰는 이유는 이렇다. `003-market-data`가 분봉 timestamp를 **구간 시작 기준**으로 다루므로(`StockReplayService.findRevealedCandle`이 첫 분봉 09:00을 09:00~09:00:59 동안 "마감 전"으로 취급) **그날 마지막 분봉은 15:29일 가능성이 높다.** `15:30 종가`를 찾는 코드는 **항상 `null`을 받고 예외는 안 난다** — 시가 갭 카드와 `atClose`가 매일 조용히 비게 된다.
