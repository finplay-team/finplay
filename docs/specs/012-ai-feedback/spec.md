...
| 서술 재생성 | `postSellFlow`가 `READY`이고 `peerComparison.status != NOT_YET`. 주식·코인 공통 |
...
매 분 실행 (feedback.batch.crypto-watch-cron):
  ...
  근거 기사 0건이면 온디맨드 수집(ADR-0016) 후 1회 재매칭 → 그래도 0건이면 종료 (FEED-003과 동일)
...
| 근거 기사 0건 | 카드 미생성 (설계 의도). **코인은** 이 상태에 도달하기 전에 온디맨드 수집·1회 재매칭을 거친다(ADR-0016) |
| 코인 온디맨드 수집 실패 | 그 종목의 이번 틱만 건너뛴다. `watch()`의 기존 실패 격리와 동일 (ADR-0016) |
...
| 네이버 플랫폼 (2026-08-04 확인) | 검색 API는 **NAVER API HUB**(네이버 클라우드 플랫폼이 중개 운영)에서 발급받는다. `developers.naver.com`의 일반 애플리케이션 등록 화면에는 `검색`이 없다. **콘솔이 값을 "Client ID/Secret"이라 부르지만 실제 헤더는 위의 API Gateway 규격이며**, 구 `openapi.naver.com` + `X-Naver-Client-*` 조합으로 부르면 이 키는 인증되지 않는다. 응답 본문 필드(`title`·`originallink`·`link`·`description`·`pubDate`)는 구 API와 같다. 한도는 하루 25,000회·월 775,000회이고 이 spec의 사용량은 종목 28 × 하루 48회 = 1,344회다. **코인 온디맨드 수집(ADR-0016)이 최악 코인 종목 12 × `daily-limit`(6) = 72회/일을 더해 합계 최악 1,416회/일**이며, 여전히 한도에 크게 못 미친다 |
