...
**매수 회고가 여러 건이면 `- 매수 09:30:` 줄이 매수 시각 오름차순으로 늘어난다**(상한 `journal.max-buy-journals`). 시각을 줄머리에 두는 이유는 위 파생 사실·변동 목록이 전부 시각 기준이라 모델이 같은 축에서 읽게 하기 위해서다. 본문은 **`journal.max-journal-chars`자에서 절단한다**(§C-7).
