...
#### 결정 4 — 보유 구간 최고가는 200분을 경계로 정밀도가 갈린다

가장 실질적인 제약이다. 코인 분봉 공급자는 **200봉 상한**(`013-candle-interval` 38행)이고 `CryptoCandleStore`의 Redis TTL도 같은 계약에서 유도한 4시간이라, **200분을 넘게 보유하면 1분봉으로 구간 최고가를 구할 수 없다.**

```
보유 구간(buyAt ~ sellAt)이 200분 이내
...
보유 구간이 200분 초과
  ...
  → holdHighBasis = "DAILY"
  → 해당하는 일봉이 하나도 없으면 (같은 날 안에서 200분 초과 보유)
...
**`postSellHighPrice`에는 일봉 근사를 쓰지 않는다.** 매도~자정 구간에 걸친 일봉은 그 하루 하나뿐인데 **그 일봉은 매도 전 시간대를 통째로 포함**한다. "매도 후 최고가"로 올리면 근사가 아니라 **틀린 값**이다. 그 구간이 200분을 넘어 1분봉을 못 구하면 `postSellHighPrice`·`postSellHighAt`은 `null`이다.
...
| 코인 보유 200분 초과로 1분봉 부족 | 극값을 일봉 표본으로 낮춰 계산하고 `holdHighBasis="DAILY"`. 해당 일봉도 없으면 극값·`atHoldHigh`가 `null`. 오류 아님 (§FEED-012 결정 4) |
