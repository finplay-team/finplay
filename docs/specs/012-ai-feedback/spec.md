...
>
> **선행 조건**: PRD C-004의 "외부 시장 정보 미사용" 조항 개정이 먼저 반영되어야 한다 (§정책 전제).
>
...
이 spec은 PRD 운영 정책의 "AI 피드백 — 종목 추천 금지, **외부 시장 정보(뉴스·공시) 미사용**"과 충돌한다. 착수 전 다음과 같이 개정한다.
...
- [ ] LLM 실패 시에도 수치 요약은 200으로 반환한다 (서술만 템플릿).
...
문장 수는 파트별로 다르다 — 변동 카드 2~3문장, 매도 회고 2~3문장, 종목 뉴스 요약 3~5문장, 개장 전 브리핑 3~6문장.
...
| 매도 회고 | `{매수가}원에 매수해 {매도가}원에 매도했습니다. 수익률은 {±X.XX}%입니다.` |
...
- [ ] LLM 호출이 실패해도 매도 회고 수치 요약이 200으로 반환되는 테스트 통과.
