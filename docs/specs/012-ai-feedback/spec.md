...
> **위 두 규칙은 2026-08-07 실측으로 개정됐다 (이슈 #179).** 개정 전 규칙은 질의어 `이름 + " 코인"`, 필터 "다른 종목명이 있으면 제외"였다. 12종목 × 100건을 세 가지 질의어로 받아 대조한 결과다 (`tools/coin-news-measure/`, 원자료는 재현 가능).
>
...
> | 개정 후 (`이름 + 심볼`) | 735 | **261** |
>
> **개정 전 규칙은 자기 이름이 제목에 있는지를 아예 보지 않았다.** 그래서 통과한 680건 중 **535건(79%)이 제목에 그 코인 이름이 없었다** — `체인링크 코인` 질의에 `"두바이듀티프리 암호화폐 결제, 디르함 정산이 핵심"` 같은 기사가 근거로 붙었다. `비트코인캐시`·`이더리움클래식`은 저장된 51건 중 **자기 이름이 든 제목이 0건**이었다.
>
> **`" 코인"` 접미가 오히려 해로웠다** — 일반 암호화폐 기사를 끌어와, 이름만 쓰는 것(170)보다 낮은 145를 냈다. 심볼로 바꾸면 261로 80% 오른다.
>
> **누락(다른 종목명 때문에 버려지는 정상 기사)은 작은 문제였다.** 제외된 520건 중 자기 이름이 든 것은 30건(6%)뿐이라, 제외 규칙만 완화하는 안은 1,200건 중 30건을 살렸다. 그래서 그 안을 단독으로 쓰지 않았다.
>
...
>
> **심볼을 자기 이름으로 함께 인정하는 안(331건)은 채택하지 않았다.** `ETC`·`DOT`가 영어 일반어(`etc.`·`dot`)와 겹쳐 단어 경계 처리 없이는 새 오탐이 생긴다. 필요해지면 별도로 다룬다.
