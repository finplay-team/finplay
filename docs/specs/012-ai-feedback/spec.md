...
| 이름 | 범위 | 쓰는 곳 |
|---|---|---|
| `전장` | `[D-1 마지막 분봉, D 09:00]` | 브리핑, `PRE_MARKET` 요약, 시가 갭 근거 |
| `FULL` | `[D-1 마지막 분봉, D 마지막 분봉]` | `FULL` 요약 |
| `ROLLING_24H` | 최근 24시간 | 코인 요약·브리핑·카드 목록 |
| `근거창(주식 장중)` | `[windowEnd − 30분, windowEnd + 5분]` | 장중 카드 근거 |
| `근거창(코인)` | `[windowEnd − 35분, windowEnd]` | 코인 카드 근거 |
...
| 09:00 ~ 마지막 분봉 | `PRE_MARKET` | `전장` | `[D-1 마지막 분봉, 현재 재생 시각]` |
| 마지막 분봉 이후 | `FULL` | `FULL` | `FULL` |
...
**"15:30"·"09:00 시가" 같은 리터럴을 쓰지 않는다.** `003-market-data`가 분봉 timestamp를 **구간 시작 기준**으로 다루므로(`StockReplayService.findRevealedCandle`이 첫 분봉 09:00을 09:00~09:00:59 동안 "마감 전"으로 취급) **그날 마지막 분봉은 15:29일 가능성이 높다.** `15:30 종가`를 찾는 코드는 **항상 `null`을 받고 예외는 안 난다.** 반드시 "그 거래일의 마지막 분봉"으로 표현한다. 게이트 시각(장 마감 판정)은 `15:30` 벽시계 그대로 쓴다 — 이건 분봉 존재 여부와 무관하다.
...
**`D` 접수 공시는 `FULL`에서만 나온다** — 마지막 분봉 이후이므로 장중 유출이 성립하지 않는다. §노출 판정의 `clamp`는 **`D-1` 접수 공시에만** 적용된다. 뉴스는 분 단위 발행시각이 있으므로 구간 필터를 그대로 쓴다.
...
`origin_trade_date`를 `NULL`로 두지 않는 이유는 **일일 상한을 셀 컬럼이 필요하기 때문**이다. 코인 카드의 중복을 실제로 막는 것은 유니크가 아니라 쿨다운·일일 상한이다(`occurred_at`이 분마다 달라 유니크에 걸리지 않는다). 응답에서는 코인의 `originTradeDate`를 `null`로 내린다.
...
- [ ] 주식 16종목과 코인 12종목 뉴스를 `market_news_items`에 저장한다. 공시는 주식만 해당한다.
...
- [ ] 코인 12종목의 롤링 수익률을 감시해 임계치를 넘으면 카드를 생성한다 (§탐지 알고리즘(코인)).
...
리터럴을 쓰지 않는 근거는 **§C-2**에 있다. "없으면 생략(오류 아님)" 규칙 때문에 리터럴로 찾으면 **시가 갭 카드가 매일 영구히 0건이 되고 로그조차 남지 않는다.**
...
```
소유     market. CryptoPriceSnapshotService 가 PriceStore 를 감싼다 (§C-6)
...
**시가 갭 카드에는 더하지 않는다.** 첫 분봉의 시가는 개장 시점부터 가격 API로 정당하게 공개되므로 앞서지 않는다. 오히려 `+1분`을 더하면 첫 분봉 시각이 09:01인 경우 `revealTime`이 09:02가 되어 개장 직후에 갭 카드가 비는데, 갭 카드는 개장과 동시에 필요한 산출물이다(§C-6의 생성 순서).
...
**매도 후 흐름의 노출 게이트가 중요하다.** 14:40에 매도하고 14:41에 조회하면 장 마감까지의 가격은 아직 재생되지 않은 미래다. 그걸 보여주면 사용자가 같은 종목을 재매수할 때 답을 아는 상태가 된다. `now() >= (그 매도 체결이 일어난 서비스 날짜의) 15:30`일 때만 채운다. **"오늘 15:30"이 아니다** — 오늘로 잡으면 어제 판 체결을 오늘 오전에 열었을 때 `READY`였던 값이 `NOT_YET`으로 되돌아간다.
...
[반사실]  같은 수량을 다른 가격에 팔았다면
  ...
  시나리오 3개
    atClose             P = 원본 거래일 "마지막 분봉"의 close (§C-2)
...
- [ ] **첫 분봉이 09:00이 아니어도(09:01) 시가 갭 카드가 생성된다** — 리터럴에 의존하면 매일 0건이 된다(§C-2).
...
- [ ] **자정을 넘긴 카드(`occurred_at` 00:03)의 `windowStart <= windowEnd`가 유지되고 `originTradeDate`가 그날 KST 날짜다** (§C-9).
...
- [ ] **시가 갭 카드에는 `+1분`이 붙지 않는다** — 첫 분봉이 09:01인 픽스처에서 `revealTime`이 09:01이어야 한다.
...
- [ ] **Part C 요약이 장중에 장중 기사를 언급하지 않는다** — 오후 기사를 픽스처에 넣고 11:00에 조회해 `summaryScope="PRE_MARKET"`이며 요약에 그 내용이 없음을 확인한다. 마지막 분봉 이후 다시 조회하면 `FULL`로 바뀐다.
...
- [ ] 코인 요약·브리핑이 UPSERT로 하루 1행을 유지한다.
...
- [ ] **`atClose`·`closePrice`가 그 거래일 "마지막 분봉"의 close다** — `15:30` 리터럴로 찾으면 항상 `null`이 된다(§C-2).
...
- [ ] `판단·훈수` 표현(`버티`·`놓치`·`았다면`)이 매도 회고에서 템플릿으로 대체된다.
