...
**시가 갭에서는 두 정렬이 같은 순서다** — 후보 뉴스가 전부 이벤트(`D 09:00`) 이전이라 "이벤트에 가까운 순"과 "최신순"이 일치한다. 그래서 이 변경으로 달라지는 것은 **공시가 살아남는가 하나뿐**이고 뉴스가 실리는 차례는 그대로다. 장중 카드는 근거창이 이벤트 앞뒤 양방향이라 두 정렬이 실제로 갈리며, 공시도 섞이지 않으므로 거리순을 그대로 쓴다.
