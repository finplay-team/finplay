...
`spec.md`·`plan.md`·`tasks.md`는 착수 시점의 계획을 담은 기록이며, 그 기능이 출시된 뒤 다른 이슈로 변경·축소·제거돼도 **소급해서 갱신하지 않는다.** 지금 무엇이 실제로 동작하는지는 `docs/prd.md` §3 "구현 현황"과 `docs/api-contracts.md`가 정본이다 — ADR을 수정 대신 새 번호로 대체(superseded)하는 것과 같은 이유다(CLAUDE.md 규칙 2).

## 번호 규칙 — 유일성과 충돌 회피 (2026-08-10 신설)

- **번호는 저장소 전체에서 유일하다.** PRD·`api-routes.md`·`api-contracts.md`·다른 spec은 `016`·`026`처럼 **번호만으로** spec을 지목하므로(예: `docs/api-routes.md`의 근거 칸), 같은 번호가 둘이면 그 참조가 어느 문서를 가리키는지 판별할 수 없게 된다.
- **새 spec을 만들기 직전에 `git ls-tree origin/dev docs/specs/`로 다음 빈 번호를 확인한다.** 로컬 `ls`만 보면 병렬 브랜치가 선점한 번호를 놓친다 — Flyway 마이그레이션 번호와 같은 종류의 선점 충돌이며 `docs/agent-mistakes.md`에 동형 사례가 있다(ADR-0004).
