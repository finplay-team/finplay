...
| 2026-08-17 | implementer | 매도 이후 관찰 배제 필터 2곳 제거 + api-contracts 동기화 (Gradle 미실행, 검증은 메인 세션 위임) | 026 spec.md "매도 여부와 무관하게" 원칙을 031이 상속, 이슈 #420 프로덕션 재현 |
...
- 2026-08-17 — 이슈 #420: 매도 체결 이후 관찰을 evidence에서 배제하던 필터를 복기 서비스와 진행 조회에서 제거해, 매도 후 evidence를 채운 사용자의 영구 409를 해소하고 현재 run 귀속(risk snapshot 시각) 판정만 남김.
