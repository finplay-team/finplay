...
**구현**: `resolveToParam`이 KST→UTC 변환 뒤 **1초를 더해** 빗썸에 전달하도록 수정. 빗썸의 최소 봉 간격이 1분(`1m`)이라 1초 보정으로는 다음 봉을 끌어올 위험이 없고, `interval`별 분기 없이 네 값(`1m`·`1d`·`1w`·`1M`) 모두 같은 로직으로 처리된다.

**재검증 스모크(2026-08-03 KST, `market=KRW-BTC`, `count=2~3`, curl 직접 호출)**:
...
네 interval 모두 +1초 보정 후 경계 봉이 포함되고, 그다음 봉(예: `1d`에서 `2026-08-04`)이 끌려오지 않음을 확인했다 — 빗썸 최소 간격(1분)이 보정값(1초)보다 훨씬 크기 때문에 예상대로 안전했다.

**문서 동기화**: `docs/api-contracts.md` 코인 캔들 전용 규칙 절의 "`to`는 경계 시각을 배제한다" 문구를 "`to`는 항상 포함(inclusive)이다"로 갱신(같은 커밋). 단위 테스트(`BithumbRestCandleProviderTest`)에 `to` 쿼리파라미터 검증값을 +1초 반영, 일·주·월봉 경계 포함 회귀 테스트 2개 추가.
