...
## 이슈 #157 조치 — `to`를 inclusive로 통일 (2026-08-03 KST)

**계약 결정**: 우리 API의 `to`는 항상 포함(inclusive)으로 통일한다. 클라이언트가 경계를 보정하게 하는 대신, `BithumbRestCandleProvider.resolveToParam`이 흡수한다. `interval=1m`도 같은 원칙으로 함께 고친다(이슈에서 지적한 대로 계약 일관성을 위해 필요).

**구현(최초)**: `resolveToParam`이 KST→UTC 변환 뒤 **1초를 더해** 빗썸에 전달하도록 수정.

**재검증 스모크(2026-08-03 KST, `market=KRW-BTC`, `count=2~3`, curl 직접 호출, KST 리터럴 값 그대로 전달)**:

| interval | 원래 경계(`to`=봉 시작 시각, 문제 재현) | +1초 보정 후 |
|---|---|---|
| `1m` | `to=2026-08-03 13:27:00` → 최신 봉 `13:26:00`(당분 빠짐) | `to=2026-08-03 13:27:01` → 최신 봉 `13:27:00`(포함) |
| `1d` | `to=2026-08-03 00:00:00` → 최신 봉 `2026-08-02T00:00:00`(오늘 빠짐) | `to=2026-08-03 00:00:01` → 최신 봉 `2026-08-03T00:00:00`(포함) |
| `1w` | `to=2026-08-03 00:00:00` → 최신 봉 `2026-07-27T00:00:00`(이번 주 빠짐) | `to=2026-08-03 00:00:01` → 최신 봉 `2026-08-03T00:00:00`(포함) |
| `1M` | `to=2026-08-01 00:00:00` → 최신 봉 `2026-07-01T00:00:00`(이번 달 빠짐) | `to=2026-08-01 00:00:01` → 최신 봉 `2026-08-01T00:00:00`(포함) |

**PR #164 리뷰(pcb2002) 차단 대응 — KST→UTC 변환 자체가 실제 빗썸 동작과 다름을 발견**:

리뷰어가 `resolveToParam`이 실제로 만드는 값(KST→UTC 변환 후 +1초, 예: `2026-08-02 15:00:01`)으로 다시 curl 검증한 결과, 위 표의 검증(KST 리터럴 값을 그대로 보낸 것)과 **다른 요청**이었고 오늘 봉이 여전히 빠지는 것을 확인했다.

```
# resolveToParam이 실제로 보내던 값 (KST→UTC 변환 + 1초)
GET .../candles/days?market=KRW-BTC&count=2&to=2026-08-02 15:00:01
→ 최신 봉 2026-08-02T00:00:00 (오늘(8/3) 봉 여전히 빠짐 — 수정 목적 달성 실패)

# KST 값 그대로 +1초만 (변환 없이)
GET .../candles/days?market=KRW-BTC&count=2&to=2026-08-03 00:00:01
→ 최신 봉 2026-08-03T00:00:00 (오늘 봉 포함 — 의도한 동작)
```

**결론**: 빗썸의 `to`는 문서·기존 코드 주석의 전제("UTC 기준")와 달리 **`candle_date_time_kst`와 그대로 비교되는 KST 값**이다. 이 오해는 013(MKT-008, `1m` 경로) 최초 구현부터 있었을 가능성이 있다 — `to`가 지정된 모든 기존 조회가 KST 기준 9시간 밀린 엉뚱한 구간을 반환했을 수 있다는 뜻이다(1분봉으로 교차 검증: `to=2026-08-03 13:27:00`(KST 리터럴)은 정확히 13:26까지 반환하지만, 기존 코드가 실제로 보내던 UTC 변환값 `2026-08-03 04:27:00`은 KST 04:26까지의 완전히 다른 구간을 반환).

**수정**: `resolveToParam`에서 KST→UTC 변환을 제거하고, 원본 KST `LocalDateTime`에 그대로 `+1초`만 적용해 전달하도록 변경. 미사용이 된 `ZoneId`·`ZoneOffset` import와 `KST` 상수도 제거. 위 표의 값들은 KST 리터럴을 그대로 보낸 결과이므로 수정된 코드의 실제 출력과 일치한다(재확인 완료 — 수정 후에도 동일한 curl 결과).

**문서 동기화**: `docs/api-contracts.md` 코인 캔들 전용 규칙 절의 "`to`는 경계 시각을 배제한다" 문구를 "`to`는 항상 포함(inclusive)이다"로 갱신하고, 내부 구현 메모에 KST→UTC 오해 정정 내용 반영(같은 커밋). 단위 테스트(`BithumbRestCandleProviderTest`)의 `to` 쿼리파라미터 검증값을 KST 리터럴+1초 기준으로 재수정, 일·주·월봉 경계 포함 회귀 테스트 2개 유지.
