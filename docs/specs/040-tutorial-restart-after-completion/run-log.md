...
| implementer | Gradle 미실행(오케스트레이터가 순차 단독 실행) | 이슈 #433 — spec.md 비즈니스 규칙 경계 조건, plan.md "소스 변경 대상과 근거" #2 정정, ADR-0003 |

## 모니터링 (사람용 요약)
- 리뷰 완료, 차단 0건 — 보상 중복 지급 방지·소급 판정·동시성 잠금 순서 확인, 머지 가능.
- 이슈 #433: V32 샌드박스 종목 도입 이전에 실제 종목으로 완료한 legacy 완료자가 `POST .../restart`에서 항상 409 `PRACTICE_EVIDENCE_MISSING`을 받던 문제 수정. 진입이 실제 종목을 심은 `COMPLETED` replay attempt를 만들어 주는데 정리 서비스가 샌드박스 샘플만 허용하기 때문이었다. 실제 자산 보호 방어선은 그대로 두고, 재시작 서비스가 legacy replay일 때만 종목을 넘기지 않도록 했다. 회귀 테스트 3종(단위 2·통합 1) 추가.
