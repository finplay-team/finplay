...
- [ ] `VerificationCodeHasher`(생성자에서 시크릿을 받는 final 클래스)·`VerificationCodePolicy`(`@Component`, 5분 만료·60초 재발송·1시간 5회·하루 10회·최대 5회 시도 상수 + `generateCode`·`expiresAt`·`isAttemptLimitReached`) 도입과 세 서비스 치환 — 각 서비스의 `@Value` 생성자 파라미터를 그대로 두어 `EMAIL_VERIFICATION_SECRET`(가입 인증·이메일 변경) / `PASSWORD_RESET_SECRET`(재설정) 소유 관계를 바꾸지 않고, 한쪽 시크릿으로 만든 코드가 다른 쪽 경로에서 통하지 않는 가드 테스트를 추가한다
