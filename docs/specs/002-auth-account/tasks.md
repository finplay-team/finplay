...
- [ ] `EmailChangeVerificationRepository.findFirstByUserIdAndNewEmailOrderByCreatedAtDesc`에 같은 잠금 적용 — 같은 "수정 전 실패 실측 → 수정 → 통과" 절차를 밟고, 같은 커밋에서 `EmailChangeConfirmIntegrationTest`가 잠금 쿼리를 트랜잭션 밖에서 직접 호출하는 6곳(152·164·185·201·247·287행)을 jdbcTemplate 헬퍼로 교정한다(read-only 트랜잭션에서 `SELECT ... FOR UPDATE`가 거부되기 때문). 생성 SQL에 `users` 조인이 생기지 않는지 함께 확인한다. `validateAndConsumeCode`에 `@Transactional`을 새로 붙이지 않는다
