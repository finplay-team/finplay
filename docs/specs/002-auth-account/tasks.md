...
- [ ] `checkSendRateLimit`(집계 함수만 인자로 받아 60초 `> 0` → 1시간 `>= 5` → 하루 `>= 10` 순서와 비교 연산자를 그대로 이관)과 `expirePreviousCodes`(`ExpirableVerification` 인터페이스 + `expireAll`, 조회 쿼리는 세 서비스에 그대로 남김) 공통화 — 발송 제한 429 경계 3종과 재발송 시 이전 코드 무효화가 세 도메인에서 불변임을 확인한다
