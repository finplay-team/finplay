...
| 15:05 | implementer(PR #387 권장사항) | `./gradlew compileTestJava && ./gradlew test --tests "...StockReplayHoldFallbackIntegrationTest"` | PR #387 리뷰 권장사항(getRevealedAggregatedCandles 1d 폴백 미검증), ADR-0003(핵심 시나리오 Testcontainers) |
...
- 15:05 — `StockReplayHoldFallbackIntegrationTest`에 `weekendAggregatedDailyCandleFallbackReturnsFridaySessionFromRealMySql` 1건 추가(기존 시딩 재사용, 신규 시딩 없음). 토요일에 interval=1d 폴백 집계가 실 MySQL에서 금요일 OHLC·거래량을 정상 반환함을 확인, 3/3 통과.
