...
>
> **차수: 2차 MVP(1차 고도화) — 3단계 투자 실습의 활성 트랙 (2026-08-05 확정)**. 주식 튜토리얼(`016`)의 나머지 완성(세션 귀속·15:30 자동 만료를 포함한 OCO·진행조회·관찰·복기)은 3차 MVP(2차 고도화)로 이동했다. `docs/prd.md` §2·§3 참고.
...
이 spec은 코인 시장 실습을 독립적으로 완결되는 튜토리얼로 확정하고, 2026-08-05 결정으로 2차 MVP(1차 고도화)의 활성 트랙이 된다 — 주식의 세션 특화 마무리보다 코인 실습 완결을 먼저 production한다. 주식 규칙을 수정하지 않고 코인 경로만 정의하며, 세션 경계가 없는 24시간 시장에서 튜토리얼 identity·GTC plan 수명·관찰 증거·소수 수량 비교를 확정한다. 016의 공통 규칙(클라이언트 완료 주장 불허, 서버 evidence 연결, 보상·LLM 미사용)은 그대로 상속한다.

production 구현은 이 이슈 범위가 아니다 — `019`(#199)와 같은 문서 설계 전용 spec이다.
