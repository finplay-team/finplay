...
- [ ] nullable `trades.stock_replay_session_id`와 주식 fill session 기록, 공통 reservation ledger, 기존 MARKET SELL `availableQuantity` 변경을 OCO보다 먼저 또는 같은 atomic release로 배포한 뒤 tutorial-only OCO 생성 orchestration을 활성화한다. (candidate 5/#186의 FK·주식 fill session 기록까지 완료했으며 reservation ledger와 MARKET SELL 변경, OCO 생성 orchestration은 미완료) 일반 LIMIT SELL도 ledger 선행 전 활성화하지 않으며 order는 education repository에 직접 의존하지 않는다.
