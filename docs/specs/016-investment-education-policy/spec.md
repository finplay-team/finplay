...
>
> **차수 재분류 (2026-08-05)**: 이미 구현된 candidate 1~5(즐겨찾기·사전 의도·합성 시세·주식 체결 세션 FK)는 2차 MVP 완료로 유지된다. **나머지 미착수 production(세션 귀속·15:30 자동 만료를 포함한 OCO·진행조회·관찰·복기의 주식 완성)은 3차 MVP(2차 고도화)로 이동했다.** 2차 MVP(1차 고도화)의 3단계 실습 활성 트랙은 코인이며 `docs/specs/020-coin-practice-tutorial`(이슈 #222)이 그 정본이다. `docs/prd.md` §2·§3 참고.
