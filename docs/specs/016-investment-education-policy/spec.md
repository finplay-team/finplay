...
- 사용자는 매수 전에 `instrumentId`, `quantity`와 PRICE 방식의 `stopLoss`·`takeProfit` 또는 PERCENT 방식의 `stopLossRate`·`takeProfitRate`를 실습 의도로 기록한다. 기존 타입 생략+가격 요청은 PRICE로 호환하며 상세 tagged union은 `docs/specs/019-exit-price-policy`를 따른다.
