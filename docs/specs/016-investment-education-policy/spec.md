...
>
> **차수 재분류 (2026-08-06 재확정)**: 이미 구현된 candidate 1~5(즐겨찾기·사전 의도·합성 시세·주식 체결 세션 FK)는 2차 MVP 완료로 유지된다. **이 문서가 정의한 OCO 기반 2·3단계(진행조회·관찰·복기 포함)는 코인·주식 구분 없이 3차 MVP(2차 고도화)로 이동했다.** 2026-08-05에는 코인 경로(`020-coin-practice-tutorial`)를 2차 MVP 활성 트랙으로 우선 production하기로 했었으나, 2026-08-06 결정으로 철회했다 — 튜토리얼 OCO(코인·주식 모두)와 일반 리스크관리 OCO(`021-general-risk-management-oco`)를 함께 3차 MVP에서 하나의 엔진으로 착수하기로 확정했기 때문이다. 코인 시장 실습의 설계 정본은 여전히 `docs/specs/020-coin-practice-tutorial`(이슈 #222)이며, 착수 시점만 3차로 미뤄졌다. `docs/prd.md` §2·§3·§4 참고.
>
> **2차 MVP 활성 경로는 `docs/specs/026-market-order-practice-tutorial`이다 (2026-08-06 신설)**. OCO 없이 이미 구현된 시장가/지정가 매매만으로 2·3단계를 완결시키며, 이 문서의 1단계(즐겨찾기)·사전 의도 기록 API(`POST /api/education/practice/intentions`)와 공통 판정 원칙(클라이언트 완료 주장 불허 등)을 변경 없이 그대로 재사용한다. **지금 이 문서의 2·3단계·candidate 6 이후를 구현하러 왔다면 바로 위 3차 재분류 문단 때문에 대상이 아니다 — `026`을 봐야 한다.**
