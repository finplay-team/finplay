...
| `GET /api/education/practice/oco?market=` | `market` 필수(`STOCK|CRYPTO`) | 200 `InvestmentPracticeResponse` | 선택 시장의 OCO 전용 단일 튜토리얼 조회, pagination·write 없음 | 400 `VALIDATION_ERROR`; 인증 공통 오류 |
...
| `POST /api/education/practice/oco/intentions` | 기존 intention과 같은 body | 201 `PracticeIntentionResponse` | 시장별 OCO 전용 progress와 내부 tutorial key가 귀속된 intention 생성; holding 기반 intention과 ID 공간은 공유하되 key로 구분 | 기존 intention과 같은 오류 계약, 단 완료 판정은 OCO 전용 key 기준 |
...
2. key miss일 때만 education application이 사용자 단위 in-process 락에서 요청 `intentionId`를 현재 OCO intention과 내부 `intentionInstanceKey`로 해석한다. holding 기반 intention이거나 exit plan 시장의 OCO tutorial key와 다르거나 찾지 못하면 409 `PRACTICE_EVIDENCE_MISSING`이며 새 mapping을 만들지 않는다.
...
- `InvestmentPracticeResponse(String tutorialKey, String status, Integer currentStep, List<PracticeStepResponse> steps, LocalDateTime completedAt)`: OCO 조회의 `tutorialKey`는 필수 `market`에 따라 `INVESTMENT_OCO_PRACTICE_V1|COIN_OCO_PRACTICE_V1`; `status` 허용값은 `NOT_STARTED|IN_PROGRESS|COMPLETED`; `currentStep`은 완료 시 null, 그 외 1~3; `completedAt`은 완료 전 null. `steps`는 항상 1,2,3 순서의 세 항목이다.
...
`GET /api/education/practice/oco?market=STOCK|CRYPTO`는 선택한 시장의 OCO 전용 key(`INVESTMENT_OCO_PRACTICE_V1|COIN_OCO_PRACTICE_V1`)를 아래 표대로 계산한다. `unlocked`는 `locked=false`를 뜻한다. locked 단계의 `PracticeEvidenceResponse`는 객체 자체는 non-null이고 모든 필드가 null이다. unlocked 단계는 현재까지 실제로 확인된 리소스만 위 DTO 채움 규칙대로 반환한다.
...
- `practice_intentions`(인메모리): `PracticeIntentionRepository`(`@Repository`, JPA 아님) 내부 `ConcurrentHashMap<Long userId, List<IntentionRecord>>`(또는 `CopyOnWriteArrayList` 값) 형태로 사용자별로 여러 건 누적한다. `PracticeIntentionService`는 이 repository를 통해서만 저장소에 접근해 ADR-0002의 레이어 경계를 유지한다. 순수 Java record에 공통 식별자·수량·시각, API 비노출 `tutorialKey`와 `exitPriceType`, PRICE에서만 non-null인 `stopLoss`·`takeProfit`, PERCENT에서만 non-null인 `stopLossRate`·`takeProfitRate`를 표현한다. JPA 엔티티·테이블·Flyway migration 대상이 아니며 삭제 경로가 없어 사용자가 반복 등록할수록 리스트가 무한히 누적된다(ADR-0012 결과 참고).
...
OCO 복기는 요청 exit plan의 market으로 OCO tutorial key를 결정해 그 `practice_progresses` 행을 잠그고, reflection·completion 저장과 같은 key의 progress 완료 전이를 한 트랜잭션에서 처리한다. holding 기반 progress/completion은 조회하거나 변경하지 않는다.

> Issue #193(ADR-0012) 이후 `favorite`·`practice_intention`은 DB 행이 아니므로 `SELECT ... FOR UPDATE` 비관 잠금을 걸 수 없다. "favorite 잠금"·"intention 잠금"은 사용자 단위 in-process 잠금을 뜻하며, 다른 도메인 서비스는 `FavoriteService.withFavoriteLock(...)` 같은 명시적 service 메서드로만 빌려 쓰고 락 객체나 repository를 직접 참조하지 않는다(ADR-0002의 "다른 도메인 repository·내부 구현 직접 참조 금지" 원칙을 락에도 동일 적용). 유스케이스별 정본 순서는 intention 생성·복기에서 `progress(DB) → favorite 락(in-memory) → 나머지 자원`, OCO 신규 생성에서 `favorite 락 → intention 락 → replay session/holding/plan(DB)`이다. key hit OCO 멱등 재현은 영속 mapping·plan만 읽어 in-memory 락이 필요 없다. 획득한 in-memory 락은 해당 DB 작업의 커밋·롤백 이후 finally에서 해제하며, JVM 내부에서만 유효해 다중 인스턴스 경계를 넘지 못한다.
...
- intention 생성: 요청 경로(holding/OCO)와 종목 market으로 tutorial key를 먼저 결정하고 `(user_id, tutorial_key)` progress를 무변경 MySQL 원자 upsert로 확보해 같은 DB 트랜잭션에서 `SELECT ... FOR UPDATE`로 잠근 뒤 대상 `(userId,instrumentId)`의 favorite in-memory 락을 획득한다. favorite 존재·종목을 검증하고 intention에 내부 tutorial key를 함께 저장한 뒤 DB 트랜잭션을 커밋하고 마지막에 favorite 락을 해제한다. 완료 판정은 선택된 key만 보므로 holding 완료가 OCO 생성을 막거나 그 반대가 되지 않는다. favorite 없음·불일치는 `PRACTICE_STEP_LOCKED`이며 intention을 남기지 않는다.
...
12. **OCO 실습 진행 조회** — `GET /api/education/practice/oco?market=`, 실제 OCO 증거·별도 불변 완료 기반 순수 조회. 후보 2·4·7·8 선행.
