...
- **AI 피드백의 뉴스·공시 수집과 개장 전 브리핑(`012`)의 샌드박스 종목 제외** — 이 spec의 범위가 아니었고
  **검토 후 제외한 것도 아니다.** 이 spec은 사용자 계정에 귀속되는 산출물(보유·회고·랭킹·손익)만 다뤘는데,
  수집·브리핑은 계정과 무관한 시장 단위 배치라 같은 축으로 잡히지 않았다. 그 결과 샌드박스 종목이 실사명
  (`알파전자` 등, `V33`)으로 외부 뉴스 검색에 나가 전 회원 공유 브리핑에 실리는 경로가 남아 있었다 —
  **이슈 #406에서 별도로 닫았다.**
  - 그 이슈는 수집 대상에서 제외하는 것과 **시장 단위 조회 질의에 `tutorialSample = false`를 거는 것**을
    함께 했다. 이미 저장된 오염 행을 지우는 마이그레이션은 두지 않았는데, 근거는 **질의 조건이 영구적이라
    남은 행이 어느 시점에도 결과에 들어오지 않는다**는 것이다(시간이 지나 구간 밖으로 밀려나서가 아니다 —
    그 근거였다면 보관 기간이나 구간이 바뀌는 순간 깨진다).
  - **단 이미 생성된 브리핑 행은 예외다.** `MarketBriefingService.generateStockBriefing`은 그 거래일
    브리핑이 이미 있으면 건너뛰므로(중복 방지), 배포 시점에 그날 브리핑이 이미 만들어져 있었다면 그
    **산출물 안의 오염된 문장은 다음 거래일로 넘어가야 해소된다.** 위 질의 필터는 앞으로 만들어질 브리핑에만
    걸린다 (PR #411 리뷰 [권장 2]).
