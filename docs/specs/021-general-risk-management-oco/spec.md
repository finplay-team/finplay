...
>
> `docs/prd.md`는 손절·익절 OCO를 2차 MVP(1차 고도화)에서 "튜토리얼 전용"으로 명시하고, intention 없는 일반 리스크관리 OCO는 3차 MVP 후보로 분리해 두었다(§2). 이 spec은 그 3차 MVP 후보를 앞당겨, 교육 튜토리얼에 종속되지 않는 일반 기능으로 처음 설계한다. `POST /api/exit-plans` 등 관련 production 코드는 아직 전혀 없다 — 이 문서는 리팩터링이 아니라 최초 설계다.
...
- [ ] `docs/prd.md`와의 차수 불일치(§2·§4의 "3차 MVP 후보" 문구, §3 구현 현황)가 후속 동기화 대상으로 tasks.md에 남는다.
