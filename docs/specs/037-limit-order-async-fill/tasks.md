...
- [ ] `LimitOrderFillExecutorProperties`(+`Config`) 추가 — `order.limit-fill-executor.*`(enabled/partition-count/queue-capacity-per-partition/batch-size), 단위 테스트 + yml 바인딩 테스트
- [ ] `LimitOrderFillExecutorRouter` 추가 — 종목별 파티션 라우팅·백프레셔 드롭, 단위 테스트
- [ ] `LimitOrderFillService.fillBatch(List<Long>)` 추가 — 청크 원자적 처리, spec 033 샌드박스 로직 보존, 단위 테스트
