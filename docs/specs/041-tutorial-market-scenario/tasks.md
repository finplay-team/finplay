...
| 1b | 042 | `SNAP-1b` — snapshot 단건 조회를 최신 진입 기준으로 교체(호출 7곳) | **제약만 풀고 쿼리를 두면 재진입 직후 500이 난다.** `SNAP-2`보다 먼저 |
...
- [ ] **4. 진행 계산 서비스** — plan §상태 전이표 전 항목. 특히 **대기 구간에서 매수 시 다음 진행 구간
  ...
  `act == ACT2`에서만 `IDLE_REENTRY` 이동, `MAX_TICK_GAP = 30초` clamp, 초 단위 누적, 봉 3값 갱신,
  **건너뛴 가상 분마다 순차 정산**(SCENARIO-013).
  **테스트**: 시각 주입 단위 테스트 — 전이표 각 행, 2초 간격 tick에서도 드리프트 없음,
  tick 간격을 가상 10분으로 벌려도 루머 손절이 재현됨.

- [ ] **5. tick 통합과 시간 제한 폐지** — `POST .../tick`이 진행 계산을 호출하고, `GET .../chart`는
  진행을 갱신하지 않는 순수 조회를 유지한다. 시간 기반 만료 판정을 제거하고 `saleDeadlineAt`을 V2에서
  `null`로 내린다(V1·legacy는 기존 값 유지). `docs/api-contracts.md`를 **같은 커밋에서** 갱신한다.
  **테스트**: 통합 — 대기 구간에서 아무리 오래 있어도 만료되지 않음, 2막 손절 후 재진입 대기로 이동,
  3막 익절 후에도 4막이 재생됨.
...
- [ ] **7. 통합 시나리오와 문서** — Testcontainers 통합 테스트로 **0막 대기 → 매수 → 1막 → 2막 손절 →
  ...
  확정에서 손절되는 분기도 함께 확인. `docs/api-routes.md` 최종 확인, `docs/prd.md` §3에 `SCENARIO-001~024` 행 추가 + **SANDBOX 행의 5분 만료
