...
- 이슈 #22 항목 3(`OrderService.createOrder` 멱등성 오케스트레이션) 구현 완료: plan.md 확정 코드 그대로 `createOrder`/`findReplayResponse` 적용(`@Transactional` 없음, 해시 불일치 즉시 409, 동시성 폴백 catch 재조회 실패 시 방어적 409). `OrderServiceTest`에 재요청 재현·해시불일치 409·신규키 execute 위임·동시성 폴백 성공/실패 5케이스 추가(기존 `getMyOrders` 2케이스 유지, 총 7케이스). 응답 mock 금지 컨벤션 준수해 `OrderResponse`는 실제 레코드로 생성. `com.finplay.api.order.*` 전체(통합 테스트 포함) 통과, compileJava/compileTestJava 통과.
- 이슈 #22 항목 4(통합 테스트) 구현 완료: 신규 `OrderIdempotencyIntegrationTest`(4케이스 — 매수/매도 재요청 동일응답·무추가행, 다른 본문 409, 서로 다른 사용자 무간섭) 작성, `OrderResponse` 비교는 DB 재조회 시 BigDecimal scale이 달라져(`quantity`/`price`) record `equals` 대신 필드별(`isEqualByComparingTo` 포함) 비교로 처리. `./gradlew build` 전체(Spotless·SpotBugs·JaCoCo 포함) 통과 확인(2회 연속 그린). 중간에 한 차례 `InstrumentRepositoryTest`/`StockReplaySessionRepositoryTest`가 무관한 이유로 실패했으나, 부분집합 재실행으로 신규 테스트와 무관한 기존 정적 Testcontainers 공유 컨테이너 + 하드코딩된 동일 날짜(2026-07-29) 간 교차 오염에 의한 기존 테스트 스위트의 순서 의존 플레이키니스임을 확인(재실행 시 재현 안 됨, agent-mistakes.md에 기록할 만한 확정 재현은 아니라 별도 기록 안 함 — 후속 조치로 spawn_task 등록).
- 리뷰(코드리뷰 모드, 이슈 #22 재검토, HEAD 75cf170): 차단 0건 — 트랜잭션 경계(OrderExecutionService.execute만 @Transactional 소유, findReplayResponse 개별 조회는 order+trade가 같은 트랜잭션에서 커밋되므로 안전), self-invocation 회피를 위한 클래스 분리 근거 타당, 해시 불일치 시 즉시 409(검증·체결 미실행, verifyNoInteractions로 테스트 확인), OrderExecutionService 리팩터링은 순수 이동(diff 대조로 로직 변경 없음 확인), ADR-0002 위반 없음, 문서(api-contracts.md/api-routes.md) "#22에서 구현 예정" 문구 정확히 갱신 확인. 권장 1건 — OrderService.createOrder의 `catch (DataIntegrityViolationException)`이 uk_orders_user_idempotency 위반인지 구분하지 않고 로깅 없이 전부 409 IDEMPOTENCY_CONFLICT로 매핑, 향후 다른 제약 위반이 이 경로를 타면 원인이 가려질 수 있음 — 로그 추가 권장.
