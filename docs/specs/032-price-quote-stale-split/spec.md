...
이 spec은 "체결을 막을지"와 "화면에 보여줄지"를 서로 다른 질문으로 분리한다. **연결이 끊겼거나 시세를 한 번도 받은 적이 없으면** 화면도 체결도 똑같이 막는다(보여줄 가격 자체가 없다). 반면 **연결은 살아있는데 최신 틱만 10초를 넘겼으면**, 화면은 마지막으로 받은 진짜 가격을 "지연됨" 표시와 함께 계속 보여준다(이 spec 당시엔 체결만 계속 막았으나, `034-crypto-price-rest-backup`에서 체결도 같은 규칙으로 합쳐졌다).
...
- 회원이 그 코인을 실제로 매수·매도 주문하면, 연결이 살아있고 마지막 가격을 받은 적 있으므로 최신 틱이 10초를 넘겨도(stale) 그 가격으로 주문이 체결된다. 화면의 지연 표시는 "이 가격이 오래됐다"는 사실만 알려줄 뿐 매매 가능 여부와는 무관하다.
...
- [x] **`PracticeHoldingObservationService`(실습 3단계 가격 관찰 근거값)** — 완화됨(의도적 승계, 프로덕션 코드 변경 없음). 이 서비스는 세션 귀속 체결이 없을 때 `PriceQueryService.getPrice(instrumentId)`를 그대로 호출한다(PRICE-STALE-001의 그 메서드). `getPrice`가 완화되므로 이 서비스도 자동으로 완화를 물려받는다 — 관찰은 학생이 스스로 화면에서 본 가격을 근거로 판정 로직을 채점하는 것이라 실제 최신값(비록 10초 넘게 지연됐더라도)이 고정 fallback보다 더 정확한 근거다. **(PR #360 리뷰 차단 사항 후속)** 이 서비스는 다른 3개 소비자(`HoldingValuationService`·`SyntheticPriceService`·`PracticePriceSessionService`)와 달리 `quote.status()`를 읽는 조건식 자체가 없어 "완화됨"이라는 이 문장이 코드로 확인되지 않는다는 리뷰 지적을 받았다 — `docs/api-contracts.md`(`holding-observations` 절)를 실제 동작에 맞게 갱신하고 `PracticeHoldingObservationServiceTest.createObservationUsesLastKnownPriceWhenCryptoPriceIsStale`로 STALE 가격이 그대로 관찰 근거로 쓰이는 것을 회귀 테스트로 고정했다.
