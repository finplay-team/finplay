...
- 병행 spec/plan: `../041-tutorial-market-scenario` — 대본과 포지션 앵커. 이 plan의 프리셋 수치는
  041 plan §프리셋 도달 조건 검증의 여섯 부등식으로 정당화된다.
...
**나머지 두 값의 근거는 041 대본과의 도달 조건이다.** 041 plan §프리셋 도달 조건 검증이 세 프리셋 전부에
대해 여섯 부등식을 확인했다. 요약하면 이렇다.

- `t`의 하한은 **1막에서 익절이 터지면 안 된다**가 정한다. 1막 고점이 배율 1.020이고 진입가가 최저 1.000
  이므로 `1 + t > 1.020`, 즉 `t > 2%`다. `CAUTIOUS`의 3%가 이 하한 바로 위다.
- `t`의 상한은 **3막에서 익절이 터져야 한다**가 정한다. 3막 고점 0.990, 재진입가 최대 0.905이므로
  `0.905 × (1+t) < 0.990`, 즉 `t < 9.4%`다. `RELAXED`의 8%가 이 상한 안이다.
- `s`의 상한은 **4막에서 손절이 터져야 한다**가 정한다. 4막 저점 0.840, 재진입가 최소 0.895이므로
  `0.895 × (1−s) > 0.840`, 즉 `s < 6.1%`다. `RELAXED`의 5%가 이 안이다.
- `s`의 하한은 실질적으로 없다(2막 저점 0.900이 어떤 손절선보다도 아래다). 2%는 "조심스럽게"가 체감되는
  최소값으로 잡았다.

즉 **세 값은 임의로 고른 것이 아니라 대본이 허용하는 구간의 양 끝과 가운데다.** 대본을 손보면 이 구간이
움직이므로, 041 plan의 여섯 부등식 테스트가 두 문서를 함께 잠근다.
...
새 Flyway 번호는 **V47**이다(041이 V46을 쓴다). 머지 직전에 `git ls-tree origin/dev --
src/main/resources/db/migration`으로 재확인한다(이슈 #394). **041과 이 PR의 머지 순서가 뒤집히면 번호를
서로 바꿔야 한다** — CI의 번호 역전 검사가 base 이동을 놓칠 수 있으므로 머지 버튼 직전에 본다.
...
- **1차 배포 (V47)** — `entry_sequence` 컬럼을 `NOT NULL DEFAULT 1`로 추가하고, 새 UNIQUE
...
- **2차 배포 (V48)** — 기존 `uk_practice_risk_snapshots_attempt_run`을 삭제한다. 이 배포부터 재진입
  snapshot 생성이 열린다.

**따라서 재진입 재예약(EXITPRESET-017)은 2차 배포에서 활성화된다.** 1차 배포만 나간 상태에서 재진입
매수가 일어나면 제약 위반으로 실패하므로, **041의 재진입 흐름과 이 기능의 배포 순서를 맞춰야 한다** —
041의 재진입 이동이 먼저 나가고 이 기능의 2차 배포가 늦으면, 재진입한 사용자의 매수가 깨진다.
...
| `exitPresetLocked` | `boolean` | 최초 BUY 체결 후 true |
...
| 최초 BUY 체결 후 프리셋 변경 | 409 `PRACTICE_STEP_LOCKED` |
...
| 프리셋 수치 ↔ 041 대본 도달 조건 | 단위 테스트. 041 plan의 여섯 부등식을 대본 파일과 프리셋 상수로 판정 |
...
| 재진입 재예약 | Testcontainers 통합. **2차 배포(V48) 이후에만 통과한다** |
...
2. V47 마이그레이션(컬럼 5개 + 새 UNIQUE 추가) + 엔티티
...
8. **(별도 배포)** V48 — 기존 UNIQUE 삭제 → 재진입 재예약 활성화 + 통합 테스트
...
**041과의 순서**: 5·6번이 041의 tick 변경과 같은 자리를 건드리므로 한쪽으로 몰아 순차 처리한다. 그리고
041의 재진입 이동이 배포되기 전에 8번이 끝나 있어야 한다 — 순서가 뒤집히면 재진입한 사용자의 매수가
UNIQUE 위반으로 실패한다.

## 열린 질문 — 이 plan에서 정하지 않은 것

- **재매수 시 프리셋 재선택 허용 여부**(spec "확인 필요"). 이 plan은 EXITPRESET-017대로 **같은 프리셋으로
  다시 걸리는 것**만 설계했다. 손절을 겪은 사용자가 기준을 다시 고르게 하는 것이 교육적으로 더 나을 수
  있으나, EXITPRESET-003("체결 후 변경 금지")과 어떻게 구분할지 — 즉 "이미 확정된 snapshot은 못 바꾸되
  다음 진입의 기준은 바꿀 수 있다"를 사용자가 헷갈리지 않게 표현할 수 있는지 — 가 정해지지 않았다.
  **결정이 필요하다.** 허용하기로 하면 `exit_preset` 잠금 조건이 "최초 BUY 이후"에서 "현재 보유 중"으로
  바뀐다.
...
- **2단계 배포 사이의 창.** V47만 나가고 V48 전인 상태에서 041의 재진입 흐름이 배포되면 재진입 매수가
  UNIQUE 위반으로 실패한다. 두 spec의 배포 순서를 사람이 챙겨야 하는 지점이며, 이 plan에서 자동으로
  막을 방법이 없다. **tasks.md에 배포 순서를 명시하고 PR 본문에도 적는다.**
...
- **CAUTIOUS를 고른 사용자의 익절 후 추가 상승 7.4%p.** 041 plan의 잔여 위험과 같은 항목이다. "조심스럽게
  골랐더니 덜 벌었다"가 트레이드오프로 읽히지 않고 손해로 읽히면, 프리셋 선택 자체가 부정적으로
  학습된다. 완화는 화면 문구이며 서버가 할 수 있는 것은 사실 제공까지다.
