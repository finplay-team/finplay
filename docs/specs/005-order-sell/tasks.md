...
- [x] **1. 엔티티·Repository 확장 지점 (order·account·portfolio 도메인)**
...
- [x] **2. `PortfolioSellService` — FIFO 배분·실현손익 원가 계산 (portfolio 도메인)**
...
- [x] **3. `OrderService` — SELL 분기 통합 (createBuyOrder → createOrder)**
...
- [x] **4. `OrderController` + `OrderResponse` — 응답에 실현손익 노출**
