...
| POST | `/api/education/practice/limit-orders` | Access Bearer 필수. `{"practicePriceSessionId":1,"instrumentId":1,"quantity":0.01,"limitPrice":9500}`(`PracticeLimitOrderCreateRequest`) | 201 `LimitOrderResponse`(side는 서버가 `BUY`로 고정, PENDING 상태) | 400 `VALIDATION_ERROR`(누락·수량·가격 형식); 401 `UNAUTHORIZED`; 404 `NOT_FOUND`(세션·종목 없음 또는 타인 세션, 존재 은닉); 409 `INSTRUMENT_NOT_TRADABLE`, `PRACTICE_PRICE_SESSION_CLOSED`(COMPLETED 세션), `PRACTICE_PRICE_SESSION_MISMATCH`(요청 종목이 세션과 불일치), `PRACTICE_LIMIT_ORDER_ALREADY_PENDING`(세션에 PENDING 교육 주문 이미 존재), 같은 종목에 049 시나리오 attempt가 있고 대본을 쓰는 실행이면 시장가 왕복 미완료 시 `PRACTICE_STAGE_LOCKED`(Issue #507, `LIMIT` 주문 공통 게이트), 현금 부족은 샌드박스 종목이면 `TUTORIAL_INSUFFICIENT_CASH`, 실제 종목(030 코인 연습 세션이 BTC·ETH 등을 다루는 경우)이면 `INSUFFICIENT_CASH`(047 이후, `PracticeLimitOrderCreationService.createSessionBuyOrder`가 `instrument.isTutorialSample()`로 분기) 등 기존 지정가 주문 오류 |
...
**(이슈 #432) 이 응답에는 `prompt` 필드가 없다.** 예전에는 요청과 무관한 고정 문구를 항상 같은 값으로 실어 보냈는데, 프론트가 그 값을 렌더링하지 않고 자체 문구를 쓰고 있어 사용자에게 닿는 경로가 없었다 — 문구 하나 고치자고 백엔드를 배포해야 하는 비용만 남아 폐기했다. 질문 문구는 이제 클라이언트가 소유한다. 저장된 복기가 어느 세대 질문에 답한 것인지는 `practice_market_reflections.prompt_version`에 그대로 영속되며, 버전 1이 가리키는 **원문은 `PracticeHoldingReflectionService.PROMPT_VERSION` 주석에 남겨 두었다** — 서버가 원본을 잃지 않으면서 API 표면에서만 뺀 것이다. 앞의 `016` 계획 절(`POST /api/education/practice/reflections`)의 OCO 복기는 사용자가 exit plan을 직접 계획하는 별도 경로라, **3차 MVP에서 구현될 때도** `prompt` 필드를 그대로 간다 — 두 경로를 한꺼번에 정리하지 않는다(현재는 미구현이라 `PracticeReflectionResponse` 구현체가 없다). **문구를 바꾸는 프론트 PR은 `PracticeHoldingReflectionService.PROMPT_VERSION`도 함께 올려야 한다** — 소유권이 클라이언트로 넘어간 뒤 서버에는 버전을 올릴 계기가 없어서, 그대로 두면 `prompt_version` 컬럼이 영구히 1로 굳는다.
...
| POST | `/api/education/practice/attempts/{market}/advance-script` | Access Bearer 필수. path `market=STOCK|CRYPTO`, body 없음 | 200 `PracticeAttemptResponse`. attempt를 먼저 비관 잠근 뒤(잠금 순서는 기존 경로와 같이 attempt → 튜토리얼 계좌), **2단계(주문 방법 학습) 대본을 마친 실행을 같은 run 안에서 3단계(041 이야기) 대본으로 전환한다.** `runNumber`·튜토리얼 계좌 현금·`exitPreset`은 바뀌지 않는다(전환은 재시작이 아니다) — 재시작(`runNumber+1`)은 `tutorialStageProgress` 세 값을 전부 되돌려 전환 직후 그 자격을 지우므로 쓸 수 없다. 통과하면 현재 run의 PENDING 지정가 주문과 PENDING OCO 예약을 정리하고, `scenario_script_id`를 `CRYPTO_STORY_V1`로 교체한 뒤 대본 커서(구간 id·경과 초·진행 중 봉 3값·진행 기준 시각)를 전부 `null`로 되돌려 다음 tick이 041 대본의 첫 구간 0분으로 새로 연다 | 400 `VALIDATION_ERROR`(미지원 market); 401 `UNAUTHORIZED`; 409 `PRACTICE_STEP_LOCKED`(attempt 없음), `PRACTICE_ALREADY_COMPLETED`(attempt `COMPLETED`), `PRACTICE_STAGE_LOCKED`(대본을 쓰지 않는 실행이거나 이미 3단계 대본, 시장가·지정가 왕복 미완료, 현재 run 순보유수량 > 0) | 049 ORDERBASICS-018~021, Issue #507 |
| GET | `/api/education/practice/attempts/{market}/chart` | Access Bearer 필수. path `market=STOCK|CRYPTO`, body 없음 | 200 `PracticeTutorialChartResponse`: `attemptId`, `runNumber`, `instrumentId`, `virtualDateTime`, `secondsPerVirtualMinute=3`, 시간순 30개 `candles`. ACTIVE는 조회 시각을 사용하고 `COMPLETED/REPLAY`는 `completedAt`을 관측 시각으로 고정해 이후 clock 진행에도 chart·virtualDateTime이 변하지 않는다. 주문·attempt·예약을 변경하지 않는 순수 조회다. **(041 5번, Issue #472) 생성기 버전 2 attempt는 마지막 봉의 종가가 저장된 대본 커서의 가격이고 시·고·저는 attempt에 누적된 값이다** — 이 조회는 커서를 전진시키지 않으므로 `POST .../tick` 없이 새로고침만 반복하면 같은 값이 계속 나온다. 과거 29개 완결 일봉은 대본 대상이 아니라 버전 1과 같은 seed 생성이다. `virtualDateTime`은 두 버전 모두 `anchorAt` 기준 벽시계 파생이라 대본 진행이 clamp된 만큼 커서보다 앞설 수 있다. **(041 6번, Issue #488) 응답에 `scenarioStage`·`scenarioProgressing`·`causeStatus`·`revealedEvents` 4필드가 더해졌다** — `scenarioStage`는 `IDLE_ENTRY`\|`ACT1`\|`ACT2`\|`IDLE_REENTRY`\|`ACT3`\|`ACT4`\|`FINISHED`\|`ORDER_BASICS`로 **대본 내부 구간이 아니라 act 단위**이며 `FINISHED`가 대본 종료 알림이다(별도 완료 필드를 두지 않는다). **(049, Issue #507) `ORDER_BASICS`는 2단계 대본(주문 방법 학습)의 유일한 구간이다** — `PracticeScenarioNarrativeCalculator`가 `act`가 `null`이면 구간 id를 그대로 라벨로 내보내는데, 이 대본은 사건 0개·대기 구간 없이 진행 구간 하나(`id: ORDER_BASICS`, `act: null`)뿐이라 이 값이 그대로 나간다. `scenarioProgressing`은 현재 구간이 진행 구간인지이고 **보유 여부와 무관하다**(미보유로 4막을 관전 중이어도 `true`). `causeStatus`는 `REVEALED`\|`NONE_KNOWN` **둘뿐이며 미공개 사건이 있는 구간도 `NONE_KNOWN`이라 두 경우를 구분할 수 없다**(SCENARIO-015·016). 판정은 막이 아니라 **대본 구간** 단위라 2막-b 속임수 반등은 앞 구간 루머가 공개된 뒤에도 `NONE_KNOWN`이다. `revealedEvents`는 `{stage, headline}`의 배열로 **공개 시점이 지난 사건만** 담고 공개 순서(오래된 것 → 최근)다 — **시각을 담지 않는다**(사건 공개는 대본 커서가, `virtualDateTime`은 벽시계가 정해 두 시계가 어긋나므로 화면은 순서만 보고 "방금"·"조금 전"으로 그린다). 대본을 쓰지 않는 attempt(생성기 버전 1·완료 replay)는 앞 3필드가 `null`이고 `revealedEvents`가 빈 배열이다. **(049 ORDERBASICS-009~011, Issue #507) `priceGuideRange`가 더해졌다** — `{low, high}` 형태로 그 대본이 안내하는 가격 변동 범위이며, 극값(대본 배율의 최소·최대 × 기준가)에서 폭의 5%만큼 안쪽으로 물린 뒤 안내 단위로 안쪽 반올림한 값이다(2단계 대본은 `90000~110000`). **사건이 하나라도 있는 대본(041 이야기)은 `null`이다** — 판정식은 `script.events().isEmpty()` 하나이며, 041의 4막 폭락 극값을 미리 알려주면 사건 공개 게이트(SCENARIO-015·020)를 뚫기 때문이다. 대본을 쓰지 않는 attempt도 `null`이다 | 400 `VALIDATION_ERROR`; 401 `UNAUTHORIZED`; 409 `PRACTICE_STEP_LOCKED`(attempt 없음 또는 종목 미선택) | 039 TUTORIAL-FLOW-005·009·010·011·012, Issue #378; 041 SCENARIO-002·015·016·022, Issue #472·#488; 049 ORDERBASICS-009~011, Issue #507 |
| POST | `/api/education/practice/attempts/{market}/tick` | Access Bearer 필수. path `market=STOCK|CRYPTO`, body 없음 | 200 `PracticeTutorialChartResponse`(GET chart와 동일). attempt를 비관 잠근 뒤 같은 run의 PENDING 지정가 주문을 ID 순서로 조회하고 현재 canonical close로 조건 판정·체결하며, 응답 차트와 체결가는 같은 서버 관측 시각을 쓴다. 조건 미충족 주문은 PENDING과 예약을 유지한다. **(041 5번, Issue #472) 생성기 버전 2 attempt는 이 호출이 대본 커서를 전진시키는 유일한 지점이다** — 직전 tick 이후 경과를 최대 30초로 clamp해 소비하고, 그 사이 지나간 **가상 분마다 순차로** 지정가 조건을 판정한다(tick 종점 가격 하나로 판정하지 않는다). 대기 구간에서 보유가 생기면 시간을 소비하지 않고 다음 진행 구간 0분으로 이동하며 남은 경과를 매수 체결 시각 기준으로 자른다. 매도는 커서를 옮기지 않는다. 마지막 구간 끝에서는 진행을 멈추고 마지막 가격을 유지하며 오류를 던지지 않는다 — **다만 정산은 계속한다**(새 가상 분에 진입하지 않은 tick도 현재 커서 가격으로 지정가를 1회 판정하므로, 대본이 끝난 뒤 접수한 지정가도 조건을 만족하면 체결된다). **(041 6번, Issue #488) 응답 4필드는 GET chart와 같다** — 커서를 민 뒤의 값이라 tick 응답이 사건 공개의 실시간 창구다. **(049, Issue #507) `priceGuideRange`도 GET chart와 같은 규칙**이며 tick이 커서를 밀어도 값이 바뀌지 않는다(그 대본 전체에 대한 고정 안내값이라 진행과 무관) | 400 `VALIDATION_ERROR`(미지원 market); 401 `UNAUTHORIZED`; 409 `PRACTICE_STEP_LOCKED`(attempt 없음·종목 미선택·현재 run 불일치), `PRACTICE_ALREADY_COMPLETED`(완료 replay 쓰기 차단) | 039 TUTORIAL-FLOW-010·011·012, Issue #378; 041 SCENARIO-008·010·013·014·015·016, Issue #472·#488; 049 ORDERBASICS-009~011, Issue #507 |
| PUT | `/api/education/practice/attempts/{market}/exit-preset` | Access Bearer 필수. path `market=STOCK|CRYPTO`, `{"preset":"CAUTIOUS"}`(`PracticeAttemptExitPresetUpdateRequest`, `@NotNull ExitPreset`) | 200 `PracticeAttemptResponse`. **(052) 아래 `exit-rates`로 대체됐다 — 프론트가 별도 배포라 같은 배포에서 없애면 깨지므로 남겨 뒀고, 프론트 전환이 끝나면 지운다. 지금은 `exit-rates`와 같은 서비스 메서드에 위임하므로 동작과 거부 판정이 완전히 같다.** attempt를 비관 잠근 뒤 현재 실행 세대의 손절·익절 프리셋을 확정한다. **잠금 기준은 "최초 매수 여부"가 아니라 "지금 보유 중인가"다**(042 EXITPRESET-003) — 매수 전과 손절·익절·수동 매도로 포지션을 정리한 뒤(재진입 대기)에는 몇 번이든 바꿀 수 있고, 보유 중에만 막힌다. **이미 확정된 snapshot과 그 snapshot으로 만들어진 예약은 바뀌지 않으며 다음 진입에만 적용된다.** `PUT`인 이유는 자연 멱등이기 때문이며 `Idempotency-Key`를 요구하지 않는다. **(이슈 #502)** 응답의 `tutorialCashBalance`·`tutorialAvailableCash`·`tutorialRealizedPnl`은 같은 트랜잭션에서 읽은 실제 값이다. **(049 ORDERBASICS-015, Issue #507)** 대본을 쓰는 CRYPTO 실행은 시장가·지정가 왕복을 둘 다 마쳐야 프리셋을 고를 수 있다 — 이 판정이 보유 중 잠금보다 **앞선다** | 400 `VALIDATION_ERROR`(정의 밖 값·누락·미지원 market); 401 `UNAUTHORIZED`; 409 `PRACTICE_STAGE_LOCKED`(대본을 쓰는 실행에서 시장가·지정가 왕복 미완료), `PRACTICE_STEP_LOCKED`(attempt 없음·보유 중), `PRACTICE_ALREADY_COMPLETED` | 042 EXITPRESET-003, Issue #477; Issue #502; 049 ORDERBASICS-015, Issue #507 |
| PUT | `/api/education/practice/attempts/{market}/exit-rates` | Access Bearer 필수. path `market=STOCK|CRYPTO`, `{"stopLossRate":3.0,"takeProfitRate":5.0}`(`PracticeAttemptExitRatesUpdateRequest`) | 200 `PracticeAttemptResponse`. **(052) 프리셋 3개(택1)를 대체하는 자유 입력이다** — 실전 화면처럼 손절 비율과 익절 비율을 서로 **독립적으로** 정한다. 두 값은 **퍼센트 수**이고(3%는 `3`) **손절도 양수로 받아** 서버가 −로 해석한다(042 `ExitPreset`과 같은 단위·부호 규칙). 허용 구간은 `stopLossRate` ∈ **[2, 5]**, `takeProfitRate` ∈ **[3, 8]**로 양 끝을 **포함**하며 소수 **첫째 자리까지**다(`3.05`는 물론 값이 같은 `3.00`도 거부된다 — 표기된 scale로 판정한다). **프리셋 3개 조합만 허용하는 검증은 없다** — 손절 5 + 익절 3 같은 조합도 유효하다. **거부 판정은 `exit-preset`과 한 글자도 다르지 않다**(같은 서비스 메서드에 위임한다) — 특히 **보유 중에는 거부한다**: 042 EXITPRESET-003의 취지(손실 중에 손절선을 슬금슬금 내리는 사후 합리화 차단)를 그대로 승계하며, 매수 전과 포지션을 정리한 뒤(재진입 대기)에는 몇 번이든 바꿀 수 있다. 이미 확정된 snapshot과 그 snapshot으로 만들어진 예약은 바뀌지 않고 다음 진입에만 적용된다. **(052 EXITFREE-020으로 정정) 대본을 쓰는 실행에서는 여기서 정한 비율이 자동 예약을 만들지 않는다** — 매수 체결 시 만들어지는 것은 위험 기준선(참조선)뿐이고, 실제 OCO 예약은 사용자가 `POST .../exit-plan`으로 직접 건다. 대본을 쓰지 않는 실행(생성기 버전 1·legacy)만 042 그대로 자동 예약이 생긴다. 응답의 튜토리얼 잔액 3필드는 `exit-preset`과 같이 실값이다 | 400 `VALIDATION_ERROR`(범위 밖·소수 둘째 자리 이하·누락·미지원 market); 401 `UNAUTHORIZED`; 409 `PRACTICE_STAGE_LOCKED`(대본을 쓰는 실행에서 시장가·지정가 왕복 미완료), `PRACTICE_STEP_LOCKED`(attempt 없음·보유 중), `PRACTICE_ALREADY_COMPLETED` | 052 EXITRATES-001~003 |
| POST | `/api/education/practice/attempts/{market}/exit-plan` | Access Bearer 필수. path `market=STOCK|CRYPTO`, `{"stopLossRate":3.0,"takeProfitRate":5.0}`(`PracticeAttemptExitRatesUpdateRequest` **재사용** — 구간·소수 자릿수·양수 규칙이 `exit-rates`와 같아야 해서 검증을 복제하지 않았다) | **201 `ExitPlanResponse`**(`docs/api/order.md`의 021 절과 같은 DTO). **(052 EXITFREE-020) 사용자가 직접 거는 예약이다** — 042는 매수 체결 순간 서버가 자동으로 걸었지만(EXITPRESET-004·012), 그러면 사용자가 걸 것이 남지 않아 이 튜토리얼이 가르치려는 행위 자체가 사라진다. **대본을 쓰는 실행에서는 자동 예약이 더 이상 생기지 않는다.** attempt를 비관 잠근 뒤 **현재 실행 세대의 보유 전량**을, **그 진입의 체결가**(`practice_risk_snapshots.entry_price`, scale 8로 이미 정규화된 값) 기준으로 019 공식대로 계산해 예약한다. **진입당 정확히 1회(write-once)** — 취소한 뒤 같은 진입에서 다시 만드는 것도 거부한다(그것이 EXITPRESET-003이 막은 "손절선 슬금슬금 내리기"를 다른 문으로 되살리기 때문이다). 재매수로 새 진입이 열리면 그 진입 몫으로 다시 한 번 열린다. **단계 게이트(049)를 다시 검사하지 않는다** — 이 경로는 3단계 대본 실행에서만 열리고 그 대본으로 넘어가는 길(`POST .../advance-script`)이 이미 왕복 두 번을 요구하므로 대본 식별자가 곧 게이트다. **취소는 새 경로를 만들지 않고 `DELETE /api/exit-plans/{exitPlanId}`가 그대로 받는다**(경로 공통, 021) | 400 `VALIDATION_ERROR`(범위 밖·소수 둘째 자리 이하·누락·미지원 market); 401 `UNAUTHORIZED`; 409 `EXIT_PLAN_ALREADY_EXISTS`(이 진입에 이미 예약을 만들었다 — PENDING이든 취소·체결된 것이든), `PRACTICE_STEP_LOCKED`(attempt 없음·진행 중 아님·**보유 없음**·대본을 쓰지 않는 실행), `PRACTICE_STAGE_LOCKED`(2단계 대본 실행 — 049 ORDERBASICS-022가 그 대본에 예약을 두지 않기로 했다), `PRACTICE_ALREADY_COMPLETED`, `EXIT_PLAN_INVALID_PRICE_RANGE`(계산된 기준선이 `0 < stop < entry < take`를 깨거나 `DECIMAL(18,8)` 초과), `INSUFFICIENT_QTY`(예약 가능 수량이 부족 — 지정가 매도가 이미 수량을 잡고 있을 때) | 052 EXITFREE-020 |
...
`PracticeAttemptResponse` 필드 순서는 `attemptId`, `market`, `runNumber`, `mode`, `status`, nullable `instrumentId`, nullable `anchorAt`, nullable `tutorialDate`, nullable `riskSnapshot`, nullable `completedAt`, `tutorialCashBalance`, `tutorialAvailableCash`, `tutorialRealizedPnl`, `selectedExitPreset`, `exitPresetLocked`, `availableExitPresets`, **(052)** `exitStopLossRate`, `exitTakeProfitRate`, `exitRateBounds`이다.

**(052, 이슈 052) 적용될 손절·익절 비율의 정본이 프리셋 이름이 아니라 `exitStopLossRate`·`exitTakeProfitRate`가 됐다.** 둘 다 퍼센트 수이며(3%는 `3`) 손절도 양수이고, **어떤 경로에서도 `null`이 아니다** — 아무것도 고르지 않은 실행에는 기본값 손절 `3`·익절 `5`가 실린다(042 EXITPRESET-002 승계, 즉 미선택 사용자의 결과는 052로 달라지지 않는다). `exitRateBounds`는 `{"stopLossMin":2,"stopLossMax":5,"takeProfitMin":3,"takeProfitMax":8}`이며 **양 끝을 포함**한다 — 화면이 구간을 하드코딩하지 않도록 서버가 내려보낸다(042 EXITPRESET-009와 같은 이유). **같은 `exitRateBounds`가 `GET /api/education/practice` 응답 루트에도 실린다** — 진행 조회는 루트에서, 쓰기 경로 네 곳은 `PracticeAttemptResponse`만 돌려주므로 그 안에서 읽는다. `selectedExitPreset`은 **052부터 파생값이자 nullable이다** — 적용될 비율이 프리셋 3개 중 하나와 **정확히 같을 때만** 그 식별자가 들어가고 자유 조합(예: 손절 `5` + 익절 `3`)이면 `null`이다. 열거형 밖의 값(`CUSTOM` 등)은 **오지 않는다**. 미선택 실행은 기본값 3·5가 `BALANCED`와 같아 지금까지와 똑같이 `"BALANCED"`를 받는다. `selectedExitPreset`·`availableExitPresets`는 프론트가 프리셋 픽커를 떼면 함께 없앤다. `exitPresetLocked`는 **이름·의미 모두 그대로**다("지금 보유 중인가") — 자유 입력에도 같은 잠금이 적용된다. **(042 EXITPRESET-001·002·003, Issue #477)** `selectedExitPreset`은 현재 실행 세대의 선택값이며 **미선택이면 `null`이 아니라 `"BALANCED"`(기본 프리셋)를 내려보낸다** — 클라이언트가 null 분기를 갖지 않고, 화면에 보이는 값과 실제로 적용될 값이 같아야 하기 때문이다. `exitPresetLocked`는 **현재 보유 중이면 `true`**이고 재진입 대기 중에는 `false`로 돌아온다(완료 attempt는 `mode=REPLAY`로 이미 구분된다). `availableExitPresets`는 `preset`·`stopLossRate`·`takeProfitRate` 세 필드를 가진 고정 3개 배열이며 **비율은 퍼센트 수다**(3%는 `3`). **금액 병기는 클라이언트가 계산한다** — 서버가 계산하려면 "고른 수량"을 먼저 받아야 해서 왕복이 하나 늘고, 초보자를 멈추게 하지 않는 것이 이 기능의 전제다(EXITPRESET-006). 표시 이름(조심스럽게·보통·느긋하게)은 서버가 주지 않는다. 쓰기 가능한 미완료 실행은 `mode=ACTIVE`, 완료 실행은 읽기 전용 `mode=REPLAY`이며 현재 attempt API가 반환하는 상태는 `SELECTING_INSTRUMENT|IN_PROGRESS|COMPLETED`다. 도메인 열거형의 `EXPIRED`는 향후 영속 전이를 위한 예약 값이며 현재 만료는 진행 조회의 4단계 상태로만 계산한다. `riskSnapshot`은 `entryPrice`, `stopLossPrice`, `takeProfitPrice`, `buyTradeId`, `createdAt`과 **(042, Issue #477)** `exitPreset`·`stopLossRate`·`takeProfitRate`·`entrySequence`를 반환하고 seed·generator version은 노출하지 않는다. 비율은 퍼센트 수다(3%는 `3`). `entrySequence`는 그 실행 세대의 몇 번째 진입인지이며 손절 후 재매수하면 2가 된다. **기능 도입 전에 만들어진 스냅샷은 `exit_preset`이 `NULL`인데 기본 프리셋(`BALANCED`)으로 해석해 내려보낸다** — 화면이 "기준을 모르는 진입"을 그리지 않게 하기 위해서다. **(052)** `riskSnapshot.exitPreset`도 같은 규칙으로 **nullable이 됐다**(자유 조합 진입) — `stopLossRate`·`takeProfitRate`는 그래도 항상 채워지므로 화면은 그 두 값으로 기준을 그린다. **(047, TUTORIAL-CASH-ISOL-011)** `tutorialCashBalance`·`tutorialAvailableCash`(`=tutorialCashBalance-예약현금`)·`tutorialRealizedPnl`은 그 시장의 `TutorialAccount`(사용자·시장별 1000만원 초기 현금의 샌드박스 전용 계좌, 실제 `Account`와 별개) 현재 상태를 반환한다. **(이슈 #502)** `PracticeAttemptResponse`를 반환하는 **쓰기 경로 네 곳은 모두 실제 값을 채운다** — `PUT /api/education/practice/attempts/{market}`(진입)·`POST .../restart`(재시작)는 그 트랜잭션에서 이미 조회·리셋한 계좌를, `PUT .../instrument`(종목 선택)·`PUT .../exit-preset`(프리셋 선택)은 attempt를 잠근 같은 트랜잭션에서 계좌를 한 번 더 읽어 싣는다. **세 필드가 `0`으로 내려오는 곳은 `GET /api/education/practice`의 `attempt` 필드 하나뿐이다** — tick과 함께 폴링되는 경로라 호출마다 계좌를 다시 읽지 않는다. 계좌 자체는 진입 시점에 get-or-create되어 항상 존재하므로 그 `0`은 "잔고가 0"이 아니라 "이 응답은 계좌를 조회하지 않았다"는 뜻이며, 잔액이 필요한 화면은 위 네 응답이나 계좌 API를 쓴다.

동시 진입은 attempt 행을 `FOR UPDATE`로 먼저 잠그고 없을 때만 `INSERT ... ON DUPLICATE KEY UPDATE`로 만들며, `UNIQUE(user_id, market)`가 최종 방어선이라 하나의 attempt만 생긴다. reload는 run을 증가시키거나 선택을 지우지 않는다. **(이슈 #491) 같은 사용자·시장으로 진입이 동시에 들어와도 양쪽 다 정상 응답을 받는다** — 뒤에 온 요청이 앞선 요청을 기다렸다가 같은 attempt를 돌려받는다. 예전에는 순서가 반대라(무조건 `INSERT IGNORE` → 같은 행을 `FOR UPDATE`) 중복 키 검사가 유니크 인덱스 레코드에 남긴 공유 잠금(S)을 곧바로 배타 잠금(X)으로 승격했고, S는 여러 트랜잭션이 동시에 쥘 수 있으므로 동시 진입 2건이 서로를 기다리는 교착이 되어 한쪽이 500 `INTERNAL_ERROR`로 끝났다(그 탭의 튜토리얼이 로드되지 않았다). 이 트랜잭션은 갭 잠금을 쓰지 않도록 `READ COMMITTED`로 좁혀 선언한다 — 행이 아직 없는 첫 진입 2건이 각자 갭을 잡고 서로의 INSERT를 기다리는 별개의 교착을 막기 위해서다. 그래도 남는 교착은 **진입·재시작·tick 세 경로에서 각각 1회 재시도**하며, 재시도까지 실패해야 오류로 나간다 — 셋 다 실패하면 사용자에게 다른 재시도 수단이 없는 요청이다(종목 선택·프리셋 선택은 같은 버튼을 다시 누르면 되므로 제외했다). 재시도는 트랜잭션 밖에서 일어나며, 교착은 트랜잭션 전체가 롤백되므로 이중 반영이 없다.
...
- `entries` — **진입별 대조 배열**이며 진입 순번 오름차순이다. 각 항목은 `entrySequence`·`exitPreset`·**(052)** `stopLossRate`·`takeProfitRate`·`buyOrderType`·`buyAt`·`buyPrice`·`buyQuantity`·`stopLossPrice`·`takeProfitPrice`·`sellPrice`·`sellQuantity`·`sellAt`·`sellCause`·`realizedPnl`·`unrealizedPnlIfHeld`·`scenarioScriptId`다. **`realizedPnl`과 `unrealizedPnlIfHeld`는 `sellQuantity` 기준**이라 부분 매도한 진입에서는 `buyQuantity`와 다르다 — 두 금액을 전체 수량의 것으로 읽으면 안 된다. **왜 필요한가** — 042가 재진입을 열면서 한 실행 세대에 매도가 둘 이상 생겼는데 `tradeResult`의 매도 시각·`sellCause`는 **첫 매도** 기준이라, 2막 손절 → 3막 익절한 사용자의 완료 화면에 손절 하나만 뜬다(금액은 맞고 이야기가 틀린다). 이 배열이 그 결함을 닫는다. 진입 안에서는 실행 전체와 같은 규칙(첫 매도)을 진입 범위로 좁혀 쓰고, `sellPrice`는 수량 가중평균·`realizedPnl`은 합이라 부분 매도도 금액이 전부 반영된다. **대본 여부와 무관하게 채운다** — 재진입은 시장을 가리지 않는다. 매수 전이거나 attempt가 없는 legacy 경로는 빈 배열이다. **(이슈 #503)** `buyOrderType`은 그 진입을 **연** 매수의 주문 유형(`MARKET`\|`LIMIT`)이며 진입 경계인 위험 snapshot의 매수 체결이 가리키는 주문에서 읽는다 — **매도의 유형이 아니다**(시장가로 산 포지션을 지정가로 팔거나 예약이 청산할 수 있다). **`buyQuantity`·`buyPrice`와 기준이 다르다** — 그 둘은 진입 구간의 매수를 전부 합산한 값이라, 보유 중 추가 매수(새 진입을 만들지 않고 같은 진입에 합쳐진다)를 다른 유형으로 하면 `buyOrderType`은 첫 매수의 유형만 가리킨다. "이 수량을 어떤 방식으로 샀는가"로 읽으면 안 된다. **(049 ORDERBASICS-023, 이슈 #512 "5-A")** `scenarioScriptId`는 **이 진입이 열릴 때** attempt가 쓰던 대본 식별자(`CRYPTO_ORDER_BASICS_V1`\|`CRYPTO_STORY_V1`)이며 **진입별로 고정된다** — 2단계 → 3단계 전환(`advance-script`)은 같은 실행 세대 안에서 대본만 갈아끼우므로, 전환 전에 연 진입과 전환 후에 연 진입이 같은 `entries[]`에 섞이고 서로 다른 값을 가질 수 있다. 대본을 쓰지 않는 실행(생성기 버전 1·legacy)은 `null`이다. 049 이전에 만들어진 진입(스냅샷 컬럼 도입 전)은 `CRYPTO_STORY_V1`로 해석한다. **(052)** `exitPreset`은 그 진입에 적용된 비율이 프리셋 3개 중 하나와 정확히 같을 때만 그 식별자이고 자유 조합이면 **`null`**이다 — 완료 화면이 프리셋 이름 칩을 그린다면 `null`을 다뤄야 하며, 새로 더해진 `stopLossRate`·`takeProfitRate`(퍼센트 수, 양수, 항상 non-null)로 대신 그릴 수 있다.
...
| `exitPresetSelected` | 이 실행에서 손절·익절 기준을 **직접 정했는가**. **(052) 판정이 두 번 넓어졌다** — 1차에서 프리셋(`PUT .../exit-preset`)이든 자유 비율(`PUT .../exit-rates`)이든 한 번이라도 정하면 `true`가 됐고, **2차(EXITFREE-020)에서 `POST .../exit-plan`으로 예약을 만든 것도 근거로 센다** — 3단계 화면이 매수 폼의 비율 입력을 없애고 예약 요청 본문으로 비율을 보내게 되면서 `exit-rates`를 한 번도 부르지 않는 실행이 정상 경로가 됐기 때문이다. **예약은 취소·체결돼도 근거로 남는다**(행이 사라지지 않는다) — 취소로 이미 열린 단계가 되잠기면 안 된다. **대본을 쓰지 않는 실행(생성기 버전 1)의 예약은 세지 않는다** — 그쪽은 매수 체결이 서버가 자동으로 거는 예약이라 사용자가 아무것도 고르지 않아도 행이 생긴다. **필드 이름은 그대로 둔다**(프론트가 별도 배포라 같은 배포에서 개명하면 깨진다). 넓히지 않으면 프리셋 픽커가 화면에서 사라지는 순간 이 단계가 영영 미완으로 남는다 |
...
**(052 EXITFREE-020·021·022) 최상위에 예약 관련 세 값이 더해졌다.** 3단계 예약매도를 사용자가 직접 걸게 바뀌면서, 화면이 "지금 예약을 걸 수 있는가 / 이미 걸었는가 / 무엇을 겪었는가"를 그려야 한다.

```jsonc
{
  "exitPlanCreatable": true,          // POST .../exit-plan이 201을 줄 조건과 같은 산출식. 항상 boolean(null 아님)
  "pendingExitPlan": {                // 지금 걸려 있는 예약. **없으면 이 값 자체가 null**이다
    "exitPlanId": 41,                 // 취소(DELETE /api/exit-plans/{id})에 그대로 쓴다
    "stopLossRate": 3.0,              // 퍼센트 수, 손절도 양수
    "takeProfitRate": 5.0,
    "stopLossPrice": "9874.60000000", // 그 예약이 확정한 값. 뒤에 비율을 다시 정해도 움직이지 않는다
    "takeProfitPrice": "10689.00000000",
    "entryPrice": "10180.00000000",   // 그 진입의 체결가(기준)
    "quantity": "0.50000000",
    "reservedAt": "2026-08-21T10:00:01"
  },
  "exitExperience": {                 // 항상 non-null
    "stopLossExperienced": true,      // 이 실행 세대에서 FILLED_STOP_LOSS 예약이 있었는가
    "takeProfitExperienced": false,
    "bothExperienced": false,
    "recommendedNext": "TAKE_PROFIT"  // 먼저 겪은 쪽의 반대. 아무것도 안 겪었거나 둘 다 겪었으면 null
  }
}
```

**위 예시의 따옴표는 scale을 보이려는 표기이고 전송 형식이 아니다.** 가격·수량·비율은 전부 `BigDecimal`이고 이 저장소는 Jackson을 손대지 않으므로 **JSON 숫자**로 나간다(`"9874.60000000"`이 아니라 `9874.6`). 클라이언트가 문자열로 받는다고 가정하고 파싱을 넣을 필요가 없고, 반대로 문자열인 줄 알고 그대로 이어 붙이면 금액 표시가 깨진다. `PracticeAttemptControllerTest`가 `isNumber()`로 이 계약을 고정한다.

**판정 단위는 현재 실행 세대(run)다.** 재시작하면 run이 올라가 이전 세대의 예약 행이 조회 범위에서 통째로 빠지므로 **초기화 코드 없이** 세 값이 전부 초기 상태로 돌아간다 — 052 EXITFREE-021이 새 컬럼을 만들지 않기로 한 이유가 이것이다(041 잔여 위험 4의 "갱신·초기화 지점 누락"을 구조적으로 피한다).

**수동 매도는 겪음으로 세지 않는다.** 판정 근거는 예약의 최종 상태(`FILLED_STOP_LOSS`·`FILLED_TAKE_PROFIT`) 하나뿐이다 — 이 튜토리얼이 가르치려는 것은 "미리 정해 둔 규칙이 나 대신 실행된다"이며, 손해를 보고 직접 판 것은 그 경험이 아니다. `recommendedNext`의 값은 `STOP_LOSS`·`TAKE_PROFIT` 둘뿐이고 `entries[].sellCause`와 같은 어휘를 쓴다(`MANUAL`은 여기 나오지 않는다).

**서버는 판정만 하고 강제하지 않는다.** 화면 잠금·자동 재매수·주문 거부는 하지 않으며(052 확정 결정 1), 배너의 표현과 노출 위치는 프론트가 정한다. 이 트랙이 미완료라는 이유로 복기 저장·완료·보상이 막히지도 않는다(EXITFREE-024).

**(049 ORDERBASICS-015~017, Issue #507) 이제 판정뿐 아니라 강제한다.** 이 값이 최초로 정의됐을 때는(#503) 잘못된 순서의 주문을 409로 거부하는 게이트가 이 계약에 없었지만, 049가 그 게이트를 실제로 넣었다 — 대본을 쓰는 CRYPTO 실행에서 앞 단계(시장가 왕복)를 마치지 않고 지정가 주문(`POST /api/orders/limit`, `POST /api/education/practice/limit-orders`)을 내거나, 주문 방법 단계를 마치지 않고 프리셋(`PUT .../exit-preset`)을 고르면 409 `PRACTICE_STAGE_LOCKED`로 거부된다. 판정 근거는 이 절의 `tutorialStageProgress`와 같은 산출식(`PracticeStageProgressCalculationService`)이라 화면의 잠금과 서버의 거부가 어긋나지 않는다. **강제 대상은 대본을 쓰는 CRYPTO 실행뿐이다** — 생성기 버전 1 실행(STOCK 튜토리얼, 대본이 저작되지 않은 시장)과 시장가 주문(`POST /api/orders`)은 항상 통과한다. 거부 본문은 "무엇이 막혔는지"를 싣지 않는다 — 그 정보는 이미 `tutorialStageProgress`에 있다.
