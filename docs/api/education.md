...
동시 진입은 MySQL `INSERT ... ON DUPLICATE KEY UPDATE`와 `UNIQUE(user_id, market)`로 하나의 attempt만 만들며 reload는 run을 증가시키거나 선택을 지우지 않는다. **(이슈 #491)** 예전에는 `INSERT IGNORE`였는데, 중복 키 검사가 유니크 인덱스 레코드에 공유 잠금(S)을 남기고 곧바로 같은 레코드에 `FOR UPDATE`(X)를 요구해 같은 사용자·시장의 동시 진입 2건이 서로를 기다리는 잠금 승격 교착이 됐다 — 한쪽이 500 `INTERNAL_ERROR`로 끝나 그 탭의 튜토리얼이 로드되지 않았다. `ON DUPLICATE KEY UPDATE`는 같은 자리에서 배타 잠금(X)을 잡아 두 요청이 이 구문에서 직렬화되므로 **양쪽 모두 정상 응답을 받는다**(뒤에 온 요청이 앞선 요청을 기다렸다가 같은 attempt를 돌려받는다). 그래도 남는 교착(같은 트랜잭션이 함께 잠그는 튜토리얼 계좌·진행 행이 다른 경로와 경합하는 경우)은 진입 경로에서 **1회 재시도**하며, 재시도까지 실패해야 오류로 나간다. 종목 선택은 attempt 행을 비관 잠근 뒤 요청 시장과 일치하는 `tradable=true`, `is_tutorial_sample=true` 종목만 허용하고 선택 시각을 `anchorAt`, 그 날짜를 `tutorialDate`, 서버 생성 seed와 generator version을 같은 행에 저장한다. **(041 5번, Issue #472) 새로 시작하는 실행의 generator version은 대본이 저작된 시장에서만 2(저작 대본)다 — 현재 CRYPTO 하나뿐이며 STOCK은 대본이 저작되는 SCENARIO-024 전까지 계속 1이다.** 배포 시점에 이미 진행 중이던 버전 1 실행은 재시작을 강제하지 않고 그대로 버전 1로 재현하며, legacy completion만 있는 사용자에게 만들어지는 읽기 전용 replay attempt도 버전 1이다.
