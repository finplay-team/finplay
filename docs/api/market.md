...
market 도메인의 API 계약 상세다. 전체 라우트를 한눈에 보는 지도는 `ai/api-routes.md`에 있다.
...
- **주식 집계봉의 과거 구간은 이제 `stock_daily_candles`(3년 아카이브)를 우선 소스로 쓴다** — MKT-011(`050-stock-daily-archive`+`051-stock-daily-archive-chart-connection`, 이슈 #506, 2026-08-21 완료). 원천이 `stock_candles`(20영업일 롤링) 하나뿐이던 이전에는 `1d`가 최대 20개에서 끊겼지만, 이제 재생거래일 이전 과거 구간은 아카이브가 채우고(거래일 하나당 소스 하나, 아카이브 우선·1분봉은 아카이브 미도달 구간만 보충) 재생 중인 당일만 계속 `stock_candles` 컷오프 경로를 쓴다. `interval=1m`은 이 변경과 무관하다(이슈 #495 별도 결정 대상, 무변경).
