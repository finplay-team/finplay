...
ranking 도메인의 API 계약 상세다. 전체 라우트를 한눈에 보는 지도는 `ai/api-routes.md`에 있다.
...
**여기서 "매도 체결 이력"은 튜토리얼 샘플 종목(`instrument.isTutorialSample=true`) 매도를 포함하지 않는다.** 실제 종목은 한 번도 매도하지 않고 튜토리얼 샘플 종목만 매도해본 계좌는, 매도 이력이 아예 없는 계좌와 동일하게 이 목록에서 제외된다(spec 033 SANDBOX-EXCL-003·004, 이슈 #549). 이 문서 아래의 "내 랭킹 조회"가 쓰는 "매도 체결 이력"·"매도 이력"도 전부 같은 기준이다 — 여기서 한 번만 정의한다.
...
**매도 체결 이력이 없는 사용자는 `rank`만 `null`이다.** 오류 코드를 새로 만들지 않는다 — `nickname`·`realizedPnl`(0 포함)은 매도 이력과 무관하게 항상 정상 값으로 채워진다("랭킹 목록 대상에서 제외됨"(RANK-001)과 "본인 조회 응답에 참고 정보가 없음"은 다른 개념이다). `nickname`은 RANK-001과 동일하게 마스킹 없이 노출한다. "매도 체결 이력"의 정의(튜토리얼 샘플 종목 매도 불포함)는 위 "전체 랭킹 조회" 절 참고 — 튜토리얼로만 연습한 사용자도 `rank: null`이다.
