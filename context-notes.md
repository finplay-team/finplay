...
- **`docs/prd.md` §3은 갱신하지 않았다.** SANDBOX 행의 "5분 만료" 서술이 CRYPTO에서는 더 이상 사실이
  아니지만, `041 tasks.md` 7번이 그 갱신과 SCENARIO 행 신설을 함께 하도록 배정해 뒀다. **한 PR 동안 그
  표가 옛 서술로 남는다** — 7번에서 반드시 고친다.
