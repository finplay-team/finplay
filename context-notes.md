...
- **기획 문서 안에 충돌이 있었다**: Notion 1차 고도화 목록은 "뉴스를 통한 변동 원인"을 요구하는데, 같은 페이지의 운영 정책 → AI 피드백은 "외부 시장 정보(뉴스·공시) **미사용**"이라고 못박고 있었다. 레포 `docs/prd.md` C-004도 같은 취지였다. 사용자 결정으로 **조항을 개정하되 인과 단정 금지로 대체**했다 — "뉴스·공시는 시간적 동시 발생 서술에만 사용한다". C-004의 나머지("숫자·판정은 서버, AI는 서술만", "종목 추천 금지")는 그대로 살아 있고, 설계 전체가 그 제약 안에 들어간다. **Notion 쪽 운영 정책은 아직 안 고쳤다 — 팀 합의가 필요하고, 그때까지 팀 문서와 레포 문서가 어긋난 상태다.**
