...
| 2 | "개장했지만 첫 `revealTime` 전에 Part C는 `NOT_YET`" | `NOT_YET`은 09:00 이전에만 나온다 |
| 3 | "개장 후에는 두 값이 갈리지 않는다" | 09:05에 Part C `READY`+카드 `EMPTY`가 정상 |

셋 다 **"~뿐이다"·"~하지 않는다" 형태**다. 근거로 삼은 사례는 맞았는데 그 사례가 전부라고 단정한 자리에서
틀렸다. 2라운드는 리뷰어의 부정확한 지적을 그대로 반영한 탓도 있지만, **같은 문서 안의 다른 절(`api-contracts.md:805`)과
대조하지 않은 것은 내 몫이다.**
