...
**조회가 `@Transactional(readOnly = true)` 안에서 최대 300ms 대기해 커넥션을 쥔다.** Hikari 기본 풀 10이라 만료 경계에서 압박이 생길 수 있다. 고치려면 캐시 조회를 트랜잭션 밖으로 빼는 구조 변경이 필요해 이번 범위에서 하지 않았고 PR #257의 "남은 위험"에 적었다 — **후속 이슈 후보다.**
